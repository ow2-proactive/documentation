...
- _Job Status(es)_: You can specify the state of jobs you are looking for. The possible job statuses are: `Pending`, `Running`, `Stalled`, `Paused`, `In_Error`, `Finished`, `Canceled`, `Failed`, and `Killed`. For more information about job statuses, check the documentation link:../user/ProActiveUserGuide.html#_statuses[here.] Multiple values are accepted as well.

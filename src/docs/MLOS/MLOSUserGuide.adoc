...
- *Call of Predictions*: when a specific AI model is running, the user can request predictions for a specific payload. This later has to be converted into json data in order to get prediction's values.
...
| *Variable name* | *Description* | *Type*
| `MODEL_URL`
| A valid URL specified by the user leading to the model that needs to be deployed.
| URL
| `USER_NAME`
| A valid user name having the needed privileges to execute this action.
| String

...
Using MaaS, you can easily deploy and use any machine learning model as a Web Service on physical or virtual compute host on which there is an available Proactive Node.
...
Execute the Workflow by setting the different workflow's variables as described in Table <<table_maas>>.

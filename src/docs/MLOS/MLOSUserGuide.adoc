...
*Train_Image_Classification:* trains a model to classify ants and bees images. 
...
*Search_Train_Image_Classification:* queries images into search engine (Bing or DuckDuckGo) and trains a model to classify images.
...
*Search_Classify_Images:* queries images into search engine (Bing or DuckDuckGo) and predicts a model to classify rocket_vs_plane images. The pre-trained image classiffication model which is available on this https://activeeon-public.s3.eu-west-2.amazonaws.com/models/search_image_model.zip[link^].
...
| *Variable name* | *Description* | *Type*
| `SEARCH_ENGINE`
| Defines a host site to search and download satellite images.

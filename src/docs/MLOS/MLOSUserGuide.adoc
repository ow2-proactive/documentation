...
All Multi_Tuners_AutoML variants propose six algorithms for hyperparameters' optimization.  The choice of the sampling/search strategy depends strongly on the problem tackled.
Three variants of AutoML are proposed. Each variant comes with specific pipelines and visualization tools as described in the subsections below.
...
For all the tuning algorithms except the Bayesian one, the parallel pipeline is executed similar as `Multi_Tuners_AutoML_V1`. For the the bayesian one, the sequential one is executed. This pipeline both generates the hyperparameters' values and executes the associated trials sequentially.
...
| *Variable name* | *Description* | *Type*
| `MOUNT_LOG_PATH`
|  The path of the shared folder where the Tensorboard logs will be saved.
| String (default=/shared/$INSTANCE_NAME)
| `DOCKER_LOG_PATH`
|  The path of the folder where the Tensorboard logs will be saved in the docker container.
...
image::image_classification.png[300,300]
...
image::image_segmentation.png[300,300]
...
image::object_detection.png[300,300]

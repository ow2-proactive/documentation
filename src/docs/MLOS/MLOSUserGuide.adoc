...
B. By default, the ratio is 0.7. this means that 0.7 of the dataset will be used for training the model and 0.3 for testing it.
...
D. To be able to download the model learned by each algorithm, drag two <<Load_Model>> Tasks and connect them to each <<Train_Model>> Task.
...
*Keras_Image_Classification:* classifies a input image using deep ConvNets (specifically, VGG16) pre-trained on the ImageNet dataset.
...
*Tensorflow_Image_Prediction:* returns the flower specie of a given input image.
...
.Linear regression_Task variables
[cols="2,5,2"]
|===
| *Variable name* | *Description* | *Type*
| `N_JOBS`

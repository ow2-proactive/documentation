...
The life cycle of any ML MaaS instance (i.e., from starting the generic service instance, deploying an AI specific model to pausing or deleting the instance) can be managed in three different ways in MLOS :
...
- Using the *Cloud Automation Portal* by executing the different actions associated to ML MaaS (i.e., Model_Service, Deploy_AI_Model, Finish_Model_Service, Pause_Model_Service, Resume_Model_Service)
- Using the *Swagger UI* which is accessible once the ML MaaS instance is up and running.

The management of the life cycle of ML MaaS will be detailed in the next sub-sections.

Once a ML MaaS instance is up and running, it could be used for:
...
Using ML MaaS, you can easily deploy and use any machine learning model as a REST Web Service on a physical or a virtual compute host on which there is an available Proactive Node. Going through the Proactive Scheduler,
...
There are two ways to start a service instance (i.e., where you will deploy, update and call your model) using MLOS:
...
Open the endpoint in order to access the Swagger UI.
...
There are three ways to deploy a model in the running model_server (i.e., where you will deploy, update and call your specific AI model) using MLOS:
...
Finally you can also deploy a specific AI model directly from the Studio.
...
3+^|*Workflow variables*
| `SERVICE_TOKEN`
| A valid token generated by the MaaS Service for user authentication.
| String
| `DOCKER_ENABLED`
|   If True, the workflow will be executed on a Docker container. link:https://hub.docker.com/r/activeeon/dlm3/[activeeon/dlm3] image is used by default. If False, the required libraries should be installed in the different nodes that will be used.
| Boolean (default=True)
...
There are two ways to delete a ML MaaS instance using MLOS:
...
- *Pause_Model_Service*: This action will stop the service instance.

- *Resume_Model_Service*: This action will resume the paused service instance.
...
*Satellite Imagery Bucket* searching, downloading and retrieving the metadata of Sentinel satellite
images.

==== Fetch_Images_From_Satellite_Platforms
It allows to download the metadata and image from the Copernicus and PEPS platforms or both simultaneously.
...
| *Variable name* | *Description* | *Type*
| `HOST_SITES`
| Specify a sentinel product exploitation platform.
| List [Copernicus, Peps, All] (default=Peps)
| `OUTPUT_PATH`
| Address path to save downloaded images.
| String
|===


NOTE: Please access link:https://peps.cnes.fr/rocket/#/register[Peps] and link:https://scihub.copernicus.eu/dhus/#/self-registration[Copernicus] website to create a new user account.

==== Fetch_Satellite_Images_From_PEPS

*Task Overview:* Load and return a PEPS dataset containing a folder with the metadata and a folder with the downloaded images.
...
| `COLLECTION`
| Collection within theia collections.
...
WARNING: Please add two third party credentials (USER_NAME_PEPS and USER_PASS_PEPS) in the `Scheduling and Orchestration interface -> Manage Third-Party Credentials` to connect to PEPS platform.
...
*Task Overview:* Load and return a PEPS dataset containing a folder with the metadata and a folder with the downloaded images according to the resolution & image band selected.
...
|  Boolean (default=True)
| `SPATIAL_RESOLUTION`
| Granule dimensions for each resolution band.
|  List [10m, 20m, 60m] (default=10m)
| `IMAGE_BAND`
| Choose from 13 spectral bands spanning from the Visible and Near Infra-Red (VNIR) to the Short Wave Infra-Red (SWIR).
...
WARNING: Please add two third party credentials (USER_NAME_COP and USER_PASS_COP) in the `Scheduling and Orchestration interface -> Manage Third-Party Credentials` to connect to Copernicus platform.

...
The `auto-ml optimization` bucket contains multiple tuning workflows that can be easily used to find the
operating parameters for any system whose performance can be measured as a function of adjustable parameters. These algorithms are specially used for automated tuning of the hyperparameters of machine learning systems (e.g.,
learning rates, or the number of hidden layers in a deep neural network).

//NOTE: The proposed tuning algorithms uses as a backend the https://scikit-learn.org/stable/[Chocolate library^].
==== Objective Functions

===== Himmelblau_Function

*Task Overview:* Is a multi-modal function, used to test the performance of optimization algorithms.

.Himmelblau_Function_Task variables
[cols="2,5,2"]
|===
...
| Dictionary (default={"x": 3.5976705887960527, "y": -1.8471754188451843})
| `TOKEN`
...
.Bayesian_Minimization_Task variables
[cols="2,5,2"]
|===
| *Variable name* | *Description* | *Type*
| `CALL_WORKFLOW`
...
| Dictionary (see search space representation below)
| `MAX_ITERATIONS`
...
| String (default=-1)
| `VISDOM_ENABLED`
|  If True, the results will be plotted in real time in teh Visdom dashboard.
| String (default=-1)
| `DOCKER_ENABLED`
|   If True,  the workflow will be executed on a Docker container. link:https://hub.docker.com/r/activeeon/dlm3/[activeeon/dlm3] image is used by default. If False, the required libraries should be installed in the different nodes that will be used.
| Boolean (default=True)
...
This subsection describes common building blocks to define a search space:
    ...
    - Distribution
    - ContinuousDistribution : Representation of continuous distributions.
    - QuantizedDistribution : Representation of quantized distributions.
...
*How to use this task:* It should be used after a classification, a regression or an anomaly algorithm, such as <<Support_Vector_Machines>>, <<Gaussian_Naive_Bayes>>, <<Linear_Regression>>, <<Bayesian_Ridge_Regression>>, <<Linear_Regression>>, <<Support_Vector_Regression>>, <<Isolation_Forest>> and <<One_Class_SVM>>.

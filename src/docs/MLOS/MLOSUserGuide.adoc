...
constructs tasks and workflows to execute them on the fly.
...
==== Using https://pypi.org/[PyPi]

* open a terminal

* install the proactive jupyter kernel with thee following commands:
...
We recommend to use of jupyter lab. To launch it from your terminal after having installed it:
...
To create a task, use the pragma `#%task()` followed by the task implementation script written to a notebook block code.
...
details about possible options:
...
https://doc.activeeon.com/latest/user/ProActiveUserGuide.html#_generic_information[Generic Information], you should the
provide the `generic_info` parameter. Its value should be a list of tuples `(key,value)` that corresponds the names and
adequate values of the Generic Information. Example:
...
The `export` and `import` parameters make variables propagation possible between the different tasks of a workflow.
If `myTask1` variables `var1` and `var2` are needed in `myTask2`, both pragmas have to specify this information as follow:
...
It is possible to use an external implementation file as a task implementation. To do so, the option `path` should be used.
Example:
...
To add a selection script to a task, use the `#%selection_script()` pragma. A first way to do it is by
providing the name of the corresponding task, and the selection code implementation after that:
...
If the job has already been created, the call of this pragma would just rename the job already created by the new provided name.

**Notice that it is not necessary to create and name explicitly the job. If not done by the user, this step is implicitly
performed when the job is submitted (check section <<Submit your job to the scheduler>> for more information).**

==== Visualize job

To visualize the created workflow, use the `#%draw_job()` pragma to plot the graph that represents the job into a separate window:
...
If this parameter is set to 'off', the workflow plotting is made through the https://matplotlib.org/[Matplotlib]
...
==== Save workflow in dot format

To save the created workflow into a https://www.graphviz.org/[GraphViz] _.dot_ format, use the `#%write_dot()` pragma:
...
Isf the job's name is not set, the ProActive kernel uses the current notebook name, if possible, or gives a random one.

==== List all submitted jobs

To get all the submitted job ids and names, use `list_submitted_jobs` pragma this way:

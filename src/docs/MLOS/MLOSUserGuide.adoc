...
Notice that if the language is not specified, Python is considered as default language.
...
the corresponding task and the selection code implementation after that
...
If the job has already been created, the call of this pragma would just rename the already created job to the new
provided name.

**Notice that it is not necessary to create and assign a name explicitly the job. If not done by the user, this step is
...
If this parameter is set to `off`, plotting the workflow graph is done is made through a
...
==== Save the workflow graph in dot format

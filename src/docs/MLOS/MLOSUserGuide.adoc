...
=== Multi_Tuners_Auto_ML

Multi_Tuners_Auto_ML propose six algorithms for hyperparameters' optimization. The choice of the
sampling/search strategy depends strongly on the tackled problem.
Multi_Tuners_Auto_ML comes with specific pipelines (parallel or sequential) and visualization tools
(Visdom or Tensorboard) as described in the subsections below.

*Variables:*

.Multi_Tuners_Auto_ML Variables
[cols="2,5,2"]
|===
| *Variable name* | *Description* | *Type*

| `NS`
| Specifies the Node Source where the processing will take place. In ProActive, SLURM is used as a Node Source.
| String [default=empty]
| `NS_BATCH`
| Specifies the type of the Node Source to be used. It can be either CPU or GPU. In ProActive,
    GPU resources are used for the SLURM Node Source.
| String [default=empty]
| `NODE_ACCESS_TOKEN`
| The token ID with which the Node Source can be accessed.
| String [default=empty]
| `NODE_SOURCE_NAME`
...
| `VISDOM_PROXYFIED`
| If True, requests to Visdom are sent to a proxy server before being sent to the destination server.
| Boolean (default=False)
| `VISDOM_ENABLE_LOGIN`
...
| Specifies the name to be provided to the Visdom instance.
...
| Specifies the name to be provided to the Tensorboard instance.
| String [default=tensorboard-server]
| `TENSORBOARD_LOG_PATH`
| Specifies the path where the Tensorboard execution log can be found.
| String [default=/shared/$TENSORBOARD_INSTANCE_NAME]
| `TENSORBOARD_DOCKER_LOG_PATH`
| Specifies the path where the execution log of the tensorboard docker image can be found.
| String [default=/graphs/$TENSORBOARD_INSTANCE_NAME]
|===

image::AutoML_Full.png[1000,700,align=center]
...
These workflows have several common variables as in Multi_Tuners_Auto_ML. Some of the workflows are characterized
...
.CIFAR10 CNN Variables
[cols="2,5,5"]
|===
| *Variable name* | *Description* | *Type*

| `CONTAINER_PLATFORM`
| Specifies the platform of the container that packages up the code and all its dependencies for running the application.
| List(docker, podman, singularity) [default=docker]
| `CONTAINER_GPU_ENABLED`
| If True, containers will run based on images containing libraries that are compatible with GPU.
| Boolean [default=True]
| `CONTAINER_IMAGE`
| Specifies the path of the file that contains the executable code for running the process in an isolated manner.
...
| `NUM_EPOCHS`
| The number of times that model training is repeated for minimizing the error.
| Integer [default=3]
| `INPUT_VARIABLES`
| A set of specific variables (usecase-related) that are used in the model training process.
| JSON format [default={"OPTIMIZER": "Adam","LEARNING_RATE": 0.0001, "BATCH_SIZE": 32, "WEIGHT_DECAY": 0.0005}]
| `SEARCH_SPACE`
| Specifies the representation of the search space which has to be defined using dictionaries.
| JSON format [default={"OPTIMIZER": choice(["Adam", "SGD", "RMSprop"]),"LEARNING_RATE": choice([0.0001, 0.00025]), "BATCH_SIZE": choice([32, 64]), "WEIGHT_DECAY": choice([0.0005, 0.005])}]
| `INSTANCE_NAME`
| Specifies the name to be provided for the instance.
| String [default=tensorboard-server]
| `DOCKER_LOG_PATH`
| The path for the file containing the execution log of the docker image.
| String [default=/graphs/$INSTANCE_NAME]
| `MOUNT_LOG_PATH`
| The path of the file in which the mount points can be monitored.
...
| `INPUT_VARIABLES`
| A set of specific variables that are used in the model training process.
| JSON format [default={"OPTIMIZER": "Adam","LEARNING_RATE": 0.0001, "BATCH_SIZE": 64, "WEIGHT_DECAY": 0.0005}]
...
.Diabetics Detection Using Logistic Regression Variables
[cols="2,5,5"]
|===
| *Variable name* | *Description* | *Type*
...
| `DOCKER_ENABLED`
| If True, the scripts of the workflows will be executed using docker containers.
| Boolean [default=True]
| `DOCKER_GPU_ENABLED`
| If True, containers will run based on docker images containing libraries that are compatible with GPU.
| Boolean [default=False]
| `DOCKER_IMAGE`
| The location of the docker image file, that is used to execute code in a Docker container.
| List(activeeon/dlm3, activeeon/cuda) [default=empty]
| `INPUT_VARIABLES`
| A set of specific variables (usecase-related) that are used in the model training process.
| JSON format [default={"MAX_ITERATIONS": 10, "SOLVER": "liblinear", "PENALTY": "l1"}]
| `SEARCH_SPACE`
| Specifies the representation of the search space which has to be defined using dictionaries.
| JSON format [default={"MAX_ITERATIONS": choice([5,10,15]), "SOLVER": choice(["liblinear","saga"]), "PENALTY": choice(["l1", "l2"])}]
...
.Image Object Detection Variables
[cols="2,5,5"]
|===
| *Variable name* | *Description* | *Type*
...
| `INPUT_VARIABLES`
| A set of specific variables (usecase-related) that are used in the model training process.
| JSON format [default={{"LEARNING_RATE" : 0.0025, "CONF_THRESHOLD" : 0.1, "NMS_THRESHOLD" : 0.35, "MOMENTUM" : 0.09, "WEIGHT_DECAY" : 0.005}]
| `SEARCH_SPACE`
| Specifies the representation of the search space which has to be defined using dictionaries.
| JSON format default[{"LEARNING_RATE": choice([0.0001, 0.00025]), "CONF_THRESHOLD": choice([0.1, 0.5]), "NMS_THRESHOLD": choice([0.35, 0.45]), "MOMENTUM": choice([0.0009, 0.009]), "WEIGHT_DECAY": choice([0.0005, 0.005])}]

|===

*Multiple SVM Models:* train multiple types of SVMs models that can be optimized using AutoML.

.Multiple SVM Models Variables
[cols="2,5,5"]
|===
| *Variable name* | *Description* | *Type*
...
| `SEARCH_SPACE`
| Specifies the representation of the search space which has to be defined using dictionaries.
| JSON format [default={"algo" : "SVC", "kernel" : "rbf", "C" : 0.2, "gamma" : 0.1}]
| `INPUT_VARIABLES`
| A set of specific variables (usecase-related) that are used in the model training process.
| JSON format [default=[{"algo" : "SVC", "kernel" : "rbf",               "C" : log(low=-2, high=10, base=10),               "gamma" : log(low=-9, high=3, base=10)},          {"algo" : "SVC", "kernel" : "poly",               "C" : log(low=-2, high=10, base=10),               "gamma" : log(low=-9, high=3, base=10),               "degree" : quantized_uniform(low=1, high=5, step=1),               "coef0" : uniform(low=-1, high=1)},          {"algo" : "LinearSVC",               "C" : log(low=-2, high=10, base=10)}]]
|===

*Text_Generation:* train a simple Long Short-Term Memory (LSTM) to learn sequences of characters from 'The Alchemist' book. It's a novel by Brazilian author Paulo Coelho that was first published in 1988.

.Text_Generation Variables
[cols="2,5,5"]
|===
| *Variable name* | *Description* | *Type*
...
| `INPUT_VARIABLES`
| A set of specific variables (usecase-related) that are used in the model training process.
| JSON format [default=empty]
| `SEARCH_SPACE`
| Specifies the representation of the search space which has to be defined using dictionaries.
| JSON format [default={"OPTIMIZER": choice(["Adam", "RMSprop"]), "EPOCHS": choice([2000, 2030]), "MAXLEN": choice([40, 90, 100, 200, 400]), "UNIT": choice([256, 600, 700]), "DROPOUT": choice([0.004, 0.4])}]
...
| `SEARCH_SPACE`
| Specifies the representation of the search space which has to be defined using dictionaries.
| JSON format [default={'x': uniform(-6, 6), 'y': uniform(-6, 6)}]
| `INPUT_VARIABLES`
| A set of specific variables (usecase-related) that are used in the model training process.
...
The following workflows represent python templates that can be optimized by the AutoML tuners.

*Python_Task:* is a simple python task template that can be used for AutoML.

.Python_Task Variables
[cols="2,5,5"]
|===
| *Variable name* | *Description* | *Type*

| `TOKEN`
| Specifies an ID that is used to access the deployed python task.
| JSON format [default={"_token_id": 0}]
| `SEARCH_SPACE`
| Specifies the representation of the search space which has to be defined using dictionaries.
| JSON format [default={"OPTIMIZER": choice(["Adam", "SGD", "RMSprop"])}]
| `INPUT_VARIABLES`
| A set of specific variables (usecase-related) that are used in the model training process.
| JSON format [default={"OPTIMIZER": "Adam"}]
|===
...
Once a predictive model is built, tested and validated, you can easily use it in real world production pipelines by deploying it as a REST Web Service.
This will be particularly useful for engineering or business teams that want to take advantage of this model.

The life cycle of any MLaaS instance (i.e., from starting the generic service instance, deploying an AI specific model to pausing or deleting the instance) can be managed in three different ways in MLOS :

- Using the *Studio Portal* and more specifically the bucket *model-as-a-service* where specific generic tasks are provided to process all the possible actions (i.e., Model_Service_Start, Deploy_AI_Model, Call_Prediction_Service, Model_Service_Actions[Finish/Pause/Resume]).
These tasks can be easily integrated to your AI pipelines/workflows as you can see in this <<Deployment Pipeline Example>>.
- Using the *Cloud Automation Portal* by executing the different actions associated to MLaaS (i.e. Deploy_Model_Service, Update_Model_Service, Pause_Model_Service, Resume_Model_Service, Finish_Model_Service.)
- Using the *Swagger UI* which is accessible once the MLaaS instance is up and running.

The management of the life cycle of MLaaS will be detailed in the next sub-sections.

Once a MLaaS instance is up and running, it could be used for:
...
Using MLaaS, you can easily deploy and use any machine learning model as a REST Web Service on a physical or a virtual compute host on which there is an available Proactive Node. Going through the Proactive Scheduler,
...
In the following subsections, we will illustrate the life cycle of a MLaaS instance, from starting the generic service instance,
deploying a specific model, pausing it, to deleting the instance. We will also talk about how the life cycle of a MLaaS instance
can be managed via three different ways in MLOS:
...
Drag and drop the `Start_Model_Service` task from the bucket.
...
| String (default="Model_Service")
| `INSTANCE_NAME`
| The name of the service that will be deployed.
| String (default="model-server-${PA_JOB_ID}")
| `DOCKER_IMAGE`
| The Docker image that contains all the dependencies and libraries needed by the model specific AI model to be deployed.
| String (default=link:https://hub.docker.com/r/activeeon/model_as_a_service/[activeeon/model_as_a_service])
|===

==== Deploy a Specific AI Model

You can also deploy a specific AI model directly from the link:https://try.activeeon.com/Studio[Studio Portal].

Drag and drop the `Deploy_AI_Model` task from the MLaaS bucket.

Execute the workflow and set the different workflow's variables as follows:

.Deploy_AI_Model variables
[cols="2,5,2"]
|===
| *Variable name* | *Description* | *Type*
3+^|*Workflow variables*
...
| String (default=Empty).
| `SERVICE_TOKEN`
| A valid token generated by the MLaaS Service for user authentication.
| String (default=Empty).
| `DOCKER_ENABLED`
|  If True, the workflow will be executed on a Docker container. link:https://hub.docker.com/r/activeeon/model_as_a_service/[activeeon/model_as_a_service] image is used by default. If False, the required libraries should be installed in the different nodes that will be used.
| Boolean (default=True).
3+^|*Task variables*
...
Drag and drop the `Call_Prediction_Service` task from the MlaaS bucket.
...
Drag and drop the `Model_Service_Actions` task from the MLaaS bucket.
...
| `DOCKER_IMAGE`
| The Docker image that contains all the dependencies and libraries needed by the model specific AI model to be deployed.
| String (default="activeeon/model_as_a_service")
| `HTTPS_ENABLED`
...
image::NEW_MAS_Model_Servie_PCA.png[500,500,align=center]

==== Deploy a Specific AI Model

Once the status of your generic model service is displayed as `RUNNING` on `Cloud Automation`, you can deploy your model by following the steps below :

Select and execute the `Deploy_Model_Action` from Actions to deploy your model.
...
==== Delete (Finish)/Update/Pause the Service

You can delete the launched service instance directly from the Cloud Automation portal:

Open the link:https://try.activeeon.com/automation-dashboard/#/portal/cloud-automation[Cloud Automation Portal].

Set the action `Finish_Service` under Actions and click on execute.

image::MAS_Delete_Service.png[align=center]

There are also two other actions that can be executed from the Cloud Automation Portal which are:

- *Deploy_Model_Service*: This action will put the service instance into deployment.

- *Update_Model_Service*: This action will update the deployed instance according to the updated variables.

- *Resume_Model_Service*: This action will resume the paused service instance. (This no longer exists)

- *Pause_Model_Service*: This action will pause the service instance.

- *Finish_Model_Service*: This action will finish and delete the service instance
...
==== Deploy/Undeploy/Redeploy a Specific AI Model

You can also deploy a specific AI model using the Swagger UI:
...
This example trains an Iris Flower Classifier, starts a service instance where the trained model is deployed and the input data is scored by consuming the endpoints exposed by the model_service.

image::NEW_MAS_IRIS_Workflow_Example.png[align=center]
...
| *Variable name* | *Description* | *Type*
| `DOCKER_ENABLED`
| If True, the task will be executed on a Docker container. link:https://hub.docker.com/r/activeeon/dlm3/[activeeon/dlm3] image is used by default. If False, the required libraries should be installed in the different nodes that will be used.
| Boolean (default=True)
| `DOCKER_IMAGE`
...
image::Fetch_Satellite_Images.png[align=center]
...
| List [Peps, Copernicus, All] (default=Peps)
| `OUTPUT_PATH`
| Specifies the path where the data should be downloaded.
| String
|===
...
|  If True, it uses the date defined in the START_DATE and START_DATE fields, otherwise it uses the dates defined in th .geojson file.
|  Boolean (default=True)
| `SPATIAL_RESOLUTION`

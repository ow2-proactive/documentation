...
*CIFAR_10_Image_Classification:* train a simple deep CNN on the CIFAR10 images dataset using the Keras library.
...
*Iris_Classification:* trains multiple types of SVMs models on the Iris flower dataset.
...
*Horovod_Job:* is a template to implement a native horovod task with multi-gpu support.

*Horovod_Job_Slurm:* is a template to implement a native horovod task using SLURM scheduler task with multi-gpu support.

*Horovod_Job_Docker:* is a template to implement a horovod task using docker with multi-gpu support.
...
| *Variable name* | *Description* | *Type*
| `NATIVE_SCHEDULER`
| Name of the Native Scheduler node source to use when the workflowtasks must be deployed inside a cluster such as SLURM, LSF, etc.
| String (default=empty)
...
*Check_Visdom_Support:* checks if the user wants (or not) start the Visdom service.
...
*Tensorboard_Realtime_CIFAR10_Training:* shows an example of real-time using Tensorboard for training a CNN using CIFAR10 database.

*Tensorboard_Plots_Example:* shows an example exposing the different plots available in Tensorboard.

*Check_Tensorboard_Support:* checks if the user wants (or not) start the Tensorboard service.

...
Suppose you need to predict houses prices based on the information provided by the estate agency:
...
The prediction of houses prices is a complex problem, but we can simplify it a bit for this step by step example. We'll show you how you can easily create a predictive analytics solution using Machine Learning Open Studio.
...
1. Upload your dataset into a cloud storage service for example https://www.aws.amazon.com/s3‎[Amazon S3^], once the data has been converted to *CSV* format.
...
For this example, we will use `Liner_Regression` Task and `Support_Vector_Regression` Task.

1. Find the `Liner_Regression` Task and `Support_Vector_Regression` Task and drag them into the canvas.
...
4. Find the `Export_Result` Task in the Machine Learning bucket and drag and drop it twice into the canvas.

5. Connect each `Export_Result` Task with `Predict_Model`.

NOTE: if you have a pkl predictive model that you have learned using another platform and you need to test it in the Machine Learning Open Studio, you can load it using *Load_Model* Task.
...
*How to use this task:* Should be connected with `Train_Model` or `Train_Clustering_Model` or `Predict_Model` / `Predict_Clustering_Model`.

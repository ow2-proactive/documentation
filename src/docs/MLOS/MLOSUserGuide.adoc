...
Multi_Tuners_Auto_ML proposes six algorithms for hyperparameters' optimization. The choice of the
...
*Diabetics_Detection:* trains a logistic regression model using MLOS machine learning generic tasks in order to detect diabetics.

*Image_Object_Detection:* trains a YOLO model on the coco dataset using MLOS deep learning generic tasks.
...
In the description below, multiple tables represent the main variables that characterize the MLaaS workflows.
In addition to the variables mentioned below, there is a set of generic variables that are common between all workflows
which can be found in the subsection <<AI Workflows Common Variables>>.
...
| String (default="/tmp/model_service.sif")
| `TRACE_ENABLED`
...
When running the Model Service with Singularity as an Engine, *Pause_Model_Service* and
*Resume_Model_Service* actions can not be executed.
...
In machine learning, data drift is the modification of the model input data (relative to the
training data) that leads to a degradation of the model performance.
Monitoring data drifts allows to detect
these model performance problems and try to adapt to them as in the figure below. To deal with this problem, we have integrated a data drift detection mechanism in the Machine Learning as a Service module of ProActive.
...
This list is likely to be expanded to include other drift detection methods.
...
As the DDD function is part of the MLaaS module,
it can also be launched from different Proactive portals.

==== Via Proactive Studio Portal
The data drift detection function is added to the tasks and workflows of the bucket
*model_as_a_service*. This function is directly linked to the deployment of the model
in MLaaS (where the user deploys the model, its metadata and a part of the
old training data set to be used for drift detection in the new input data set),
and to the call of the prediction service in MLaaS (where the drift detector is chosen
and the detection process is implemented using the chosen detector).
...
*Deploy_AI_Model* and *Call_Prediction_Service* have been mainly modified to include the DDD
features.

In particular, in the *Deploy_AI_Model* task, the user is asked to enter
...
use the concatenated data to extract the rows where the drift took place in the new data.
...
As we have mentioned earlier in this documentation, a model can be deployed using the *Model Service* in the
Automation Dashboard Portal. Once the service is launched, the model can be deployed by
choosing *Deploy_Model_Service* action. Before the deployment, the user can specify the url of the
baseline_data using the *BASELINE_DATA_URL* attribute that appears in the popup window of
 *Deploy_Model_Service* action. In case you need to change the baseline data, it can be updated using the *BASELINE_DATA_URL*
attribute of the *Update_MaaS_ML* action of MaaS_ML.

Once the model is deployed via the Cloud Automation portal,
the Swagger user interface can be opened via the MaaS_ML instance endpoint, offering different api's
to help the user manage the drift detection function. This
function has been particularly integrated in the three api's: the deployment api,
the prediction api and the update api. In the */deploy()* api, a user can choose the model
(using *model_file* attribute)
and its associated baseline data (using *baseline_data* attribute)
to be deployed. In the */predict()* api, the user specifies the
drift detection method and the input data to be predicted. Using the baseline data and the
specified drift detector method, our algorithm can detect in which attributes and particularly
which
rows the drift took place in the input data. The results are shown in *Response Body*
of the */predict()* api and in the *Traceability and Audit* page. In the */update*
api, the user is able to update the baseline data deployed with the model using the *baseline_data*
attribute.
...
most of the available AI workflows in MLOS associated to their descriptions.
...
NOTE: PyTorch were used to build the model architecture based on https://pytorch.org/docs/stable/torchvision/models.html[AlexNet^].
...
NOTE: PyTorch were used to build the model architecture based on https://pytorch.org/docs/stable/torchvision/models.html[DenseNet-161^].
...
NOTE: PyTorch were used to build the model architecture based on https://pytorch.org/docs/stable/torchvision/models.html[ResNet-18^].
...
NOTE: PyTorch were used to build the model architecture based on https://pytorch.org/docs/stable/torchvision/models.html[VGG-16^].
...
*Task Overview:* Manage the life-cycle of TensorBoard PCA service. It allows to trigger three possible actions: Pause_Tensorboard, Resume_Tensorboard and Finish_Tensorboard.

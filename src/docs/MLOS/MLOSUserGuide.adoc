...
Monitoring data drifts allows to detect these model performance problems and try to adapt
to them as in the figure below. To deal with this problem, we have integrated a data drift
...
function has been particularly integrated in the three endpoints: the deployment endpoint,
the prediction endpoint and the update endpoint. In the */deploy()* endpoint, a user can choose the model
(using *model_file* variable)
and its associated baseline data (using *baseline_data* variable)
to be deployed. In the */predict()* endpoint, the user specifies the
...
of the */predict()* endpoint and in the *Traceability and Audit* page. In the */update*
endpoint, the user is able to update the baseline data deployed with the model using the *baseline_data*
variable.

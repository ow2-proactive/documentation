...
- +++Add services using <a class="automationDashboardUrl" href="/automation-dashboard/#/portal/cloud-automation" target="_blank">Service Automation Portal</a>+++
- +++Perform event based scheduling using <a class="automationDashboardUrl" href="/automation-dashboard/#/portal/cloud-watch" target="_blank">Event Orchestration Portal</a>+++
...
B. Drag and drop the <<Import_Data>> task from the *machine-learning* bucket in the Machine Learning Open Studio.
...
*Diabetics_Detection:* trains a logistic regression model using PML machine learning generic tasks in order to detect diabetics.
...
MaaS_ML is dedicated to making deployments of lightweight machine learning (ML) models simple, portable, and scalable, and to easily managing their lifetimes. This will be particularly useful for engineering or business teams that want to take advantage of this model.
...
- Using the *Studio Portal* and more specifically the bucket *model-as-a-service* where specific generic tasks are provided to process all the possible actions (i.e., MaaS_ML_Service_Start, Deploy_ML_Model, Call_Prediction_Endpoint, MaaS_ML_Actions[Finish/Pause/Resume]).
...
| The size of the data to be extracted from the old training dataset.
| Integer (default=50).
|===
...
.Call_Prediction_Endpoint variables
[cols="2,5,2"]
|===
| *Variable name* | *Description* | *Type*
3+^|*Workflow variables*
...
image::NEW_MAAS_ML_Service_PSA.PNG[500,500,align=center]
...
There are also other actions that can be executed from the Service Automation Portal which are:
...
The following workflow example is available on the `model_as_a_service` bucket under the name of `IRIS_Deploy_Flower_Classifier_Model`.
This example trains an Iris Flower Classifier, starts a service instance where the trained model is deployed and the input data is scored by consuming the endpoints exposed by the maas_ml service.
...
these model performance problems and try to adapt to them as in the figure below. To deal with this problem, we have integrated a data drift detection mechanism in the Model as a Service for Machine Learning module of ProActive.
...
in MaaS_ML (where the user deploys the model, its metadata and a part of the
...
*Deploy_ML_Model* and *Call_Prediction_Endpoint* have been mainly modified to include the DDD
features.
...
detection process that will be enacted in the *Call_Prediction_Endpoint*.

In the *Call_Prediction_Endpoint*, the user is asked to choose the data drift
...
As we have mentioned earlier in this documentation, a model can be deployed using the *MaaS_ML* service in the
Automation Dashboard Portal. Once the service is launched, the model can be deployed by
choosing *Deploy_ML_Model* action. Before the deployment, the user can specify the url of the
baseline_data using the *BASELINE_DATA_URL* attribute that appears in the popup window of
 *Deploy_ML_Model* action.
...
- Using the *Studio Portal* and more specifically the bucket *model-as-a-service* where specific generic tasks are provided to process all the possible actions (i.e., MaaS_DL_Service_Start, Deploy_DL_Model, MaaS_DL_Actions[Finish/Pause/Resume], Undeploy_DL_Model).
These tasks can be easily integrated to your AI pipelines/workflows as you can see in this <<Deployment Pipeline Example>>.
...
A. Open +++<a class="studioUrl" href="/studio" target="_blank">Machine Learning Open Studio</a>+++ home page.

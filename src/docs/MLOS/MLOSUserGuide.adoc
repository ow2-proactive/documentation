...
The `satellite-imagery` bucket contains some tasks that enable you search, download and retrieve the metadata of Sentinel satellite images from the https://peps.cnes.fr/rocket/#/home[PEPS website^] and https://scihub.copernicus.eu/[Copernicus Open Access Hub^].
...
| *Variable name* | *Description* | *Type*
| `HOST_SITES`
| Defines a utility to search and download satellite images.
| List [Peps, Copernicus, All] (default=Peps)
| `OUTPUT_PATH`
| Specifies an address path to save downloaded images.
| String
|===
...
*Task Overview:* Load and return a PEPS dataset containing a `metadata folder` containing metadata files and `images folder` containing satellite images.
...
| `PLATFORM_NAME`
| Specifies instrument on a Sentinel satellite.
| List [S1, S2, S2ST, S3] (default=S2)
| `PRODUCT_TYPE`
|  Limits search to a Sentinel product type.
|  List [GRD, SLC, OCN (for S1) or S2MSI1C S2MSI2A S2MSI2Ap (for S2)] (default=S2MSI1C)
| `SENSOR_MODE`
| Specifies search to a Sentinel sensor mode.
| List [EW, IW , SM, WV (for S1) or INS-NOBS, INS-RAW (for S3)] (default=INS-NOBS)
| `START_DATE`
| Starts date of the query in the format YYYYMMDD.
| String
| `END_DATE`
| Ends date of the query in the format YYYYMMDD.
| String
| `TILE`
| Limits search to a tile number.
| String
| `LATITUDE`
|  Determines search to a latitude in decimal degrees.
|  Float
| `LONGITUDE`
|  Limits search to a longitude in decimal degrees.
|  Float
| `OUTPUT_PATH`
|  Specifies an address path to save downloaded images.
|  String
...
*Task Overview:* Load and return a Copernicus dataset containing a `metadata folder` containing metadata files and `images folder` containing satellite images according to the resolution & image band selected by user.
...
WARNING: Please add third party credentials (USER_NAME_COP and USER_PASS_COP) in the `Scheduling & Orchestration interface -> Manage Third-Party Credentials` to connect to Copernicus platform.

...
=== What's Machine Learning Open Studio (MLOS)?
...
Suppose you need to predict houses prices based on these information(features) provided by the estate agency:
...
1. Open http://try.activeeon.com/studio[Proactive Workflow Studio^] home page.

3. Create a new workflow.

4. Fill the Workflow `General Parameters`.

2. Click on `Catalog` menu then `Set Bucket as Main Catalog Menu` and select `machine-learning` bucket.
This can also be achieved by adding `/templates/machine-learning` at the end of the URL of the proactive workflow studio.

3. Click on `Catalog` menu then `Add Bucket as Extra Catalog Menu` and select `data-visualization` bucket.
...
1. Once your dataset has been converted to *CSV* format, upload it into a cloud storage service for example https://www.aws.amazon.com/s3‎[Amazon S3^]. For this tutorial, we will use Boston house prices dataset available on this link:
...
2. By default, the default ratio is 0.7. this means that 0.7 of the dataset will be used for training the model and 0.3 for testing it.
...
2. Find the `Train_Mode` Task and drag it twice into the canvas.

3. Connect the <<Split_Data>> Task to  the two <<Train_Model>> Tasks in order to give it access to the training data. Connect then the <<Support_Vector_Regression>> Task to the first <<Train_Model>> Task and <<Support_Vector_Regression>> to the second <<Train_Model>> Task.

4. To be able to download the model learned by each algorithm, drag two <<Load_Model>> Tasks and connect them to each <<Train_Model>> Task.
...
2. Connect the first <<Predict_Model>> Task to the <<Train_Model>> Task that's connected to <<Support_Vector_Regression>> Task.

3. Connect the second <<Predict_Model>> Task to the <<Train_Model>> Task that's connected to `Liner_Regression` Task.
...
NOTE: if you have a pickled file (.pkl) containing a predictive model that you have learned using another platform and you need to test it in the Machine Learning Open Studio, you can load it using *Load_Model* Task.

=== Run the Experiment and Preview the Results

Now the workflow is completed and should look like this:
...
5. Click the `Tasks` tab and click open the `Preview` tab and then click whether on `Open in browser` to preview the results on your browser or on `Save as file` to download the results locally.
...
The Machine Learning Open Studio provides a fast, easy and practice way to execute different workflows using the machine learning  bucket. We present useful machine learning workflows for different applications in the following sub-sections.
...
2. Click on `Catalog` menu then `Add Bucket as Extra Catalog Menu` and select `machine-learning-workflows` bucket.
...
More details about these workflows are available in this link:https://www.activeeon.com/resources/automated-machine-learning-activeeon.pdf[link]

=== Basic Machine Learning

The following workflows present some machine learning basic examples. These workflows are built using generic machine learning tasks available on the machine learning bucket.
...
The following workflows present useful computer vision applications using deep learning based algorithms by convolutional neural networks for image recognition, object detection, anomaly detection, and image segmentation. Open source libraries, such as TensorFlow, Keras, Caffe, OpenCV, PyTorch and scikit-learn are used as a backend for AI workflows.
...
A demo video of these workflows is available in https://www.activeeon.com/public_content/documentation/latest/user/ProActiveUserGuide.html#_publish_current_workflow_to_the_catalog[Activeeon Youtube Channel^].

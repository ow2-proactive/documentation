...
these model performance problems and try to adapt to them as in the figure below. It is in this context that we were interested in the
integration of data drift detection in the Machine Learning as a Service module of
ProActive.

image::drift.png[align=center]

To cope with this, we tackled the discovery of data drifts at a fine level of granularity.
We have developed a DDD functionality that not only allows us to discover that a drift
 ...
 *Deploy_Model_Service* action.

Once the model is deployed via the Cloud Automation portal,
The Swagger user interface can be opened, offering different api's
to help the user manage the drift detection function. This
function has been particularly integrated in the two api's; the deployment api
and the prediction api. In the */deploy()* api, a user can choose the model
...
of the */predict()* api and in the *Traceability and Audit* page.

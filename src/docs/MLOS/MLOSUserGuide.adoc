...
Using MLOS, you can deploy any machine learning model as a web service in any compute resource deployed on Proactive.

The following deployment steps are similar wherever you need to deploy your model:
...
There is two ways to start a web service (i.e. where you will deploy, update and call your model) using MLOS:
...
| `YAML_FILE`
| A YAML file that describes the OpenAPI Specification (known as Swagger Spec) of the service. This file should be stored in the catalog under the `model_as_service_resources` bucket. This file should be stored in the Catalog. More information about the structure of this file can be found in the section <<Customize the Service>>.
| String (default="ml_service-api").
| `PYTHON_ENTRYPOINT`
| This entry script starts the service and defines the different functions to deploy the model, score the requests by using the model, and return the results. This specific script is specific to your model. This file should be stored in the Catalog. More information about this file can be found in the <<Customize the Service>> section.
...
Execute the Workflow by setting the different workflow variables as following:
...
| String (default="model-server")
| `YAML_FILE`
| A YAML file that describes the OpenAPI Specification (known as Swagger Spec) of the service. This file should be stored in the Catalog under the `model_as_service_resources` bucket. More information about the structure of this file can be found in the section <<Customize the Service>>.
| String (default="ml_service-api").
| `PYTHON_ENTRYPOINT`
| This entry script starts the service and defines the different functions to deploy the model, score the requests by using the model, and return the results. This specific script is specific to your model. This file should be stored in the Catalog under the `model_as_service_resources` bucket. More information about this file can be found in the <<Customize the Service>> section.
| String (default="ml_service").
|===
...
There is two ways to deploy a model in the started model_server (i.e. where you will deploy, update and call your model) using MLOS:
...
Execute the Workflow and set the different workflow variables as following:
...
| URL
| `SERVICE_TOKEN`
| the api token to be able to deploy the model.
| String
|===
...
There is two ways to call the web service to score input data using MLOS:
...
| JSON
| `LABEL_COLUMN`
| need to be set if data is labeled.
| String (default='class')
|===

=== Delete the service

There is two ways to delete a web service (i.e. where you will deploy, update and call your model) using MLOS:
...
This example trains an Iris Flower Classifier, starts a web service where the trained model is deployed and input data is scored by consuming the endpoints exposed by the model_service.
...
It is possible to edit this script to make it more customized to your model. The entry script must take into consideration:

* the list of users that are allowed to consume the service endpoints
* the format of the model expected by the deployment and the prediction functions
* the format of the incoming data
* the data format expected by the model
...
If you need to use your own image to start the service. You need to install the following libraries in your image :

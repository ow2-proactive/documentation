...
The `auto-ml-optimization` bucket contains multiple tuning workflows that can be easily used to find the operating parameters for any system whose performance can be measured as a function of adjustable parameters. These algorithms are specially used for automated tuning of the hyperparameters of machine learning systems (e.g. learning rates, or the number of hidden layers in a deep neural network).

...
Once a predictive model is built, tested and validated, you can easily use it in real world production pipelines by deploying it as a Web Service.
This will be particularly useful for engineering or business teams that want to take advantage of this model.

The life cycle of any ML MaaS instance (i.e., from starting the instance, deploying an AI specific model to pausing or deleting the instance) can be managed in three different ways in MLOS :
...
Once an ML MaaS instance is up and running, it could be used for:

- *AI Model Deployment or Update*: the user has to provide a valid identifier in order to deploy the model of his/her choice.

- *Call of Predictions*: the user requests to give predictions for a specific payload that has to be converted into json data in order to get prediction's values.

Using ML MaaS, you can easily deploy and use any machine learning model as a Web Service on a physical or a virtual compute host on which there is an available Proactive Node. Going through the Proactive Scheduler,
...
| *Variable name* | *Description* | *Type*
| `MODEL_URL`
| The URL of the model that needs to be deployed.
| URL
| `USER_NAME`
| A valid user name having the needed privileges to execute this action.
| String
...
3+^|*Workflow variables*
| `SERVICE_TOKEN`
| A valid token required to be able to deploy the model.
| String
| `DOCKER_ENABLED`
|   If True, the workflow will be executed on a Docker container. link:https://hub.docker.com/r/activeeon/dlm3/[activeeon/dlm3] image is used by default. If False, the required libraries should be installed in the different nodes that will be used.
| Boolean (default=True)
...
There are two ways to delete a ML MaaS instance (i.e., where you will deploy, update and call your model) using MLOS:
.<<Via Cloud Automation Portal>>
.<<Via Studio Portal>>

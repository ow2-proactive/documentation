...
ProActive Node::
  ...
  will communicate with the <<_glossary_proactive_scheduler,*ProActive Scheduler*>> to execute tasks. We distinguish two types of **ProActive Node**:
  - **Server** ProActive Node: Node running in the same host as the ProActive server;
  - **Remote** ProActive Node: Nods that is not running on the ProActive Server Host.

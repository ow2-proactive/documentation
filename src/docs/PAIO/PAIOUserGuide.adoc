...
D. Click on `ai-machine-learning` catalog and pin it open, and same for the `data-visualization` catalog.
...
The following workflows are designed to detect anomalies in log files. They are constructed using generic tasks which are available on the `ai-machine-learning` and `data-visualization` buckets.
...
The following workflow presents an example of a workflow built using pre-built Azure cognitive services tasks available on the `azure-cognitive-services` bucket and custom AI tasks available on the `ai-deep-learning` bucket.

...
Rule Instance:: - instantiated rule with concrete set of variables. One rule can have several rule Instances with different variable values. The rule instance can be activated/deactivated. Proactive Cloud Watch service will monitor metrics defined inside the rule only for activated instances.
...
As explained in the document section <<_peo_monitoring, Monitoring>>, Proactive Cloud Watch service can monitor 2 types of metrics: <<_peo_monitoring_physical_metrics, Monitoring physical metrics>> and <<_peo_monitoring_host_accessibility, Monitoring host accessibility>>.

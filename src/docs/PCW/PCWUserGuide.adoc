...
PCW defines *rules*, which describe the metrics to monitor, triggering conditions and actions when the conditions are met. +
...
In order to get this information from a Node, a user can go to the *RM portal*, clickon the right button of a Node which should be monitored and click `Copy JMX Endpoint`. Then, he can come back to *Cloud Watch* portal and paste the JMX url in the corresponding field.
...
The polling and calm periods correspond to the frequency of collecting the monitoring metrics. The concrete set of the parameters rely on the implementation of a Polling class. In our example, the class is *Jmx_Metrics*. This implementation takes two parameters: the jmx node url and the jmx credentials.

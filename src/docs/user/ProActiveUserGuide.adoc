...
=== Fork environment
A fork execution environment is a new forked Java Virtual Machine (JVM) which is started exclusively to execute a task. For example, this task may need to use some third-party libraries, so it can be executed properly.  In this example, the list of path elements to be added when starting a new JVM will be specified inside the `Additional Classpath` field.
...
By default, all workflow tasks are executed in the forked mode, that is, executed in a forked JVM separated from the ProActive Node's JVM.

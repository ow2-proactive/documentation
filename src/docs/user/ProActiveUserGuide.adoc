...
That is, it allows a job to expose specific actions (e.g., soft stop, scale up, scale down) that can triggered by users from the *Workflow Execution portal*.

More specifically, the Signal API allows a workflow to:

  . expose signals, i.e., display them in the Workflow Execution portal (e.g., display the `soft_stop` signal)

  . wait to receive these signals, or check for their reception regularly, and

  . execute the corresponding behavior (defined in the workflow) at the signal reception.

The Signal API uses the synchronization API to provide a *persistent key-value channel* to store job signals.
...
As already mentioned, the Signal API is based on the synchronization API. However, these APIs are intended for different purposes.
...
It is easy to use and offers a set of basic operations on signals, summarized as follows:
  ...
  - Send signal(s): adds one or more signals to the set of signals of the current job.
...
* The *Signal* bucket contains many workflow examples that explain all signal operations.
These workflows use a set of predefined groovy scripts stored in the same bucket.
...
Many use cases can be fulfilled using this API. For instance:
  ...
  - _Scaling up and down resources_: a job that deploys resources (e.g., compute resources, workers/agents, etc)

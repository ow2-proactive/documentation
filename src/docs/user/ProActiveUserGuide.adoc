...
=== Fork environment
A fork execution environment is a new forked Java Virtual Machine (JVM) which is started exclusively to execute a task. This task may need to use some third-party libraries, so it can be executed properly. To do that, the list of path elements to be added when starting a new JVM can be specified inside the `Additional Classpath` field.
Starting a new JVM means that the task inside it will run in a dedicated, configurable environment.
The fork execution environment can be changed by writing a <<_glossary_fork_environment_script,fork environment script>>.
...
By default, the forked mode is configured then all workflow tasks are executed in a forked JVM separated from the ProActive Node's JVM.
...
- In case the server is configured in non forked mode (i.e. `pa.scheduler.task.fork` = `false`), the checkbox property *Fork* is unchecked and uneditable and all the fields under *Fork Environment* are uneditable.

- In case the server is configured in a forked mode (i.e. `pa.scheduler.task.fork` = `true`), the checkbox property *Fork* is checked and uneditable and all the fields under *Fork Environment* are editable.

- In case the global setting is not set (i.e. `#pa.scheduler.task.fork` = `true`), each task can specify its execution mode through the checkbox property *Fork*. If the checkbox property *Fork* is unchecked, then all the other fields under *Fork Environment* are disabled.
...
which is executed in the Docker fork execution environment can be customized by the JVM settings
...
----
This script exports a variable called `preJavaHomeCmd`, which is picked up by the Node executing the Task.

...
First the concepts of the Catalog will be explained. Then it will infromation how to interact with Catalog from GUI.
...
For security reason there are groups associated to buckets.
The user from same group can view the content of bucket.
There is special default group. The bucket associated with default group can be viewed by any user.
...
In opened window for publishing workflow to catalog the user can choose the `Bucket` where the workflow will be stored and `commit message`.
...
The Workflow Catalog Portal provides interfaces for easy administrating the catalog. The examples of provided options from this portal:
- creating new buckets
- removing workflows inside bucket
- importing/exporting Workflows into the Catalog

The portal logically is separated in 3 sections.
...
To export the selected Workflows as a ZIP archive from Workflow catalog portal: select a bucket, then the targeted Workflows (you can select multiple workflows with pressed CTRL button). Press "Export Workflows" button from portal view. You'll get a ZIP file which can be transferred to the machine on which the other Scheduler runs.

To import the zip archive of workflows to the Catalog from portal, select the bucket to which the Workflows will be added. Then press `Import Workflows` button in portal and follow the instructions `Upload a ZIP archive of workflows` opened window.

...
When a task has *IN_ERROR* status, a right click on it allows you to select _Mark as finished and resume_. 
This will set the task status as *FINISHED* and then resume job execution. 
This allows an operator to manually perform actions to the *IN_ERROR* task, without cancelling the whole job execution.

If the task has not *IN_ERROR* status, a right click on it shows the _Mark as finished and resume_ option, but it appears in grey: the action is unavailable.

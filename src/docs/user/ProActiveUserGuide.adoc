...
Job Results are composed of two elements: *Job Result List* and *Job Result Map*.
...
Each task has its own result (see section <<_task_result>>). However, some task results can be marked as *Task Result Added to Job Result*. In that case, this task result will be added to the *Job Result List*.
...
Job has key-value map associated to it, which is called *Job Result Map*.
Each task can contribute to this *Job Result Map*. It is write-only map,
such as task cannot see what other task wrote to this map.
However, task can rewrite existing values. This map contains String as a key,
...
`Task 1` writes to *Job Result Map*:
[source, groovy]
----
...
`Task 2` writes to *Job Result Map*:
[source, groovy]
----
resultMap.put("latestError", "wrong user id")
----

Once the job is finished, *Job Result Map* would contain:
[source, groovy]
----

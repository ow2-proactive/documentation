 ...
 * *Clean*: Delete the channel. This task simply deletes the channel used in this job. As there is no automatic mechanism to remove channels, it is necessary to delete them explicitly when they are not used anymore.
+
[source, groovy]
----
jobid = variables.get("PA_JOB_ID")

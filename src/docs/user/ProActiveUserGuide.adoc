...
=== Send Workflows to another Catalog Bucket, from another Scheduler or the local one.

In the Workflow Catalog, it is possible to select Workflows from the selected bucket and send them to another bucket. This bucket can belong to the same Catalog or to another Scheduler's Catalog.

==== Send Workflows directly

If the Workflows to share are sent to a Scheduler having a public API, or to another bucket of the local Scheduler, it is convenient to send them directly.

Select the bucket then the Workflows. Choose `Send to another scheduler` and click `OK`.

==== Export Workflows into an archive, then Import them

If the remote Scheduler is running in another network, of if you cannot use direct sending for some reason, you can export Workflows into an archive, that you can transfer by any file transfer method (email, usb key, etc.) and then import this archive into the other Scheduler.

To export the selected Workflows as a ZIP archive, select a bucket, then the targeted Workflows. Choose "Export selected Workflows" into the list and click on `OK`. You'll get a ZIP file that you'll be able to tranfer to the machine on which the other Scheduler runs.

To import the archive on this other Scheduler, select the bucket to which the Workflows will be added. Then choose `Import archive of Workflows` and click on `OK`.

 ...
 * It uses a single persistent channel (called *PA_SIGNALS_CHANNEL*) to store the signals of all jobs. The signal channel contains an entry per job.
 ...
 * *readyForSignal*: declares that the current job is ready to receive and handle one or more signals.
 ...
 * *sendSignal*, *sendManySignals*: adds one or more signals given as input to the current job signals.
 ...
 * *checkForSignals*: verifies whether one or more signals exist among the set of job signals.
 ...
 * *getJobSignals*: returns the current job signals.
...
==== Examples
Two menus in ProActive *Studio* contain workflow examples and templates, which illustrate the usage of the Signal API, namely:

* The *Signal* menu contains many workflow examples that use all signal operations.
These workflows use a set of predefined groovy scripts stored in the same bucket. Additionally, we give below an example of a groovy code that uses some Signal API operations.
...
* The *Controls* template menu contains two workflow templates, viz.,:
...
Concerning the usage of the link:/doc/rest/#1218178377[Signals REST endpoint], below we provide an example using curl that shows how to send a signal to a specific job:

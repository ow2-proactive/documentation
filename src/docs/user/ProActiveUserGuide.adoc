...
* *sessionid*: The ID of the connection. It allows the user to bypass the login page and directly display the content. It can be provided in the URL.
If not provided, in case the user is already logged in our portal, it will not need to log again, if not, the user will need to log in by entering his credentials.
* *name*: The name of the workflow.
* *bucket*: The name of the bucket.

[NOTE]
====
All parameters (`sessionid`, `name`, and `bucket`) are optional, and the order in which they are provided does not matter.
Additionally, the percentage symbol `%` allows using jokers in both the bucket name and the workflow name. It has to be encoded as “%25“in the URL.
====
...
image::submit_workflow_view1.png[align=center]

* *Submit any workflow from a specific bucket:* Provide the bucket name or a substring of it as a query parameter. You can use filtering with jokers by using the percentage symbol `(%)`, as explained in the example below.

.http(s)://your-server:port/automation-dashboard/#/submit?bucket=%25data

image::submit_workflow_view2.png[align=center]
...
.http(s)://your-server:port/automation-dashboard/#/submit?bucket=%25data&name=%25service

image::submit_workflow_view3.png[align=center]

* *Submit a specific workflow:* When both the bucket and workflow are determined exactly, do not use the percentage symbol (`%`).

.http(s)://your-server:port/automation-dashboard/#/submit?bucket=data-big-data&name=Docker_Swarm_HDFS_Spark_Service

image::submit_workflow_view4.png[align=center]
...
After submitting a workflow (with the method in the section above, or any other means like standard portals, API, etc.),
you can view the monitoring data and the resulting Job info (progress, interactive graph, results, log, tasks, etc.) by opening a dedicated window.

When using the method presented in the section above, this can be done by clicking on the 'See Job Details' button.

image::edit_vars_view_1.png[align=center]
...
[NOTE]
====
Note that you can customize the logo “Your LOGO here“ with your own portal logo, such that the windows smoothly integrate in your own portal.
See how to achieve that at section “3.9. Custom logo configuration” of the Admin Guide. Link: link:../admin/ProActiveAdminGuide.html#_glossary_custom_logo[here]
====

Your user will get access to these kinds of window in fully interactive mode:

image::job_details_view1.png[align=center]

image::job_details_view2.png[align=center]

image::job_details_view3.png[align=center]

image::job_details_view4.png[align=center]

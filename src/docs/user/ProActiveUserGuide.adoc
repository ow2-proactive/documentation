...
Inside a workflow, you can use the `${VARIABLE_NAME}` syntax to access it.
...
be performed at that moment (for instance job name, task attributes...).

[[_templating]]
TIP: Variables are very useful to use workflows as templates where only a few parameters change for each
execution. Upon submission you can define variable values either from <<_scheduler_command_line, CLI>> or from
the Web interfaces.

where `user_variable` will be defined by the user at submission time, either with the <<_scheduler_command_line,CLI>>
or the Web interfaces.
Then in the rest of the workflow, you can refer to the variable `${variable}` as usual.
...
Some variables are explicitly defined by the Scheduler to retrieve runtime information about the job or task.
You can retrieve them through the `variables` map or the `${}` syntax in the workflow XML definition.

[source]
.Retrieving the task ID from a Groovy script
----
...
and put it to the special place called *Local Space*, the working directory of a task.
Then files can be accessed normally like shown in example:
...
All the files produced by the task with the name _titi*_ (e.g titi, titi2 etc) will be transferred back to Global Space
...
List creentials (only keys are listed):
...
returns the credental corresponding to the key passed as parameter:

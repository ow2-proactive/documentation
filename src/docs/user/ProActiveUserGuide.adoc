...
It is up to the user code to decide the metadata semantics. However, some specific metadata can have a meaning when downloading or previewing results from the *Scheduler portal*
...
Once the job is finished, you can download/visualize Job Results from the Scheduler portal as shown in screenshot below.
...
To create a <<_mpi_application,multi-nodes>> task execution often used for MPI applications you need to add a *parallel environment*
...
In order to better control your workflows, two ProActive Studio templates *Web Validation* and *Email Validation* are provided under the *Manuals* menu. These templates aim at validating part of your workflows manually.

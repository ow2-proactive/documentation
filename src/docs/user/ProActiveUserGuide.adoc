...
The typical usage scenario of the Signal API is the following:
  ...
  . A job declares to be ready to handle one or more signals. The signals are then displayed in the Workflow Execution portal or Scheduler portal(e.g., display the `soft_stop` signal).
 ...
 * *sendSignal*, *sendManySignals*: adds one or more signals with or without variables given as input to the current job signals.
 ...
 * *checkForSignals*: verifies whether all of the signal given as input exist among the set of job signals.
...
The specified operations can be perform for signals and for *signals with variables*.

A signal can have input variables and updated variables.

The variables of a signal have predefined values called input variables that can be set using readyForSignal operation.

The updated variables of a signal can be set using sendSignal and sendManySignals operations.

...
* *model*: the type of the value. This field should be empty
...
This task provides LDAP client for querying LDAP server. The query is the search query with parameters.
...
In order to be able to authenticate to the LDAP server you need to add define for the right Scheduler user the <<_third_party_credentials,Third-party credentials>>: 
...
In order to use secure connection to LDAP server, specify a correct LDAP over SSL URL (for example ldaps://localhost:636).

You need to have a valid certificate to be able to connect to LDAP server.

In the case you are using a self-signed certificate, add this certificate to the trusted certificates of the JRE provided with the Scheduler.

Below is an example for adding a certificate to a Java cacerts file:
----
keytool -import -alias ldap-self-signed -keystore </pathToScheduler/jre/lib/security/cacerts> -file ldaps.crt
----

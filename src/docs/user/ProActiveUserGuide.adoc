...
The Catalog view in the Studio or in the Catalog Portal allow to browse all the workflows in the Catalog grouped by buckets and to list all the revisions of a workflow along with their commit message.

=== Studio
Inside the Studio portal there is a `Catalog` menu from which a user can directly interact with the Catalog to import or publish Workflows.

image::studio_catalog_menu.png[align=center]

Besides, the Palette of the Studio lists the user's favourite buckets to allow easy import of workflows from the Catalog with a simple Drag & Drop.
...
The Palette makes it easier to use specific templates stored in a specific Bucket of the Catalog when designing a Workflow.
...
The palette preset can be changed by clicking `View` → `Change Palette Preset`. This will show a window with the actual list of presets as in the image below.

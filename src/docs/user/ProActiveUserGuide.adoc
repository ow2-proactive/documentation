...
The basic behavior of a Docker_File task is to build an image and run a container using the built image. At the end of the task (the dockerfile execution), the container is stooped and the image is deleted. 
...
When the Docker_File task is launched, a succession of docker commands is to build image, run container, stop container and remove image. 

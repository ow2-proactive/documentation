...
Each Catalog object is identified by a unique object name. The object naming should match the following rules:

* The object name must not be empty, should start and finish with non-whitespace character.

* The object name can contain all characters except forward slash and comma.
...
When the user creates a zip archive for uploading objects to Catalog, the hidden files should be excluded from the archive.

===== Create a zip archive with Catalog objects in Mac OS
First navigate to the folder with the objects that you want to archive. You can use next zip command in Terminal to archive the files without the `.DS_Store`, `__MACOSX` and other `.* files`:

    zip -r ArchiveForCatalog.zip . -x ".*" -x "__MACOSX"

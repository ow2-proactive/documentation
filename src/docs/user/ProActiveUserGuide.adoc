...
We added a main bucket dropdown menu in order to make it easy to use templates stored in the catalog when designing a workflow. 

The main bucket dropdown can be changed by clicking on the `Catalog` -> `Set Bucket as Main Catalog Menu` menu button.

image::studio-catalog-main-menu.png[align=center]

==== Add Bucket as Extra Catalog Menu
This allows the user to add an extra bucket dropdown menu in the Studio. By selecting a bucket, the user can add a new Catalog menu to display additional Workflows.

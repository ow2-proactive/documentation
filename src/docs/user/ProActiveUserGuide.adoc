...
The following list describes the various model syntax available:

* *BOOLEAN* : variable is either "true", "false", "0" or "1".
* *INTEGER* , *INTEGER[min,max]* : variable can be converted to https://docs.oracle.com/javase/7/docs/api/index.html?java/lang/Integer.html[java.lang.Integer], and eventually is contained in the range [min, max]. +
Examples : INTEGER will accept "-5" but not "1.4", INTEGER[0,20] will accept "12" but not "25".
* *LONG* , *LONG[min,max]* : same as above with https://docs.oracle.com/javase/7/docs/api/index.html?java/lang/Long.html[java.lang.Long].
* *FLOAT* , *FLOAT[min,max]* : same as above with https://docs.oracle.com/javase/7/docs/api/index.html?java/lang/Float.html[java.lang.Float]. +
Examples: FLOAT[-0.33,5.99] will accept "3.5" but not "6".
* *DOUBLE* , *DOUBLE[min,max]* : same as above with https://docs.oracle.com/javase/7/docs/api/index.html?java/lang/Double.html[java.lang.Double].
* *SHORT* , *SHORT[min,max]* : same as above with https://docs.oracle.com/javase/7/docs/api/index.html?java/lang/Short.html[java.lang.Short].
* *URL* : variable can be converted to https://docs.oracle.com/javase/7/docs/api/index.html?java/net/URL.html[java.net.URL]. +
Examples: URL will accept "http://mysite.com" but not "c:/Temp".
* *URI* : variable can be converted to https://docs.oracle.com/javase/7/docs/api/index.html?java/net/URI.html[java.net.URI]. +
Examples: URI will accept "/tmp/file" but not "c:\a^~to" due to invalid characters.
...
A range can also be used in the DATETIME model. In that case, each bound of the range must match the date format used. +
+
Examples: +
DATETIME(yyyy-MM-dd) will accept "2014-12-01" but not "2014". +
DATETIME(yyyy-MM-dd)[2014-01-01, 2015-01-01] will accept "2014-12-01" but not "2015-03-01". +
DATETIME(yyyy-MM-dd)[2014, 2015] will result in an error during the workflow definition as the range bounds [2014, 2015] are not using the format yyyy-MM-dd.
* *LIST(item1,item2,...)* : variable must be one of the values defined in the list. +
Examples: LIST(a,b,c) will accept "a", "b", "c" but no other value.
* *REGEXP(pattern)* : variable syntax must match the regular expression defined in the pattern. The regular expression syntax is described in class https://docs.oracle.com/javase/7/docs/api/index.html?java/util/regex/Pattern.html[Pattern]. +
Examples: REGEXP([a-z]+) will accept "abc", "foo", but not "Foo".
* *MODEL_FROM_URL(url)* : variable syntax must match the model fetched from the given url. This can be used for example when the model needs to represent a list of elements which may evolve over time and is updated inside a file. Such as a list of machines in an infrastructure, a list of users, etc. +
Examples: MODEL_FROM_URL(file:///srv/machines_list_model.txt), if the file machines_list_model.txt contains LIST(host1,host2), will accept only "host1" and "host2", but may accept other values as the machines_list_model file changes.
...
a valid workflow xml file. For example, you can add the workflow that you want to execute into *Workflow Catalog*, then retrieve its URL by indicating its Bucket ID
and workflow ID. Moreover, SubmitJobAndWait will wait for the completion of the submitted workflow before proceeding further with the execution of the dependant tasks.
...
In order to be able to authenticate to the LDAP server you need to define the right Scheduler user in the <<_third_party_credentials,Third-party credentials>>: 

* ldapUsername (example : `cn=username,dc=test,dc=com`)
* ldapPassword

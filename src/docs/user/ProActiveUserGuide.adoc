...
In a Task you can define *variables* that are available only in the scope of that Task. Therefore, those variables won't be propagated to depending Tasks. However, if a different value is set into a script, this value will be propagated. A variable is defined using the following attributes:
...
Task variables override Workflow variables, and can be accessed as well from the scripts or from the Workflow definition (for more information please refer to <<_workflow_variables,Workflow variables>> section).

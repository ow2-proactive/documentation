...
NOTE: Though `Dockerfile`, `Docker Compose` and `Kubernetes` are not really scripting language but rather description languages, ProActive tasks can interpret Docker files, Docker Compose or Kubernetes yaml files to start containers or pods.

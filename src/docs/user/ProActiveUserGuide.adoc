...
----
/automation-dashboard/#/submit?sessionid={sessionid}&name={workflowName}&bucket={bucketName}
----
...
* If you provide a wildcard (e.g., %data, data%, or %data%) as the workflow name or bucket name, it will filter the bucket list and workflow names accordingly.
* If both the workflow name and bucket name are provided without a wildcard (%), the window will display the edit variable submission.

*Note*: The parameters are optional, and the order is not important.

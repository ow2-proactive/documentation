...
Notifications will only be sent for jobs whose generic information (workflow level) contains user's email address under the "EMAIL" key. Example:
...
EDF policy sends 3 types of email notifications, when:

1. [UNDER CONSTRUCTION] job deadline is at risk (if we do not start job in next X minutes we will miss its deadline)

2. job most likely will miss its deadline (we already at the moment that job will miss its dealine, if `JOB_EXEC_TIME` is precise)

3. [UNDER CONSTRUCTION] job is already missed its deadline.

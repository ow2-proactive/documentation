...
The typical usage scenario of the Signal API is the following:
  ...
  . If the signal has variables, then a new window will appear where the user can set the variables values and then send the signal with variables to the job. If the signal does not have variables then the window will not appear.
    ...
    After receiving the signal with attached variables, the workflow handles the provided input data properly.
  ...
  . At the signal reception, the job executes the corresponding behavior (defined in the workflow).
    When the signal has attached variables, the workflow will receive the signal only if the received variables are compatible with the variables set at the ready instruction.
 ...
 * *readyForSignal*: declares that the current job is ready to receive and handle one or more signals with or without variables.
  Variables are passed as a list of <<_job_variables>> objects.

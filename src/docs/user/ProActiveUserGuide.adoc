...
Beside the big data that you might use for your computations there is an ability to pass some information
from one task to another. On the job level you can defined *variables* that are available in all tasks.
...
Another important mechanism of data exchange is the task *result*. Anywhere in a task (usually at the end) you a reserved
...
* a *global space* which is available to all users of the scheduler
* a *user space* which is a personal user data storage

Usually these are folders on the machine where "ProActive Scheduler" server is launched.

Files that are stored there can be read from all computing nodes. In order to do it in a task set up the data management
...
Then before executing this task Proactive Scheduler transfers all files to a host where task will be executed

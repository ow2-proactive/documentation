...
The basic behavior of a Docker_File task is to create an image with Docker build and to create a container based on the built image. At the end of the job, the created image and container are deleted. Advanced options will allow to parametrize these actions.
...
The Docker_File task will run a combination of docker commands to build, start, stop and remove containers. 

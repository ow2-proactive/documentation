...
The basic behavior of a Docker_File task is create an image with Docker build and create a container based on the built image. At the end of the job, the created image and container are deleted. And, advanced options will allow to do more specific actions.
...
A Docker_File task expects the content of a Dockerfile inside the _Script_ section. You can find out how to write Dockerfile
with the official https://docs.docker.com/engine/reference/builder/[Dockerfile documentation^].

To get started: a simple Docker_File task example is explained below.

The content of the _Script_ section (equal to the content of a Dockerfile) is:
----
FROM ubuntu:18.04
RUN echo "Hello ProActive »
RUN sleep 30
----
The above example is a simple task that will create an image with Docker build.
After that a container, which will run the commands 'echo "Hello ProActive"' and 'sleep 30', is created from the built image.
...
=== Studio
Inside the Studio portal there is a `Catalog` menu from which a user can directly interact with the Catalog storage.
...
==== Set Bucket as Main Catalog Menu
In the `Catalog` dropdown menu, you can select a catalog Bucket to be used as the main Menu in the Studio. This makes it easier to use specific templates stored in a specific Bucket of the Catalog when designing a Workflow.

The main bucket dropdown can be changed by clicking on the `Catalog` → `Set Bucket as Main Catalog Menu` entry. See image below.

image::studio-catalog-main-menu1.png[align=center]

The name of selected Bucket now appears at the top of the Studio, and all its Workflows are now readily available in a Menu.

image::studio-catalog-main-menu2.png[align=center]


==== Add Bucket as Extra Catalog Menu
This allows the user to add extra buckets from the Catalog as dropdown menus in the Studio.

Add a new menu by clicking on the `Catalog` → `Add Bucket as Extra Catalog Menu` entry. See image below. 

image::studio-catalog-main-menu3.png[align=center]

The selected deep-learning-workflows Bucket now appears as a menu in the Studio. See image below.

image::studio-catalog-main-menu4.png[align=center]

You can repeat the operation as many times as you wish in order to add other specific Menus you might need for your workflow design. See image below.

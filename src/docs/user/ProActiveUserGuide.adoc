...
=== Docker task advanced options

The Docker task allows to add command options for the `docker-compose up` command.
You can specify these options by supplying a space character separated list in the <<_glossary_generic_information>>.
Use the key "docker-compose-up-options" and supply, as value, for example "--option1 --option2", this leads to a command
 similar to "docker-compose up --option1 --option2".

If splitting by space is prohibitive you can specify the split regex in the <<_glossary_generic_information>> with the
key "docker-compose-options-split-regex", if you supply e.g. "!SPLIT!" as value, then your "docker-compose-up-options"
 will need to look like this "--option1!SPLIT!--option2".
...
/usr/local/opt/python3/bin/python3.6) to the python executable that you want to use. If the Generic Info is put at task level this version of Python will be only used for this task, if
it is put in the job level this version of Python will be used for all the tasks in this job. Of course the required Python needs to be installed on your machine,
...
depending on it. If a task has several dependencies, *results* will be an array.
...
==== Assigning metadata to result variable
...
You can use variables on the <<_glossary_selection_script, selection script>>, these variables must be visible within the task
context, i.e., declared on job scope, on task scope, or result variable from a previous task. In the last
case is mandatory that the task that declares the inherited variable precedes the task using the variable. Below
an example jon in XML format that uses a task variable to select Unix/Linux as operating system, in this example
...
==== Set Bucket as Catalog Menu
This allows the user to change the bucket used to get workflows from the Catalog in the studio. By selecting a bucket, the user can change the content of the main Catalog menu (named as the current bucket) to get workflows from another bucket as templates.
This can also be achieved by adding `/templates/bucket_name` at the end of the URL.

==== Add Bucket as Extra Catalog Menu
...
NOTE: The `credentials` <<_glossary_script_bindings,script binding>> can also be used in other <<_glossary_additional_task_scripts,scripts>> defined inside a <<_glossary_task,ProActive Task>>.
...
- Scheduler API : using the `schedulerapi` binding, it is possible to interact directly with the scheduler and do several operations such as submitting jobs, waiting for tasks, listing running jobs etc.
...
- Synchronization API : using the `synchronizationapi` binding, it is possible to handle advanced dynamic task synchronization.
...
NOTE: The `schedulerapi` <<_glossary_script_bindings,script binding>> can also be used in other <<_glossary_additional_task_scripts,scripts>> defined inside a <<_glossary_task,ProActive Task>>.
...
NOTE: The `userspaceapi` and `globalspaceapi` <<_glossary_script_bindings,script bindings>> can also be used in other <<_glossary_additional_task_scripts,scripts>> defined inside a <<_glossary_task,ProActive Task>>.
...
Task synchronization is traditionally handled by the ProActive Scheduler through <<Dependency, Task Dependencies>>, where dependant ProActive Tasks are started only after their parents Tasks completed.

In some cases, *static task dependencies* are not enough to synchronize tasks execution.

This is the case when a task depends on the state of another task executed by a different branch of a workflow, a sibling task or even another job/workflow.
...
The *Synchronization Service* works as a _Key-Value Store_ organized in *channels*. Each *channel* acts as a private _Key-Value Store_ or _Hash Map_.
...
Users of the Synchronization API are responsible to create *channels* as fit their needs. For example, a channel which is used inside a specific job should be created inside some *initialization task* at the beginning of this job, and deleted in some *cleaning task* at the end.
 ...
 * Each operation must take a _channel identifier_ parameter. Some methods like _createChannel_ allows to manipulate channels.

 * The Synchronization API supports https://docs.oracle.com/javase/tutorial/java/javaOO/lambdaexpressions.html[Java 8 lambdas]. All methods present in the https://docs.oracle.com/javase/8/docs/api/java/util/Map.html[Java 8 Map interface] are implemented, but lambdas must be passed as string arguments defining http://groovy-lang.org/closures.html[Groovy Closures].
 ...
 * In addition to the methods present in the https://docs.oracle.com/javase/8/docs/api/java/util/Map.html[Java 8 Map interface], Synchronization API provides *blocking methods* to wait for a specific *condition* on a channel.

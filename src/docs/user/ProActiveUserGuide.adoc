...
The Docker task allows to set parameters to the `docker-compose` tool with regard to the docker-compose CLI https://docs.docker.com/compose/reference/overview/[reference].
...
link:../admin/ProActiveAdminGuide.html#_get_notification_on_job_events_configuration[Get Notification on Job Events Configuration].
...
Scheduler has `Earliest deadline first policy` (EDF policy) that priorities jobs with deadlines.
...
2. relative format: `+HH:mm:ss` , e.g. `+12:30:00`, `+5:30:00`, or `+mm:ss`, e.g. `+4:30`, or `+ss`, e.g. `+45` (45 seconds)
...
EDF policy orders jobs by applying following rules.
Each rule takes two jobs and define if some of these jobs are more important than another.
But it is possible that rule cannot figure out which of two jobs are more important.
Thus EDF policy applies these rule (in the given order) until one rule which actually sorts two jobs.
List of the EDF policy rules:

1. compare priority

2. job with deadline has advantage over job without deadline

3. job that is already started has advantage over job that is not started yet

4. job that is started earlier has advantage over job that is started later

5. compare distance between job expected finishing time and deadline, job with smallest distance gets an advantage

6. job with smallest submitted time.

Distance between job expected finishing time and deadline computed as a difference
between `effectiveDeadline` and `expectedJobFinishingTime`.

If job deadline is absolute then `effectiveDeadline` equals to job deadline.
If job deadline is relative then `effectiveDeadline` equals to `submittedTime` + deadline.

`expectedJobFinishingTime` equals to current date time + job expected execution time.

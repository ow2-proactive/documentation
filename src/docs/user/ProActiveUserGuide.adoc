...
This task provides Ldap client for querying Ldap server. The query is the search query with parameters. The next parameters is provided as task variables:
...
In order to use secure connection to LDAP server, specify a correct LDAP server url (for example ldaps://localhost:636 ).

You need to have valid certificate to be able to connect to LDAP server.

In case of use self-signed certificate, add this certificate to the trusted certificates of included jre provided by scheduler.

For example, the command should be like the next one for adding certificate to your Java cacerts file:
----

...
Definition of the tasks graph is made at workflow definition, before
...
One of the major questions after being able to submit and execute simple tasks is how to handle data.

=== Workflow Variables

ProActive Scheduler provides a mechanism to propagate data down the execution chain.
On the job level you can defined *variables* that are available in all tasks.
...
Then anywhere in your task you can reference to it. For example in a *Groovy* script task do it in the following way
...
If the a task depends of several tasks and each task modifies the same variable, the final value of the variable
which is propagated down the execution chain, depends on the *order of task execution*.
Therefore the users need to take appropriate measure to prevent any mal-effects of such raise conditions.

You can set any Java serializable object as a variable value. They will be converted into strings using
toString() method when required. For instance to make those values available as environment variables in *native* tasks.
...
TIP: The same as for variables *results* will be aggregated according to tasks termination, so results[0] can contain
...
Job result is a serializable object and you can retrieve it either from Java API or command line (it will be converted to
a string).

Also it is available in the *ProActive Scheduler Portal* in the *Preview* tab.

=== Input/Output Data in Shared Directory

The easiest way to use your data on computing machines is to set up a *shared directory* available on all nodes. For
Linux it's typically an NFS directory, for Windows it could be a Samba Directory. Than in tasks just read files as
there where local
...
* shared folder must be mounted *identically* on all computing hosts
* cannot be used with *heterogeneous resources*, e.g. Linux and Windows hosts

=== Data Spaces

If a shared folder is not an option in your environment the scheduler provides a convenient way to access
you data. It has two types of storage on the host where the server is running:

* a *global space* where anyone can read/write files
* a *user space* which is a personal user data storage

By default these spaces are linked to folders in the *temp* directory
...
All files that are stored in these spaces can be read from all computing nodes. In order to do it in a task set up the data management
...
Then before executing this task Proactive Scheduler automatically transfers all files to a host where task will be executed
...
Files can be also transferred from/to use space by specifying *transferFromUserSpace*/*transferToUserSpace*
accordingly.

Before running your jobs you should first *upload* your files into either global or local space. To do it you can export folders linked to
these spaces by one of a standard protocols like *FTP*, *NFS* etc or use available web interfaces like
...
Now when your tasks are ready you can customize several aspects of the execution.
...
To create a http://tbd[multi-nodes] task often used for MPI applications you need to add a *parallel environment*
...
In addision to the number of ProActive Nodes you can specify the *nodes network topology*, e.g
...
*exceptions* (for java/script tasks). Then it desides what to do next accodting to the following parameters:
...
==== Cron Tasks
*ProActive Scheduler* supports the execution of time-based tasks or cron tasks. The users can assign a cron expression to  the loop variable of a Control Flow Block in a ProActive Job. All tasks which belong that Control Flow Block will be executed iteratively and indefinitely. The start time of the next iteration is the next point of time specified by the cron expression. 

===== Cron Expression Syntax
The syntax of the follows the UNIX crontab pattern, a string consists of five space-separated segments.  Please refer http://www.sauronsoftware.it/projects/cron4j/manual.php#p02[cron4j documentation] for further details.

===== Setting up Cron Tasks
Setting up a cron task requires two steps:

* Define a Control Flow Block with the desired cron tasks
...
* The default ProActive Scheduler policy does not support cron task executions. Therefore it is required to set 'org.ow2.proactive.scheduler.policy.ExtendedSchedulerPolicy' as the scheduler policy at ProActive Scheduler startup.
...
* If the execution of a task fails within an iteration, the cron execution is terminated. In other words, the 'loop' breaks, in case of a failure of any task execution belong to the iteration. 

* ProActive Scheduler executes any cron task indefinitely. Therefore the state of ProActive Job will remain 'unfinished' unless the cron task execution terminates either by the user (for instance by killing the job or the task) or due an error in the task execution. 

...
1. [UNDER CONSTRUCTION] when job deadline is at risk (if we do not start job in next X minutes we will miss its deadline)

2. when job most likely will miss its deadline (we already at the moment that job will miss its dealine, if `JOB_EXEC_TIME` is precise)

3. [UNDER CONSTRUCTION] when job is already missed its deadline.

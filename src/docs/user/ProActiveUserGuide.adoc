...
----
$ PROACTIVE_HOME/tools/create-cred -F config/authentication/keys/pub.key -l username -p userpwd -k path/to/private/sshkey -o myCredentials.cred
----
...
Here is an link:examples/remote_visualization_script.sh[sample script^] that starts a Xvnc server and runs

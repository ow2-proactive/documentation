...
If the Workflow with the same name already exists in the specified bucket, a new revision of the Workflow is created.
We recommend to always specify a commit messages at any commit for an easier differentiation between stored versions.

==== Get a Workflow from the Catalog
When the Workflow is selected from Catalog you have two options:

1. `Open as a new Workflow`: Open the Workflow from Catalog as new Workflow in Studio.
...
To make it easy to use templates stored in the catalog when designing a workflow, we added a main bucket dropdown menu. 

The main bucket dropdown can be changed by clicking on `Catlog` -> `Set Bucket as Main Catalog Menu` menu button.

image::studio-catalog-main-menu.png[align=center]

==== Add Bucket as Extra Catalog Menu
This allows the user to add a an extra bucket dropdown menu in the studio. By selecting a bucket, the user can add a new Catalog menu that will contain its workflows.


=== Catalog Portal
The Catalog Portal provides the capability to store objects like Workflows, Calendars, Scripts, etc.
Powerful versioning together with full access control (RBAC) is supported, and users can share easily Workflows, templates, and other objects between teams, and various environments (Dev, Test, Staging, Prod).
...
In the `Create the new bucket` window is possible to associate a bucket to a specific user's group. 
...
==== Removing Objects inside bucket
Select the objects to remove (CTRL + left click to select multiple objects) and click on the `trash` button. 

WARNING: All the bucket content will be removed from the database and it cannot be restored.

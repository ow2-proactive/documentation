...
Beside from the _id_ and the _name_ of the Workflow, a few more information are indexed from the XML content such as _Variables_ and _Generic Information_.
...
Search on the latest versions of Workflows contained in a particular Bucket:
    ...
    http://localhost:8080/workflow-catalog/{bucket_id}/workflows

Search through all versions of a particular Workflow:

    http://localhost:8080/workflow-catalog/{bucket_id}/workflows/{workflow_id}/revisions
...
An *Expression* can be a composition of multiple conditions using conjunctions such as `AND` (or `&&`) and `OR` (or `||`).

Since the `AND` conjunction has precedence over `OR`, an *Expression* is a *Or_Expression* that can have one or more *And_Expression*.
A *And_Expression* has one or more *Clause* that represents the condition.
Ultimately, a *Clause* is composed of:

* an *Attribute* (the name of an indexed information)
* an *Operator* (`=` or `!=`)
* a *Literal Value*

Alternatively, a *Clause* can be a nested *Or_Expression* if you use parenthesises.
...
Can be expressed as:
    ...
    name="Deploy OpenStack Instance" OR generic_information.name="infrastructure" AND generic_information.value="openstack"

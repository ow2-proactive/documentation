...
Add a "NOTIFICATION_EVENTS" Generic information, to list events on which a user wants to be notified. List is comma-separated and should be taken from: *JOB_PAUSED*, *JOB_RESUMED*, *JOB_IN_ERROR*, *JOB_RUNNING_TO_FINISHED*, *JOB_PENDING_TO_FINISHED* and *JOB_RESTARTED_FROM_ERROR*. Scheduler can send email to the users when one of the above events is received.

To enable this functionality, the following properties should be set in `config/scheduler/setting.ini`:

[source, groovy]
----
pa.scheduler.notifications.email.enabled
pa.scheduler.notifications.email.from
----

The machine running the scheduler should be configured to accept outgoing mail (i.e. it should have a mail transport agent).
...
<genericInformation>
    ...
    <info name="NOTIFICATION_EVENTS" value="JOB_PAUSED, JOB_RESUMED"/>
</genericInformation>
----

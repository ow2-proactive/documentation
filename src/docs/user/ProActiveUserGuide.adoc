...
link:https://docs.docker.com/engine/reference/builder/[Dockerfile, window="_blank"],
...
NOTE: `Dockerfile`, `Docker Compose` and `Kubernetes` are not really scripting languages but rather description languages, ProActive tasks can interpret Dockerfiles, Docker Compose or Kubernetes yaml files to start containers or pods.
...
.Job Status for Pending and Current Jobs
...
Workflows come with constructs that help you distribute your computation. The tutorial
...
-   The target must have the initiator as only dependency: the action is
    ...
    other pending task as dependency, the behavior cannot be specified.
...
TIP: If you are familiar with programming, you can see the branch as an if/then/else.
...
include::./references/GenericInformation.adoc[]

[[_task_result]]

=== Task result

Another way to propagate data from a task to another is the use of the *result* script binding.
Anywhere in a parent task (usually at the end) you can set a value to a reserved
variable named *result*. This value will be available in the child tasks i.e which are depending on the parent task.
If a task has one or several parents, *results* will always be an array.
...
Result metadata can contain additional information associated with the result. In order to store metadata information, you have to use the *resultMetadata* script binding.
As for the *result* binding, the *resultMetadata* value of a task is available in its child tasks.
...
It is the user code which specifies the metadata semantics. However, some specific metadata can have a meaning when downloading or previewing results from the *Scheduler portal*
...
The label in *Result List* corresponds to the task name that generated the value.
...
Each task has its own result (see section <<_task_result>>). Some of intermediate task results may be precious such that the user would like to
access them easily at the job level. To do that, the user has to select precious task(s) and check the *Task Result Added to Job Result* checkbox as shown in the screenshot below.
In that case, the task result will be added to the *Result List* when the job execution is finished.
...
=== Example: Install specific software tools and libraries before launching tasks

Some workflow tasks require the prior installation of specific software tools and libraries
that are necessary to properly execute these tasks. ProActive offers two methods to enable the installation of such tools and libraries:

1. By using software containers encapsulating the required libraries and tools.
...
See Section <<_fork_environment, Fork environment>> for more details about executing tasks inside software containers.
2. By using pre-scripts, i.e., scripts that are executed just before the considered computational task, and on the same compute resource.
...
See Section <<_pre_post_clean, Pre, Post & Clean Scripts>> for more details about pre-scripts.
...
[[_generic_information_selection]]
==== Generic Information

A few <<_generic_information>> allow to control where the associated task will be executed:

 * <<_node_access_token,NODE_ACCESS_TOKEN>>: the task will be executed only on Nodes which have the specified token. A node token is defined as a link:../admin/ProActiveAdminGuide.html#_policy_common_parameters[parameter] of a link:../admin/ProActiveAdminGuide.html#_node_source_policies[Node Source Policy].
 * <<_node_source_generic_info,NODE_SOURCE>>: the task will be executed only on Nodes which belong to the specified <<_glossary_node_source,Node Source>>.
...
image::fork-environment.png[align=center, width=512, height=256]
...
After a task is executed, an internal mechanism called the *Process Tree Killer* ensures that all children processes started by the task are automatically terminated.
This mechanism is explained in details in the link:../admin/ProActiveAdminGuide.html#_task_termination_behavior[Task Termination Behavior] section.

The *Process Tree Killer* mechanism can be disabled using the <<../user/ProActiveUserGuide.html#_disabled_ptk,DISABLE_PTK>> generic information.
...
To create a <<_mpi_application,multi-nodes>> execution (often used for MPI applications), you need to add a *parallel environment*
...
located. Thus, in addition to the number of ProActive Nodes, you can specify the *nodes network topology*, e.g
...
image::multi-nodes-execution.png[align=center, width=320, height=192]
...
Successor Tasks of each *In-Error* Task are *Pending* until errors are fixed. The Job is in *In-Error* status.
...
Successor Tasks of each *In-Error* Task are paused until errors are fixed. The Job is paused.
The user can restart *In-Error* Tasks as many times as he wishes.
...
A `timeout` (also known as *_walltime_*) can be set at the task's level to stop a task's execution when the timeout is reached.
...
The walltime mechanism is started just before a task is launched.
If a task does finish before its walltime, the mechanism is canceled. Otherwise, the task is terminated.
Note that the tasks are terminated without any prior notice.

The walltime can also be configured using the `WALLTIME` <<_generic_information>>. The syntax of this generic information is similar to the task `walltime` attribute.
...
* Each iteration yields one or more task results and the user can query the job status and task status to
...
In case one wants to promote the variable `called_workflow` at the level of the Workflow in order to be able to select the submitted Workflow at submission time, he just has to duplicate the variable at the Workflow level and mark the one at the Task level as inherited.
...
All actions are final for each user. A user cannot modify his response.
Once a Validation receives a *Yes* or *No action*, the Validation is considered processed and *no future actions* can be done by any user.

[title="A Validation in the Notification-Portal", align="center"]
image::notification-service-validation.png[align=center,768,121]
...
Before being able to use labels for Jobs, an Administrator has to define the authorized label list, see link:../admin/ProActiveAdminGuide.html#_manage_job_labels[*Manage job labels*].
If you are not an Admin of your platform, and want to use Labels, ask your administrator to add a label list.

It’s possible to add a label on a job in <<_glossary_workflow_automation_portal,*Workflow Execution*>>
The user can set a label on a job by clicking on *Job Actions* button of the job and then click on *Edit label*.
The user can select any of the labels available in the sub-menu list.
It’s possible as well to remove a label from a job by selecting *Job Actions* button of the job and then click on *Remove label*.

The user can set a label as well on a job from <<_glossary_scheduler_web_interface,*Scheduler Portal*>>.
It’s possible to select one or more jobs and then, from the right-click drop-down menu, the user can select *Edit label* and chose from the available labels.
The user has the option to remove a label from a job or a list of jobs by selecting *Remove label* from the same drop-down menu.
...
See link:../admin/ProActiveAdminGuide.html#_data_spaces_configuration[Data Spaces configuration] for further details on how to configure the Global and User spaces.
...
These information are used in the various portals which interacts with the catalog such as the +++<a class="automationDashboardUrl" href="/automation-dashboard/#/portal/workflow-automation" target="_blank">Workflow Execution Portal</a>+++.
...
==== Create a zip archive with Catalog objects
...
TIP: *Create a zip archive with Catalog objects in Mac OS.*
...
include::./references/Notification-service.adoc[]
...
In such cases it is often desirable to store the credentials
...
<<_glossary_scheduler_web_interface,Scheduler Web Interface>> & Workflow Execution  (Portal
...
[[_task_apis]]
== Scheduler, Resource Manager, DataSpace, Synchronization, and Signal APIs
...
Using *Task Synchronization API*, task _A_1_ can advertise the server URL to the Synchronization Service.

Task _B_1_ can use Task Synchronization API to wait until the apache server is advertised, it can also perform the necessary operations and advertise task _A_1_ that the operation is complete and the apache server can be terminated.
...
*Wait_For_Signals* is a template workflow that contains a single task, which sends a ready notification for the specified signals in the 'SIGNALS' variable, then checks if one or more signals are received by the job. If not, the workflow runs in a loop where each iteration lasts one minute.
...
The typical usage scenario of the Signal API is the following:
  ...
  . Signals are defined by their name and, optionally, a list of variables. Variables attached to a signal have the same structure as <<_workflow_variables>> but are declared exclusively using the <<_task_signal_api,Signal API>>.
  ...
  . A user clicks on the signal displayed in the Workflow Execution portal or Scheduler portal (e.g., click on the `soft_stop` signal), which sends this signal to the job.

  . At the signal reception, the job executes the corresponding behavior (defined in the workflow).
...
As already mentioned, the Signal API uses the <<_task_synchronization_api,synchronization API>> to store jobs' signals, with respect to the following specifications:
...
The Signal API is based on the <<_task_synchronization_api,synchronization API>>. However, these APIs are intended for different purposes.
The Synchronization API is used to exchange information (e.g., messages) between tasks and between jobs, whereas the
Signal API is useful to exchange signals between a job and users (via the Workflow Execution portal or the REST API), it cannot be used to send signals from one job to another.

==== Usage
*Task Signal API* is a *Java API* defined by the following link:../javadoc/org/ow2/proactive/scheduler/signal/SignalApi.html[API documentation].

The Signal API is available inside ProActive Tasks through the *signalapi* <<_glossary_script_bindings,script binding>>.

It is easy to use, and offers a set of basic operations on signals summarized as follows:

 * *readyForSignal*: declares that the current job is ready to receive and handle one or more signals with or without variables.
  Variables are passed as a list of link:../javadoc/org/ow2/proactive/scheduler/common/job/JobVariable.html[JobVariable objects]
   This operation constructs new signals (by adding the prefix `ready_` to the input signals), then adds them to job signals.
   For example, if a job declares ready for the signal `soft_stop`, the signal `ready_soft_stop` is added to the set of job signals.
   Additionally, if the input signal (e.g., `soft_stop`) already exists among the job signals (which means that it has been sent and handled previously),
    the operation ready for signal automatically removes it, thus the job is ready again to receive and handle the same signal.

 * *sendSignal*, *sendManySignals*: sends programmatically one or more signals with or without variables given as input to the current job signals.
   This operation removes the corresponding ready signals if they already exist. For instance, sending `soft_stop` removes the signal
   `ready_soft_stop` if it already exists among the job signals.
 ...
 * *waitFor*, *waitForAny*: blocking instruction that waits until one signal (among those given as input) is added to the set of job signals.
...
In addition to the above operations available within ProActive Tasks, users and third-party systems can send signals
to a specific job via link:/doc/rest/#1218178377[ProActive Signals REST endpoint].

==== Examples

===== Standard and simple use with predefined tasks in the Control template menu

  . _Check_For_Signals_: a control pattern that executes a loop until one ore more signals, given as input, are received.

  . _Wait_For_Signals_: a blocking control pattern that blocks the task execution waiting for the reception of one or more signals given as input.
...
Below, we provide an example based on link:https://curl.se/[curl], which shows how to use the link:/doc/rest/#1218178377[Signals REST endpoint] for sending a signal to a specific job:
[source,shell,subs="verbatim,quotes"]
----
curl -X POST -H "sessionid:$sessionid"
http://try.activeeon.com:8080/rest/scheduler/job/`job_id`/signals?signal="*soft_stop*"
----
==== Conclusion
The Signal API allows users to interact with jobs during their execution, thus enabling to impact and change jobs' behaviors at runtime.
Many use cases can be fulfilled using this API. For instance:

 * _Softly stopping a job_: when the stop signal is received, the job executes some code that properly finish this job.

 * _Stop or continue job execution based on the user decision_: a job suspends its execution waiting for the user to send a signal, either stop or continue.

 * _Scaling up and down resources on user demand_: a job that deploys resources (e.g., compute resources, workers/agents, etc)
  ...
  or from third-party systems (via the Signals REST endpoint).

== ProActive Modules APIs
...
=== ProActive Service Automation (PSA) API

The ProActive Service Automation API, or ProActive Service Automation Client, is a way by which a workflow can communicate with the link:../PSA/PSAUserGuide.html[Service Automation module]

The ProActive Service Automation Client hides the complexity of building and sending HTTP requests manually by providing
ready-to-use code callable from various tasks.

Below is an example of a groovy script starting a managed service.

NOTE: This example has been truncated for readability and is not meant to be used as is.
...
The complete ProActive Service Automation Client API is available in the javadoc link:../javadoc/cloudAutomationClient/index.html[Service Automation Client JavaDoc].

=== Notification Service API

The Notification Service API, or Notification Client, is a way by which a workflow can communicate with the <<_notification_service,Notification Service>>

The Notification Client hides the complexity of building and sending HTTP requests manually by providing
ready-to-use code callable from various tasks.
...
The complete Notification Client API is available in the javadoc link:../javadoc/notificationClient/index.html[Notification Client Javadoc].
...
include::./references/ApplicationConnectors.adoc[]

== Integration of ProActive Services in Third-party User Portals
...
* Submission of a Workflow in a Dedicated Window: How to use an external window for workflow submission outside our software.
* Monitoring the Execution of a Specific Job in a Dedicated Window: How to view monitoring data and workflow information after submission.

=== Submission of a workflow in a dedicated window
We offer an external window that can be accessed outside our software using the following URL format:

----
http(s)://your-server:port/#/submit?sessionid={sessionid}&name={workflowName}&bucket={bucketName}
----

* *sessionid*: This is the ID of the connection. It allows the user to bypass the login page and directly display the content.
* *name*: The name of the workflow.
* *bucket*: The name of the bucket.

Additionally, there is the possibility to filter within the view. Here’s how it works:

* If you provide a percentage symbol e.g., `%data`, `data%`, or `%data%` as the workflow name or bucket name, it will filter the bucket list and workflow names accordingly.

image::submit_wf_view_with_filters.png[Submit Workflow View With Filters]

* If both the workflow name and bucket name are provided without a percentage symbol (`%`), it will directly display the submission window to edit the variable values and submit.

image::edit_vars_view.png[Edit Variables View]

[NOTE]
====
All parameters (`sessionid`, `name`, and `bucket`) are optional, and the order in which they are provided does not matter. Additionally, the percentage symbol `%` should be encoded as `%25` in the URL.
====
...
----
http(s)://your-server:port/#/submit?bucket=%25data%25
----

This url display only the buckets with names that include the substring "data" within the name, not at the beginning or end, like *my-data-bucket* and *bucket-data-test*.

Example 2:

----
http(s)://your-server:port/#/submit?bucket=data%25
----
This one display only the buckets with names that end with "data", like *data-analysis*.

Example 3:

----
http(s)://your-server:port/#/submit?bucket=ai-deep-learning
----

This url display all buckets with names that include the substring *ai-deep-learning* anywhere with the name, the position here doesn't matter.

Example 4:

----
http(s)://your-server:port/#/submit?bucket=ai-deep-learning-workflows&name=%25train%25
----

This url display all buckets with names include the substring *ai-deep-learning*, and workflows with names include the substring *train* (not at the beginning or end).

Example 5:

----
http(s)://your-server:port/#/submit?bucket=ai-deep-learning-workflows&name=train%25
----

This url display all buckets with names include the substring *ai-deep-learning*, and contain workflows with names end by *train*.

Example 6:

----
http(s)://your-server:port/#/submit?bucket=ai-deep-learning-workflows&name=%25detection
----

This url display all buckets with names include the substring *ai-deep-learning*, and contain workflows with names start by *detection*.


=== Monitoring the execution of a specific job in a dedicated window

After submitting your workflow, you can view the monitoring data and the workflow info (result,
log, tasks, etc.) by opening a dedicated window. This can be done by clicking on the 'See Job Details' button.

image::edit_vars_view_1.png[Edit Variables View]

Alternatively, you can use the following URL separately by retrieving your job ID and replacing {jobid} with your job ID:

----
http(s)://your-server:port/#/job-info?jobid={jobid}
----
...
[[_scheduler_graphql_api]]
=== Scheduler GraphQL API
include::./references/SchedulerGraphQLAPI.adoc[]

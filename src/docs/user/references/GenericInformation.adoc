...
|contains the portal name from which the job was submitted
|job-level
|job-planner
|===
...
The `submission.mode` Generic Information displays the name of the portal from where the job was submitted.

`Possible values:`
workflow-api, cli, rest-api, scheduler-portal, job-planner, studio, catalog, workflow-execution, service-automation, event-orchestration

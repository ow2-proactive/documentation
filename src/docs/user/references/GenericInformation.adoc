...
`NS` (short for *Native Scheduler*) and `NS_BATCH` are Generic Information used to deploy workflow tasks inside a *Native Scheduler infrastructure*.
...
`NS` and `NS_BATCH` can be defined at *job-level* (applies to all tasks of a workflow) or at  *task-level* (applies to a single task).

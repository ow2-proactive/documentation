...
Data connectors workflows may need to add some third-party dependencies to their classpath (notably, JDBC jar libraries), so they can execute properly. The user determines the required dependencies depending on the workflow implementation and the used DataBase system. Third-party dependencies can be added to the workflow classpath in three ways (depending on the nodes that will run the workflow):

- If the workflow will be executed locally (i.e., on local nodes running in the same system as the server), the dependencies can be placed under the addons directory of the server (i.e., $PROACTIVE_HOME/addons);
- If the workflow will be executed using remote nodes that will be started using the proactive-node command (i.e., based on activeeon_enterprise-node-xxxxx.jar) , third party dependencies can be placed under the addons directory of the node (i.e., $PROACTIVE_NODE/addons);
- If the workflow will be executed using remote nodes that will be started using the executable jar 'node.jar' (usually used for cloud infrastructure), third party dependencies has to be added inside the lib folder of the node.jar archive itself, which is located under $PROACTIVE_HOME/dist/war/rest.

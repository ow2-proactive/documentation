...
Data connectors workflows may need to add some third-party dependencies to their classpath (notably, JDBC jar libraries), so they can be executed properly. Please refer to the Admin documentation to learn how to add link:../admin/ProActiveAdminGuide.html#_add_custom_third_party_dependencies_to_a_workflow_classpath[third-party dependencies].

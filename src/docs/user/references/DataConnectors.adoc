...
* Export_to_FTP/SFTP_Server: Uploads files or folders recursively from the data space (global or user space) to an FTP/SFTP server.
...
{key: `<mysql|postgresql|sqlserver|oracle|greenplum>://<<USER>@<<HOST>:<<PORT>`, value: `<RDBMS_NAME>_PASSWORD`}.
...
Data connectors workflows may need to add some third-party dependencies to their classpath (notably, JDBC jar libraries), so they can be executed properly. Please refer to the Admin guide documentation to learn how to add link:../admin/ProActiveAdminGuide.html#__add_custom_third_party_libraries_to_proactive_server_and_nodes_classpath].

In case the password is required, you have to add the following third-party credential: {key: `<mysql|postgresql|sqlserver|oracle|hsqldb:hsql>://<USERNAME>@<HOST>:<PORT>`, value: `<RDBMS_NAME>_PASSWORD`}. This is a one-time action and will ensure that your credentials are securely encrypted. Please refer to the User guide documentation to learn how to add link:../user/ProActiveUserGuide.html#_third_party_credentials[third-party credentials].
...
The Snowflake data connector allows to import and export structured data as well as semi-structured data (JSON, Avro, XML) data from and to a Snowflake cloud Data Warehouse.
The Snowflake connector workflow is composed of two tasks: an import task and an export task.

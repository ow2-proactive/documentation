...
This feature is showcased through two workflow templates: SQL_Pooled_Connection_Query and SQL_Pooled_Connection_Update. Both are a single task workflow.

*Variables:*

The SQL_Pooled_Connection_Update workflow shares with the SQL_Pooled_Connection_Query workflow the same list of variables. Consequently, we describe them in the same following table.
...
| if not empty, the value will be the name of a variable that contains the resultSet (converted into a List of Maps) of the query. This variable can be used in other tasks.
| Workflow, Task
| No
| String
| Values: `{CSV, HTML}`.

...
To be executed, each workflow requires the jdbc connector jar of the used rdbms to enable connection to the database.
In case the Scheduler is running with only local Nodes, make sure that you have properly added the jdbc jar into the addons directory ($PROACTIVE_HOME/addons) of the Scheduler before starting the server.
Otherwise, adding the jdbc connector jar depends on how you will deploy the ProActive Node. If you use the enterprise Node release, make sure you have it properly added into the addons directory ($PROACTIVE_NODE/addons) of the node.
If you use the node.jar, go under $PROACTIVE_HOME/dist/war of the Scheduler and inject the jdbc jar into the node.jar.

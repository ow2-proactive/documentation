...
The following example shows how to define a job variable in the XML schema:
[source, xml]
----
<job ... >
    <variables>
...
* *name*: the name of the variable
* *value*: the value of the variable
* *inherited*: asserts whether the propagated value should be use instead of the *value* on the workflow attribute.
If _true_, the *value* field can be empty and as a consequence this variable is dynamically propagated from a
previous task.
* *model*: the type of the value (optional). See section <<_variable_model,Variable Model>>
...
As opposed to the <<_workflow_variables>>, *dynamic variables* are all variables created outside the XML job definition.
We have mainly two types of dynamic variables :

- <<_system_variables>>, declared by the ProActive scheduller.
...
Sometimes we want to propagate values between tasks and still define a default value.
Setting the inherited field to true a task variable will read its value from a the *variables* map.
Below a XML schema that shows two tasks: *first* and *second*. Task *first* insert a new
variable using `variables.put("inherited_var", "somevalue")`. Task *second* declares *inherited_var*
but with *defaultvalue*, this value will be overwritten by the  *first* task, `variables.put("inherited_var", "somevalue")`.
This might be useful if for some reason the *first* task fails before inserting the *inhereted_var* value on *variables*
map.

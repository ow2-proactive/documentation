...
* *PA:NOT_EMPTY* : variable must be provided with a non-empty value.
* *PA:HIDDEN* : variable which allows the user to securely enter its value (i.e., each character is shown as an asterisk, so that it cannot be read.) during submit the workflow.

...
through the use of the "variables" map (see <<_variables_quick_reference>>).
...
Java as http://docs.oracle.com/javase/8/docs/api/java/util/Map.html[Map^].
...
For Java native script you can set any link:http://docs.oracle.com/javase/8/docs/api/java/io/Serializable.html[Java serializable object, window="_blank"]
...
The dynamic replacement will resolve `job_var3` to `hello` just before submitting the job to the scheduller.

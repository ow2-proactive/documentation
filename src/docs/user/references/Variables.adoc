...
* *PA:CATALOG_OBJECT*, *PA:CATALOG_OBJECT(kind)*, *PA:CATALOG_OBJECT(,contentType)*, *PA:CATALOG_OBJECT(,,bucketName)*, *PA:CATALOG_OBJECT(,,,objectName)*, *PA:CATALOG_OBJECT(kind,contentType,bucketName,objectName)* : variable syntax must be a valid expression that matches the following pattern: `bucketName/objectName[/revision]`.
...
Examples:
    ...
    ** PA:CATALOG_OBJECT(,,basic-example, Native_Task) will accept catalog objects that is in buckets ia-basic-example or basic-example-python and named Native_Task or Native_Task_Python but not NATIVE_TASK.
...
So, by adding `%` in the beginning (resp. end) of a bucketName or an objectName we mean that the filtered value should end (resp. start) with the specified string.
Examples:

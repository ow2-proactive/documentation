...
Models can use the value of other variables in their definition. +
To define a variable substitution, the workflow designer can simply use a variable name proceeded by the character `$`. +
 +
When submitting the workflow, the user will have the ability to update the models dynamically by changing the value of reference variable.  +

For example:  +
`variable_name1` has model `PA:LIST(PA:GLOBAL_FILE, PA:INTEGER)` +
`variable_name2` has model `$variable_name1` +

Then the model of `variable_name2` is the value that the variable `variable_name1` will have in runtime. Thus, it will be either `PA:GLOBAL_FILE` or `PA:INTEGER`
...
| The URL of cloud automation REST api.
| String

| `PA_CLOUD_AUTOMATION_REST_PUBLIC_URL`
| The URL of cloud automation REST public api if the setting pa.cloud-automation.rest.public.url is defined in `PROACTIVE_HOME/config/scheduler/settings.ini`.
| String

...
** *PA:SPEL(SpEL expression)* : variable syntax will be evaluated by a https://docs.spring.io/spring/docs/current/spring-framework-reference/html/expressions.html[SpEL] expression. Refer to <<_spring_expression_language_model>>.
** *PA:SPEL2(error message,SpEL expression)* : variable syntax will be evaluated by a https://docs.spring.io/spring/docs/current/spring-framework-reference/html/expressions.html[SpEL] expression. This model allows in addition to provide a user-friendly error message pPlease note that the error message must not contain any comma character). Refer to <<_spring_expression_language_model>>.

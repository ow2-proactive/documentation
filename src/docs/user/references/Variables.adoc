...
Let's distinguish between the name of a variable and its value. If `variable_name` is the name of a variable, then `$variable_name` or `$variable_name?` is a reference to its value.

In our case, variable model can be referenced by another variable value, for example using a variable B as the PA:MODEL of a variable A.

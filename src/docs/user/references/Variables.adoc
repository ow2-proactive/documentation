...
A filter can be specified in the PA:CATALOG_OBJECT model. In that case, the variable value must be a catalog object which matches the kind and content type requirements.
The scheduler server verifies that the object exists in the catalog and fulfills the requirements during workflow submission. +
...
PA:CATALOG_OBJECT(Script,text/x-python) will only accept catalog objects which is a Python script but not a workflow object or a Groovy script.

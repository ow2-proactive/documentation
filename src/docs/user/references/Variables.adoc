...
To define a variable substitution, the workflow designer can simply add `$` at the beginning of another variable name. +
 +
When submitting the workflow, it will be allowed to update the models dynamically by changing the value of reference variable.  +

For example: If we have two variables `A` and `B` with models `PA:LIST(PA:GLOBAL_FILE, PA:INTEGER)` and `$A`, then `$A` will be either `PA:GLOBAL_FILE` or `PA:INTEGER`

...
| *Task result*. Variable set by the user to return the result of a task. See <<_passing_data_between_tasks_using_variables>>.
| result = "value";
...
| *Results of previous tasks*. An array containing the results of parent tasks. See <<_passing_data_between_tasks_using_variables>>.
| ... = results[0];
| $results_0
| -
| <<_scripting_language_support,task>>
...
For Python script, the *results* variable contains a list of http://doc.activeeon.com/javadoc/latest/org/ow2/proactive/scheduler/common/task/TaskResult.html[TaskResult java object].

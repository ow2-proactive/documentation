...
Note that a workflow name that contains a variable i.e. my_workflow_${my_var} can be replaced by its value in case this
variable is defined at the workflow level. The replacement is performed when the job subscription is created.

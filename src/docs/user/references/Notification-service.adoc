...
User can provide multiple emails as recipients for notifications.
...
In such case, after providing the notification method parameter, an input field will appear below the third-party parameter or telephone parameter asking for a confirmation code.
For emails, the verification code input is always displayed.
...
Presets are not standalone event types as the one described above but rather preconfigured selections within a subscription's event type list.
These presets serve as convenient groupings of related event types. Once selected, multiple events are added to the subscription.

...
A user that is a notification service administrator can subscribe to any channel. Otherwise either the channel is authorized
for all user groups to subscribe or the user is part of at least one of the authorized groups of the channel.

To visualize past notifications and start receiving new ones, a user must update his channel subscription
by activating a notification method, selecting desired channels from the user's available channels list and finally activating the subscription.
...
| Association failed
| ERROR
| The association 52 of the workflow Variables_Propagation to the calendar every_monday had a failed execution
, as it is configured, the association status is set to FAILED

...
To receive Microsoft Teams notifications using the Notification Third-Party notification method, some configuration must be applied in the Microsoft and the Notification Service sides. +
These configurations are described in the admin guide link:../admin/ProActiveAdminGuide.html#microsoft_teams_configuration[here] and is mandatory to be able to receive Teams notifications.

==== Configure User Settings

To be able to send Teams notification, the Third-Party script needs to connect and retrieve a token using a user's microsoft credentials.
...
- Provide your microsoft login and you encrypted password with the ENC( prefix separated by a comma
- Click on save
...
Each subscription that needs to send notification to teams have to provide channel URLs.
At least one must be provided and it defines the channel(s) where the notification is sent.

You can easily retrieve channels URL from Microsoft Teams desktop App or Web browser:
...
To do so, the Third-Party param input accepts a list of channel URL, separate by a carriage return or white spaces.

...
The <<_glossary_job_planner,*Job Planner*>> can send notifications to users when particular events occur. The <<_glossary_notification_service,*Notification service*>> documentation can be found link:../admin/ProActiveAdminGuide.html#_notification_service[here].

To start receiving notifications from the <<_glossary_job_planner,*Job Planner*>>, you must:

- Configure your Job planner <<_glossary_notification_subscription,*Subscription*>> as it is described in the link:../user/ProActiveUserGuide.html#_service_subscriptions[Service subscriptions documentation] of the <<_glossary_notification_service,*Notification service*>>.
- Inside the Job planner subscription, select from available events the ones you wish to be notified of.
Available Job planner events with their description and example values are detailed in the link:../user/ProActiveUserGuide.html#_job_planner_notifications[Job Planner event types] table.

== Job Planner Configuration

<<_glossary_job_planner,*Job Planner*>> provides features to specify actions it will take regarding different situations it can encounter when submitting <<_glossary_workflow,*Workflows*>> according to a <<_glossary_calendars_definition,*Calendar*>>.

This configuration is done in the properties file located `PROACTIVE_HOME/dist/war/job-planner/WEB-INF/classes/application.properties`

=== Planned execution(s) has been missed
This configuration controls how the <<_glossary_job_planner,*Job Planner*>> reacts when it detects that execution(s)
have been missed for a given association. For instance if the server is down for a period of time, the planned execution will not take place.
Two properties are used to define how <<_glossary_job_planner,*Job Planner*>> reacts to this situation.
----
...
If set to false, <<_glossary_job_planner,*Job Planner*>> will not submit a new execution.

The second property defines the delay in milliseconds from a missed execution and the current time for which the <<_glossary_job_planner,*Job Planner*>> will submit a delayed execution.
By default there is no delay. The <<_glossary_job_planner,*Job Planner*>> will submit an execution whatever the time span between the missed execution and the current time.
---
...
<<_glossary_job_planner,*Job Planner*>> can either cancel the submission (default). The workflow will be submitted at the next calendar occurrence.
----
...
----
By setting this property to true, <<_glossary_job_planner,*Job Planner*>> will postpone the execution immediately after the previous execution finishes instead of canceling it.
...
This feature defines the behavior of the <<_glossary_job_planner,*Job Planner*>> when a previous execution for a planned workflow has errors.
By default, it will continue to submit according to the calendar
----
pa.on_faulty_jobs_association.put_in_error=false
----
If set to true, the <<_glossary_job_planner_calendar_asssociation,*Calendar Association*>> with be set to FAILED and <<_glossary_job_planner,*Job Planner*>> will

...
The <<_glossary_job_planner,*Job Planner*>> can send notifications to users when particular events occur. The <<_glossary_notification_service,*Notification service*>> user guide documentation can be found link:../user/ProActiveUserGuide.html#_notification_service[here] and it's
configuration in the admin guide link:../admin/ProActiveAdminGuide.html#_notification_service[here].

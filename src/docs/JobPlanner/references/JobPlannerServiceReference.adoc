...
However, the user can eventually have multiple authorizations due to a mix of username and user-group related authorizations for an association or a group of them (due to an authorizations resulting from the second type of RBAC operations).
...
* Read: The user or user group can only read the association(s) and visualize its planning in the gantt chart.
* Write: The user or user group have, in addition to the read right, the privilege to modify the association(s)' variables, deactivate or reactivate it(them).
* Admin: The user or user group have, in addition to the write rights, the ability to delete an association(s).
Moreover, they can create, modify or delete the authorizations of other users for the association(s).
* No Access: The user or group of users have no access to the association(s).
This access type is usually used when the admin wants to authorize a user group except for some specific users, or for example authorize access to her/his all associations except for some of them.

To manage the association authorizations, first you need to select one then a shield-like button above the associations view in the Job-Planner calendar associations portal will be activated, as shown in the figure below.

image::../../images/Association-authorisations-button.png[align=center]

Once you click on it, a dedicated authorization view will open.
The figure below shows an example.

image::../../images/Association-authorisations-view.png[align=center]
...
* Second, under the buttons, there is the list that shows the authorizations that are assigned to the selected association.
...
* Priority: This attribute is only used for access associated with a group. Thus, if the user belongs to 2 groups, each of which has a different type of access, the system will choose the access with the highest priority for the user.
...
The figure below shows an example of an authorization which authorize the user named `Olivia` the `write` access to association `1`.

image::../../images/Create-association-authorisation.png[align=center]
...
image::../../images/List-of-authorisations.png[align=center]

Once the first authorization has been created, the admin will be able to visualize a "shield" icon next to the association to which the authorization was created.
The figure below shows an example.

image::../../images/Admin-association-shield.png[align=center]

Moreover, when the user, who is benefiting from the authorization, log in to his/her account, she/he will be able to visualize an icon indicating the rights she/he possess over the association(s).
These icons are the following:

image::../../images/Association-authorrizations-icons.png[align=center]
...
* The third icon indicates that the user has "admin" rights over the association(s).

The figure below shows how the user visualize the authorization icon next to the association.

image::../../images/User-association-icon.png[align=center]
...
image::../../images/Delete-association-authorisation.png[align=center]
...
image::../../images/Update-association-authorisation.png[align=center]

Once the selection is made the authorization will be updated as shown in the figure below.

image::../../images/Updated-association-authorisation.png[align=center]

To manage authorization for all associations owned by a specific user, the process is nearly identical.
The admin or the associations' owner need to click on the dedicated button above the associations view in the Job-Planner calendar associations portal will be activated.
The button has a shield-like with a user icon, as shown in the figure below.

image::../../images/Association-owner-button.png[align=center]

Once you click on it, a dedicated authorization view will open.
The figure below shows an example.

image::../../images/Association-owner-view.png[align=center]

Each authorization has the same attributes as an association authorization, however, only the association ID is replaced by `Owner`.
This attribute is used to give access to all associations created by the specified username.

To create a new associations' owner authorization, the admin just needs to click on the "+" button.
Then the view of creating authorization (as shown in the figure below) will be popped up.
The admin needs to fill in the authorization attributes (as presented above), then click the "Add authorization" button.
The figure below shows an example of an authorization which authorize the user named `Joseph` the `write` access to all of admin's authorizations. .

image::../../images/Create-association-owner-autthorisation.png[align=center]

Once the admin has created an authorization, it will appear in the authorizations list in the authorization view, as shown in the figure below.

image::../../images/List-of-owner-authorisations.png[align=center]
...
* If the username-assigned authorization for the association exists, it is prioritized and its access type will be the user's resulting rights over the association.

* If the previous case does not exist, the username-assigned associations' owner authorization access type will be the resulting user's rights over the association.

* If multiple user-groups association authorizations exist for the selected association, without the previous cases, the resulting user's rights over the association will be the access type of the group authorization that have the highest priority.
...
* If the previous case does not exist and multiple user-groups assigned associations' owner authorization exist, the resulting user's rights over the association will be the access type of the group authorization that have the highest priority.
The same apply as before if the authorizations have the same priority level.

* If all cases do not exist, the user will have no-access to the association (unless she/he is a Job-Planner admin or the association's creator).

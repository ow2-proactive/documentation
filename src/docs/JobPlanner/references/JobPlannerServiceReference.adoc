...
Available Job planner events with their description and example values are detailed in the table in link:../user/ProActiveUserGuide.html#_job_planner_notifications[Job Planner event types].

== Job Planner Configuration

<<_glossary_job_planner,*Job Planner*>> provides features to specify actions it will take regarding different situations that it can encounter when submitting <<_glossary_workflow,*Workflows*>> according to a <<_glossary_calendars_definition,*Calendar*>>.

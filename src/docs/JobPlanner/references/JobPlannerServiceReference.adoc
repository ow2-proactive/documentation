...
Job Planner uses a  Calendar Definition to know how the job will be planned over the time. As shown on the example below, this definition is composed of 4 fields:
 ...
 - a description (providing humain-readable description of the used cron expression, when is the Calendar Definition used, etc.)
 - a time zone to define a specific time zone for the future scheduled jobs depending on the region (Europe/Paris, US/Eastern, etc.)
...
The aim of the Time Zone is to make the user able to select a specific time zone for his future scheduled jobs depending on the region. The default time zone is the client time zone and the server time zone is always displayed on the top right corner.

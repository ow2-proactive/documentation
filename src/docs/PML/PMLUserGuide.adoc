...
| `IMPORT_FROM`
| Downloads files or folders to the data space.
| String (default=PA:GLOBAL_FILE)
...
| `WALLTIME`
|  Determines the maximum execution time of a task
...
WARNING: PNote que as variaveis `TIME_TO_RETRIEVE_IN_SECONDS`, `TIME_TO_CHECK_ONLINE_IN_SECONDS` and `WALLTIME` are valid only for the Copernicus platform.

...
| Int (-1 means preview all the rows)
| `NATIVE_SCHEDULER`
| Name of the Native Scheduler node source to use when the workflow tasks must be scheduled on a native scheduler such as SLURM, LSF, etc.
| String (default=empty)
| `NATIVE_SCHEDULER_PARAMS`
|Parameters given to the native scheduler (SLURM, LSF, etc) while requesting a ProActive node used to execute the workflow tasks.
| String (default=empty)
| `NODE_ACCESS_TOKEN`
|If not empty, the workflow tasks will be run only on nodes that contains the specified token.
| String (default=empty)
...
| Boolean (default=False)
| `CONTAINER_IMAGE`
| Specifies the name of the image that will be used to run the workflow tasks.
| String (default=Empty).
|===

...
| String (default=Empty)
| `USE_NVIDIA_RAPIDS`
| If True, the service will be configured to use the GPU and the library Nvidia Rapids.
| Boolean (default=False)
| `DRIFT_ENABLED`
...
| `GPU_ENABLED`
| Configures the service to use the GPU and the library Nvidia Rapids.
| Boolean (default=False)
| `PROXIFIED`

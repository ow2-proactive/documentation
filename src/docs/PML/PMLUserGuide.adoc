...
https://ai.googleblog.com/2017/04/federated-learning-collaborative.html[Federated Learning (FL)] enable us to train an algorithm across multiple decentralized devices (or servers) holding local data samples, without exchanging them.
The `federated-learning` bucket contains a few examples of Federated Learning workflows that can be easily used to build a common, robust machine learning model without sharing data, thus allowing to address critical issues such as data privacy, data security, data access rights and access to heterogeneous data.
...
It can federates any workload using any ML framework, and any programming language.

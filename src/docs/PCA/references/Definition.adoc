...
[[_definition_Action]]
Action::
An Action will launch a workflow on an instance modifying its status. An action is characterized by < a Service ID called pca.service.id in the liste of the workflow Generic Information + a workflow ID + a state called pca.states in the liste of the workflow Generic Information>.
An Action goes from an Initial state, and conduct to a Target state after the transition is completed.
An Action must be deterministic: for an Initial given state, an action can conduct to a unique Target State.

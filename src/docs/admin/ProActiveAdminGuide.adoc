...
The Kubernetes YML configuration pulls the ProActive Server, Node, and Database images from the https://dockerhub.activeeon.com/[Private Activeeon Docker Registry^].
...
After installing the Chart, all ProActive pds are deployed. The ProActive Web Portals will be available at `<PROTOCOL>://<PUBLIC-IP>:<PORT>/`. The `<PROTOCOL>`, `<PUBLIC-IP>`, and `<PORT>` are specified in the YML file.

The user can access any of the four portals using the default credentials: `admin/<PROACTIVE_ADMIN_PASSWORD>`. `PROACTIVE_ADMIN_PASSWORD` is also defined in the YML file.
...
image::install_config.png[align=center, width=512, height=256]
...
image::agent_control.png[align=center, width=612, height=306]
...
image::config_editor_general.png[align=center, width=612, height=306]
...
image::config_editor_connection.png[align=center, width=612, height=306]
...
image::config_editor_planning.png[align=center]

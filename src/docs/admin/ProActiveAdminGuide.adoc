...
The Resource Manager allows for Node Sources to be *undeployed*. In this case, the Node Source is shut down and its Nodes are removed, but the Node
Source definition is kept. An undeployed Node Source can be *redeployed* later, using its initial configuration.

Node
Source undeploying can be *preemptive* or not. If a Node Source undeploying is initiated with a non-preemptive
requirement, then the Nodes of this Node Source which are currently running Tasks will not be removed until their Task
is finished. Consequently, the Node Source will be completely undeployed once all its Nodes have finished executing their Task.

Deploying and undeploying Node Sources is possible from the *Resource Manager portal*, the *REST API*, and the *command line
client*.

TIP: The deployment status of Node Sources is persistent. If the Resource Manager is restarted, then the Node Sources will be *restored*
in their previous deployed or undeployed state.
...
== Full control of the scheduling policy

Default scheduling policy can be extended to perfectly fit with user scheduling constraints. For instance, we integrated a license policy
to control the number of concurrent licenses usage. To use it, user has to edit `PROACTIVE_HOME/config/scheduler/settings.ini` following this

----
pa.scheduler.policy=org.ow2.proactive.scheduler.policy.license.LicenseSchedulingPolicy
----

Then, the maximum number of licenses per software must be specified into `PROACTIVE_HOME/config/scheduler/license.properties` like this
...
node source can be done through the Resource Manager portal, through the *REST API*, or through the <<_cli,CLI>>.

** Through the *Resource Manager portal*, the form to create a node source contains a checkbox that specifies nodes
recoverability for this node source.

** Through the *REST API*, the `nodesRecoverable` parameter of the `rm/nodesource/create` and `rm/nodesource` *REST
endpoints* specify whether the nodes of the new node source will be recoverable. _Nodes recovery will be disabled_
for this node source if the given value does not match any case of the "true" string.

** Through the *CLI*, the `createns` and `definens` *commands* take an optional parameter to specify whether the nodes of
the new node source will be recoverable. _Nodes recovery will be disabled_ for this node source if the optional
parameter is provided and does not match any case of the "true" string.

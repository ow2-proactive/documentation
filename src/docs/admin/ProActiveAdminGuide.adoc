...
The provisioning of ProActive Nodes is controlled by a specific <<../user/ProActiveUserGuide.adoc#_generic_information,*Generic Information*>> `"NS"` which can be defined at Task or Job level.
...
Using the <<_glossary_rm_web_interface,*Resource Manager Web Interface*>>, you can create a <<_glossary_node_source,*Node Source*>> used to aquire <<_glossary_proactive_node,*ProActive Nodes*>> from a Native Scheduler.
 ...
 * `RMCredentialsPath` : point to a file which contains the credentials of an administrator user which will own the node source. The ProActive Scheduler Server release contains two admin users credentials files : `config/authentication/rm.cred` and `config/authentication/admin_user.cred`
 ...
 * `sshOptions` : additional options which can be passed to the ssh command used to connect to NSFrontalHostAddress.
 * `NSNodeTimeoutInSeconds` : timeout to wait for the deployment of ProActive Nodes on the cluster. As it the time needed to deploy ProActive Nodes depends on the cluster load, this timeout should be a large value.
 * `ìmpersonationMethod`: when a job is submitted to the native scheduler, the submission is performed under the current ProActive Scheduler user. An impersonation is thus performed between the scheduler process user and the target user.
 ...
 ** `sudo`: similar to `none` regarding the connection to the head node, but a `sudo` command will be initiated to impersonate as the current Scheduler User, before doing a job submission.
 ...
 * `nsSubmitCommand` : this is the main command used to start ProActive Nodes on the cluster. The command can used patterns which will be replaced dynamically by the ProActive Resource Manager. +
[cols=2*]
|===

|`%NS_JOBNAME%` |contains a configurable job name dynamically created by the resource manager.

|`%LOG_FILE%` |contains a log file path dynamically created by the resource manager and located in side the NSSchedulerHome installation. This log file is useful to debug errors during native scheduler job submission.

|`%PA_USERNAME%` |contains the current ProActive Scheduler user.
|===

Depending on the actual native scheduler implementation, `nsSubmitCommand` will vary, here are examples definitions: +
 ...
 * `nsKillCommand` : this is the command used to kill ProActive Nodes started previously by the nsSubmitCommand. It can use the following patterns:
...
Similarly to nsSubmitCommand, `nsKillCommand` will vary for each native scheduler syntax: +
 ...
 * `submitReturnsJobId`: is the job id returned plainly when calling the nsSubmitCommand? This is the behavior of PBS, and this is why this setting should be set to `true` when using PBS.
 ...
 * `maxDeploymentFailure`: NativeSchedulerInfrastructure can make several attempts when starting a ProActive Node on the cluster.
 ...
 * `schedulerUrl` : the url used by the ProActive Nodes to contact <<_glossary_scheduler,*ProActive Scheduler*>>. This url is displayed on ProActive server startup. Example: `pnp://myserver:64738`.
 * `schedulerCredentialsPath` : point to a file which contains the credentials of an administrator user which will connect to the scheduler. The ProActive Scheduler Server release contains two admin users credentials files : `config/authentication/rm.cred` and `config/authentication/admin_user.cred`
...
It must contain the name of the target node source. Example:
...
The NS value can also be defined at the job level, in that case, every tasks of this job will be executed on the Native Scheduler node source:
...
It is also possible to define in a ProActive Task or Job the `#BATCH` generic information. This parameter allows to provide custom arguments to `nsSubmitCommand`.
...
If the node remains in `Deploying` state, it is possible to monitor the job execution on the native scheduler itself (by logging into the head node):
...
If the native scheduler job is in _running_ state, but the ProActive Node associated remains in `Deploying` state, it probably means there is a connection issue between the ProActive Node and the Resource Manager.
 ...
 * inspect the output of the native scheduler job if provided in the `nsSubmitCommand`
 * add the following log4j loggers to the ProActive Server `config/log/server.properties` +
```properties
...
```
 * inspect the Scheduler server logs.
 
When the deployement is successful, the ProActive Node state will change to `Free` and shortly after to `Busy` as soon as the associated task will be deployed on the node.
After the task completes, the node will be removed and the native scheduler job will be cancelled using the `nsKillCommand`.
...
From now, given a task, user can specify which software licenses are required, by adding the REQUIRED_LICENSES generic information,
which value can be

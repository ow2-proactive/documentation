...
node source can be done through the *REST API* of the Resource Manager or through the <<_cli,CLI>>.

** Through the REST API, the `nodesRecoverable` parameter of the `rm/nodesource/create` *REST endpoint* specifies
whether the nodes of the new node source will be recoverable. _Nodes recovery will be disabled_ for this node source if
the given value does not match any case of the "true" string.

** Through the CLI, the `createns` *command* takes an optional parameter to specify whether the nodes of the new node
source will be recoverable. _Nodes recovery will be disabled_ for this node source if the optional parameter is provided
and does not match any case of the "true" string.

+
However, all node sources that are created through the *Resource Manager portal* will be created _with recoverable
nodes_. The only way to disable nodes and task recovery for these node sources is to set the global property
`pa.rm.nodes.recovery` to `false`.

...
All password fields in the `inputConfig.json` file should be left empty. Their values will be provided at runtime. They will be then encrypted and inserted back into the `inputConfig.json` file.
...
This method provides a simple and minimal installation of the ProActive server without supporting advanced configurations (such as LDAP, additional Users, TLS, or database setup).

It installs the required configuration of the ProActive server by passing passwords as command-line arguments. This allows you to supply the required credentials directly via the command line, avoiding the need for interactive password input during installation.

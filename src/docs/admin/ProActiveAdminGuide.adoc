...
In the ProActive Scheduler, all the computational resources (<<_glossary_compute_host,Compute Hosts>>) are managed as a set of <<_glossary_node_source,*Node Sources*>>.
...
As shown in the previous screenshot, the ProActive Scheduler provides a <<_glossary_rm_web_interface,*Resource Manager Web Interface*>> to show and manage all the node sources with its nodes.

Adding a cluster of Compute Hosts to the ProActive Scheduler requires to *create a node source*.
Its typical process involves copying the necessary ProActive binary files to all those Compute Hosts,
running ProActive Nodes on the hosts, and connecting the nodes to the ProActive Scheduler.
There are two principal ways of doing that:

- Launch a process of ProActive Node *on the Compute Host* and connect it to the ProActive Scheduler
- Initiate the deployment *from the ProActive Scheduler*

If you are not familiar with ProActive Scheduler you may want to try the <<run_node_manually,*first method*>> as it's directly executed on the Compute Hosts.
...
The <<_deploy_nodes_from_proactive_scheduler,*second method*>> commands the ProActive Scheduler to remotely launch the process of ProActive Nodes on Compute Hosts.
...
- the local Server Host (i.e., the machine installed the ProActive Scheduler)
...
To support the deployment of these various computational resources, the ProActive Scheduler provides a set of supported <<_node_source_infrastructure, Node Source Infrastructures>>.
...
When you connect the Compute Hosts to the ProActive Scheduler through the *first method*, a node source of <<_default_infrastructure_manager,*Default Infrastructure*>> is automatically created.
...
//The *second method* supports all the various kinds of computational resources by using different <<_node_source_infrastructure,*node source infrastructures*>> to create Node Sources.
...
To use `proactive-node` command, you need to unpack the release archive of the ProActive Scheduler on the Compute Host and run the following command
----
...
It is also possible to launch a ProActive Node without copying the ProActive Scheduler release to the Compute Host:
...
[[_deploy_nodes_from_proactive_scheduler]]
=== Deploy ProActive Nodes from the ProActive Scheduler

The second way of deploying ProActive Nodes is to create <<_glossary_node_source,*Node Sources*>> from the ProActive Scheduler.

ProActive Scheduler provides three ways to create a Node Source:
...
Whichever the way you use to create a node source, ultimately you just need to choose a *Node Source Infrastructure* from the list of <<_node_source_infrastructure,supported Node Source Infrastructures>>
and a <<_node_source_policy,*Node Source Policy*>> that defines rules and limitations of nodes' utilization.
...
[[_deploy_local_nodes]]
==== Deploy local nodes from the ProActive Scheduler
In order to deploy the ProActive nodes locally (i.e., on the <<_glossary_server_host,server host>> where running the ProActive Scheduler),
...
See <<_local_infrastructure_manager, *Local Infrastructure*>> reference for details of each parameter.
...
See <<_node_source_infrastructure, *Node Source Infrastructure*>> reference for details of each Cloud Node Source Infrastructure parameters.

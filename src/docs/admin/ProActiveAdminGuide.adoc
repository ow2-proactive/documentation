...
It is possible to control the acceptable password strength by using the following options in `PROACTIVE_HOME/config/scheduler/settings.ini`:

----
# Enable password strength check, used when a user is created or modified
pa.scheduler.password.strength.enable=false
...
pa.scheduler.password.strength.regexp=^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[*.!@$%^&(){}\\[\\]:;<>,.?/~_+\\-=|]).{8,32}$
----
...
In order to enable this feature, simply set `pa.scheduler.password.strength.enable=true` and modify `pa.scheduler.password.strength.regexp` according to your needs.
...
----
pa.rm.username.regexp=^[A-Za-z0-9_\\-@.]+$
----

In `PROACTIVE_HOME/config/scheduler/settings.ini`:

----
pa.scheduler.username.regexp=^[A-Za-z0-9_\\-@.]+$
----

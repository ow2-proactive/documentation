...
* `Needed` - Metric shows how much more nodes Scheduler needs to schedule all possible tasks.
...
Please notice, that *Needed Nodes* shows how many more nodes could be used right now. However, it does not give indication on the kind of nodes needed, e.g. Selection Script of the task could require some specific nodes, thus *Needed Nodes* could be positive while we still have free nodes (they can be inappropriate for the current task).

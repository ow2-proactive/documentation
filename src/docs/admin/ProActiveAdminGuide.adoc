...
Scheduler, Resource Mananger, and three microservices require to have direct access to the database.
...
==== Install Agent Windows Using Silent Mode

When installing the agent in batch on several desktop machines one might use the
command line installer instead of the graphical one. This eases to deploy several
installations in parallel. To do so it is requiered to have remote access to
command prompt with full admin priviledges.

The silent option works to execute the installer directly on the command line
without the need for a graphical interfaces. When the silent option is activated
we need to provide at least user and password as parameters, domain name is optional
since it is automatically detected.


For instance to install the latest windows agent go
link:http://jenkins.activeeon.com/job/agent-windows/lastSuccessfulBuild/artifact/[here]
and select the installer right for your platform, in my case `ProActiveAgent-8.3.0-standalone-x64-setup.exe`.
Now open a command prompt as administrator and go to the directory
where the installer resides. Then run the command below so it will install the
Proactive windows agent from the prompt. Note that any previous installation will
be uninstalled first before installing the new version.
...
More command line options below. Note that these opitons are only available when using
the `/S` silent option, which is also included on the list for completness.
...
|`/USER=user` | Associate `user` account to manage proactive agent service, creates user if does not exist

|`/PASSWORD=pass` | Define password for the proactive agent `user`

|`/DOMAIN=dom` | Specify `dom` as the windows domain

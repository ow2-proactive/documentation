...
=== Install and Start ProActive using Docker Containers
...
==== Define and configure ProActive containers
...
This Docker Compose configuration `docker-compose-hsqldb.yml` can be used to run the ProActive server container with an embedded HSQL Database.
...
This Docker Compose configuration `docker-compose-postgres.yml` can be used to run the ProActive server container with an external PostgreSQL Database container.
...
Te Docker Compose configuration `docker-compose-mysql.yml` can be used to run the ProActive server container with an external MySQL Database container.
It will start up to three containers, one for the server, one for the Nodes, and one for the Database.
...
ProActive could be easily deployed and started in a Kubernetes cluster. For this purpose, we provide a ready-to-use Kubernetes configuration that enables you running ProActive Workflows and Scheduling as a set of Pods.
...
==== Set up ProActive Pods using Helm
...
You can then run `helm search repo activeeon` to search for the local `activeeon` Helm repository. You can also access to the Chart on the public Helm hub: https://artifacthub.io/packages/helm/activeeon-proactive-scheduler/activeeon-proactive-chart[Artifact Hub^].
...
link:./references/kubernetes/values.yml[Kubernetes configuration file, title="Click to donwload"^]:
[source,yaml]
----
include::./references/kubernetes/values.yml[]
----
...
Administrators can define which labels can be applied to jobs in the Admin menu of the <<_glossary_scheduler_web_interface,*Scheduler Portal*>>. Physically, possible labels are stored in the scheduler database.

Once a label is applied to a job, it will be stored alongside the job information in the scheduler database.
If, in the future, the administrator changes the label name or deletes it, this operation will not be applied retroactively (the original label attached to the job will not be modified).
...
ProActive supports several methods to authenticate users:
...
When defined as a <<_ldap,LDAP attribute>>, the name of the <<_configure_tenant_attribute,attribute>> which will contain tenant information must be configured.

When the authentication method is set to <<_ldap_with_multiple_domains,LDAP with Multiple Domains>>, the tenant is automatically set to the current user _LDAP domain_.

Tenants allow users to work in isolated silos, where users belonging to one tenant cannot see the activities launched by users belonging to other tenants.
Tenants can thus improve the management and governance of an organization's hybrid compute infrastructure.

In order to isolate tenants, the setting `pa.scheduler.core.tenant.filter` must be set to `true` in `PROACTIVE_HOME/config/scheduler/settings.ini`.
...
.Users Tenant View
image::TenantSPUsers.png[Users Tenant View]

.Experts Tenant View
image::TenantSPExperts.png[Experts Tenant View]
...
Tenants can also be used to restrict access to <<_glossary_node_source,Node Sources>> (see <<_policy_common_parameters,Node Source Policy Common Parameters>>).
...
LDAP can be used to retrieve the tenant information associated with a user. Tenants are used to isolate users into organization units. Each unit will not be able to monitor other units' activity.
...
IMPORTANT: In multi-domain mode, when a user logs in to any portal of the ProActive server, the user must specify the domain used.

For example, if _user1_ belongs to _domain1_, the information provided in the login form must be _domain1\user1_.
This is essential to allow the ProActive server select the adequate ldap configuration where the user is registered.

NOTE: The user domain name will be automatically recorded as <<_tenants, tenant>> name.
...
----
Mandatory for any scheduler user, defines whether each member of the group can interact (kill, pause, resume, etc) with other users jobs (`false`) or only with the jobs that belong to the user (`true`).

----
permission org.ow2.proactive.scheduler.permissions.ChangePriorityPermission "1,2,3";
----
Mandatory for any scheduler user, defines the set of link:../user/ProActiveUserGuide.html#_job_priority[Job Priorities] that the group is allowed to use in submitted workflows.
...
----
Optional permission that allows member to interact with any job that contain the mentioned link:../user/ProActiveUserGuide.html#_generic_information[Generic Information].
...
----
Administrative permission that allows changing the server's <<_scheduling_policies,Scheduling Policy>>.

----
permission org.ow2.proactive.scheduler.permissions.TenantAllAccessPermission;
----
Administrative permission that allows access to all tenants when tenant isolation is enabled. See <<_tenants>>.

Finally, the following permissions are needed for administrative access:
...
----
This mandatory permission defines which portals are accessible for members of the group.
...
----
This permission gives administrative access to <<_glossary_cloud_automation_service,Service Automation>>. A member of a group with such permission can interact with any service (not only owned services).
...
----
This permission gives administrative access to <<_glossary_job_planner,Job Planner>>. A member of a group with such permission can interact with any calendar association (not only owned associations).
...
----
This permission gives administrative access to <<_glossary_notification_service,Notification Service>>, allowing to delete any notification.

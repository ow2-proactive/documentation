...
For instance, we provided support to three database systems, namely HSQLDB, Postgres, and MySQL.
...
Login into the Private Activeeon Regitry
----
 ...
 * Every file transfer will use the SFTP protocol when it's configured. Note that SFTP protocol will be less efficient than the ProActive protocol when transferring files on the local file system.

The SFTP protocol can also be used for the Global Space, but in that case, all file transfers will be accomplished by a single system account.

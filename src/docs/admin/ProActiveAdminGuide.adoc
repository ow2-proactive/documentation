...
Initially, the ProActive Server has be started **at least once** to fill up the ProActive database.

- **Stop the ProActive Server**

Then, once the ProActive Server is up and the web portals become accessible, you have to stop the ProActive Server and proceed to the next operations.

Please, do not stop the Postgres Server.

- **Prepare the database purge operation**

Prior to execute the purge operation, a preliminary configuration step is necessary (this preliminary step is done once for a given ProActive installation).

While the ProActive Server is stopped and the Postgres Server is running, the following SQL queries have to be executed in order to alter several database tables (this is needed due to a PotsgreSQL driver issue):

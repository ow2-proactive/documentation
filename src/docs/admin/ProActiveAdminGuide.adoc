...
The archive contains all required dependencies with no extra prerequisites. ProActive Scheduler can be started in two different ways. Either as a java process or as a system service.
...
=== Start ProActive as a Java process
...
These images can be used out of the box, on any operating system. You can customize the server and Database settings, located in the `.env` file according to your requirements.
...
We provide a public Docker Compose installation package that you can use to configure and start the docker installation in your environment.
It contains the Docker Compose configuration file (`.env`) and the YAML compose definitions (`docker-compose-hsqldb.yml` and `docker-compose-pg.yml`).
...
The `.env` file referenced above can be used as it is for a minimal installation. However, it requires the definition of five mandatory parameter values:
...
- `DOCKER_IMAGE_POSTGRES_NAMESPACE`: Defines the ProActive Postgres namespace.
...
- `PWS_PAMR_PORT`: The internal port to use for PARM communication. By default, 33647.

- `PWS_JOB_CLEANUP_DAYS`: Remove jobs history older than (in days). By default, 30.
...
NOTE: The pem/crt/pfx certificate as well as the key have to be placed in the backup folder and their absolut paths need to be provided. See section <<_compose_required_configuration>> to learn more about the backup folder to be created beforehand.
...
Once you set the right configuration options in the `.env` file, you can use the Docker Compose configuration provided in the installation package (`docker-compose-hsqldb.yml`) to run the ProActive server container with an embedded HSQL Database.
...
Then, you have to configure the absolut paths to these 3 folders in the `docker-compose-hsqldb.yml` file using the `device:` property (e.g., in the example above 3 folders are created and configured: `/opt/proactive/default`, `/opt/proactive/previous`, and `/opt/proactive/backup`).
...
Once you set the right configuration options in the `.env` file, you can use the Docker Compose configuration provided in the installation package `docker-compose-pg.yml` to run the ProActive server container with an external PostgreSQL Database container.
...
Wait until the `docker logs -f proactive-scheduler` command output shows that ProActive is started:
----
$ docker logs -f proactive-scheduler
....
Loading workflow and utility packages...
Packages successfully loaded
ProActive started in 546.476727488 seconds
----
...
==== Prepare the database for the purge operation
...
ProActive could be easily deployed and started on `AKS` (Azure) or `bare-metal` (On-prem) Kubernetes clusters. For this purpose, we provide a ready-to-use Kubernetes configuration that enables you running ProActive Workflows and Scheduling as a set of Pods.
...
The Kubernetes installation package pulls the ProActive Server, Node, and Database images from the https://dockerhub.activeeon.com/[Private Activeeon Docker Registry^].
...
https://helm.sh[Helm^] is a Kubernetes deployment tool for automating creation, packaging, configuration, and deployment of applications and services to Kubernetes clusters.
...
You can use default values in the `values.yaml` file. However, the following specific properties must be configured as described below:
...
For an on-prem installation, the Kubernetes Node name to host the ProActive scheduler and database Pods:
...
For an on-prem installation, a directory must be created beforehand on the Node defined by the `nodeAffinity` (see `nodeAffinity` above) to host the ProActive scheduler and database Pods' data:

- `persistentVolumes.localVolumeConfig`:
** `scheduler.persistentVolume.localPath`: Absolut path to the empty directory created on the Node
** `database.persistentVolume.localPath`: Absolut path to the empty directory created on the Node
...
- `namespace`: Namespace to be used to create all ProActive installation resources. By default, ae-dev.
- `ports`:
** `httpPort`: Http port. By default, 8080.
** `nodePort`: For on-prem installation, you can customize the Node Port. By default, 32000.
...
- `persistentVolumes`:
** `localVolumeConfig`: Set the local node volumes settings according to your needs if you are targeting on-prem clusters. By default, local node spaces are provisioned.
...
- `resources`: Set the resources requests for the scheduler, node, and database. By default, minimal settings are defined.
...
After installing the Chart, all ProActive pods are deployed. Using the default configuration, you can access the ProActive Web Portals for example:
...
The new release version has to be provided in the `values.yaml` file:
...
ProActive could be easily deployed and started automatically using Shell scripts. For this purpose, we provide a ready-to-use shell installer that enables you running ProActive Workflows and Scheduling as a Linux service.
...
- The `install_configure_postgresql.sh` file, which installs the Postgres DB and configures the ProActive server to support the installed database.
...
===== Required Configuration
You can use default values in the `inputConfig.json` file. However, the following specific properties must be configured as described below:

- `INSTALLATION_CONFIGURATION`
** `ARCHIVE_LOCATION`: The archive location could be an absolute file path of the ProActive archive or an http(s) link.
** `INSTALLATION_DIRECTORY`: Path of the folder where ProActive will be installed. You can change the path according to your preference. This path will be created by the installer. No need to create it in advance.
...
** `RUN_HEALTH_CHECK_ONLY`: If enabled, only health check tests will be executed. It supposes that the installation is already done and the ProActive Server is already running. It is recommended to not activate it unless you would like to run the health checks separately.
...
NOTE: The pem/crt/pfx certificate as well as the key have to be provided with their absolut paths. No need to place them in the backup folder.
...
The description of the configuration options for the remaining ProActive components (`DB_RM_CONFIG`, `DB_CATALOG_CONFIG`, `DB_PSA_CONFIG`, `DB_NOTIFICATION_CONFIG`) is the same as described for the scheduler DB above.
...
The process of Upgrading ProActive described in section <<_compose_upgrade>>.

In this section, we will see how to upgrade ProActive using the Provided Shell scripts.
...
<script>
    ...
    if (confVersion && confVersion.includes('SNAPSHOT')) {
        ...
        composeInstaller.innerHTML = "$ wget <a href='https://www.activeeon.com/public_content/proactive-installers/" + confVersion + "/proactive-compose-installer-package-dev.zip'>https://www.activeeon.com/public_content/proactive-installers/" + confVersion + "/proactive-compose-installer-package-dev.zip</a>";
        helmInstaller.innerHTML = "$ wget <a href='https://www.activeeon.com/public_content/proactive-installers/" + confVersion + "/proactive-helm-installer-package-dev.zip'>https://www.activeeon.com/public_content/proactive-installers/" + confVersion + "/proactive-helm-installer-package-dev.zip</a>";
        shellInstaller.innerHTML = "$ wget <a href='https://www.activeeon.com/public_content/proactive-installers/" + confVersion + "/proactive-shell-installer-package-dev.zip'>https://www.activeeon.com/public_content/proactive-installers/" + confVersion + "/proactive-shell-installer-package-dev.zip</a>";
    }
</script>
++++
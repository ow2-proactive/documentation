...
When started, Scheduler, Resource Manager and Web interfaces URLs are displayed.
...
All configuration files of Proactive Scheduler can be found inside the archive.
...
Adding Compute Hosts of a cluster to the ProActive Scheduler typically involves unpacking the software on all those hosts.
Once it's done you need to run a ProActive Node on the host and connect it to the Scheduler. There are two principal ways of
doing that:

* Launch a process on the Compute Host and connect it to the Scheduler
* Initiate the deployment from the ProActive Scheduler - <<_deploy_nodes_through_ssh,node source creation>>

If you are not familiar with Proactive Scheduler you may want to try the *first method* as it's easier to understand.
With the combination of <<_agents> it gives you the same result.
...
by launching commands over one of the protocols we support. For instance it can be useful when before launching a ProActive Nodes you'd
...
----
where +-r+ option is used to specify the URL of the *ProActive Resource Manager*. You can find this URL in the output of the Scheduler launcher.

It is also possible to launch a ProActive Node without even copying ProActive to a Compute Host. To to is you need to:

* Open a browser on the Compute Host
* Go to the http://SCHEDULER_ADDRESS:8080/rm[ProActive Resource Manager Web Interface]
* Use default demo/demo account to access the Web Interface
...
When a node source is created you can choose an *infrastructure manager* from one of supported <<_node_source_infrastructure>>
 and a <<_node_source_policy>> that defines rules and limitations of nodes' utilization.

To create a node source you can either:

* Use the web interface of the *ProActive Resource Manager* ('Add Nodes' menu)
* Use the REST API
// TODO add link dev
...
your ProActive Nodes and number of tasks that can be executed in parallel, e.g.
...
* Launch nodes automatically when a Compute Host starts
* Restart ProActive Nodes if they fail by some reason and reconnect them to Proactive Scheduler
...
All users authenticated in the ProActive Resource Manager have they own role according to granted permissions.

...
Port 8443 is used for listening to connections when HTTPS is enabled.
...
NOTE: If you are familiar with Nginx or a similar tool, you can also use it to
enable HTTPS connection to Web applications. It has the advantage to make
possible to have ProActive web applications and additional ones to coexist.
...
This command prompts for information about the certificate and for password to
protect both the keystore and the keys within it. The only mandatory responses
are to provide a password and the fully qualified host name of the server. Once

...
NOTE: the same applies on Windows. To use a shared temporary folder, create a folder shared by allProActive users e.g. C:\TEMP and define
...
To enable it at the system level, edit the /etc/sudoers file to allow passwordless sudo from the account running theProActive node
...
To enable this configuration on theProActive node, start it with the system property `pas.launcher.forkas.method` to _none_

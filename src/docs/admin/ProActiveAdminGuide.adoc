...
The vacuum operations should be done after the first start of the ProActive server. So, the ProActiver Server has be to be running.

- Prepare the database purge operation

Prior to execute purge operation, a preliminary configuration step is necessary (this preliminary step is done once for a given ProActive installation).

While the ProActive Server is running, the following SQL queries have to be executed to alter several database tables (this is needed due to a PotsgreSQL driver issue):
...
After that the initial configuration step is done, it is possible to execute a full `vacuum` by using the following commands:

...
We provide an internal backup mechanism for the database,
in case HSQLDB is used, and for data files (workflows, logs).
In case another DB is used, it is possible to combine a vendor specific backup mechanism
with the internal mechanism for data files.
...
Backup will be performed according to provided cron expression in `pa.server.backup.period`.
Set file/folders you want to backup into `pa.server.backup.targets`.
Backup will appear in the folder specified by `pa.server.backup.destination`.

The backup mechanism automatically freezes the scheduler
while performing the backup to guarantee data integrity.
Backup may not happen if ProActive Scheduler is busy.
Backup mechanism will try to perform backup several times during the period set in `pa.server.backup.possile.delay`.
If all this time ProActive Scheduler would be busy then the backup will not be performed.

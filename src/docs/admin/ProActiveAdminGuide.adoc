...
by launching commands over one of the protocols we support. For instance it can be useful when before launching an computing node you'd
...
It is also possible to launch a node without even copying proactive on a remote machine. To to is you need to:
...
In order to create a SSH node source you should first configure an *SSH access* from the head node to computing nodes
...
When you click on 'GUI Edit', the following windows appears:
...
When PAMR is activated, the ProActive Scheduler and nodes connects to a PAMR router. This connection is kept open, and used
...
Each node source policy regardless it specificity has a common part where you describe users and groups permissions.
...
Out of the box the Scheduler support time slot policies, cron policies, load based policies and many others.
...
* users and passwords accounts are stored in +config/authentication/login.cfg+.
Each line has to look like *user:passwd*. The default +login.cfg+ file is given hereafter:
+
[source]
----
admin:admin
...
* users membership is stored in +config/authentication/group.cfg+. For each user registered in login.cfg,
...
In Proactive Scheduler, we use standard
http://www.oracle.com/technetwork/java/javase/jaas/index.html[Java Authentication and Authorization Service (JAAS)]
to address these needs.

The file +PROACTIVE_HOME/config/security.java.policy-server+ allows to configure fine-grained access for all users, e.g. who has a right to:
...
The JMX URL on which to connect can be obtained from the Authentication API of the RM or by reading the log file located in $PROACTIVE_HOME/.logs/RM.log.
...
The ProActive Resource Manager supports nodes aggregation from heterogeneous environments.
...
In the resource manager, there is always a default node source consisted of DefaultInfrastructureManager

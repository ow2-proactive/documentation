...
The job labels are stored in a table in the scheduler database.
When a label is attached to a job, the label information will be stored in the job table in the scheduler database.

It is possible to define the job labels list in the Admin menu from Scheduler Portal in order to be able to set a label on a job.
...
The REGEX constrain is specified by the property pa.scheduler.label.regex. If this property is not specified, the default value is ^[a-zA-Z0-9_/-]*$.
...
link:https://www.eclipse.org/jetty/documentation/current/configuring-ssl.html[Jetty documentation].

...
The EDF Policy considers two Generic Info variables: `JOB_DDL` and `JOB_EXEC_TIME`.
...
`JOB_EXEC_TIME` represents job exepcted execution time in the following format: `HH:mm:ss` , e.g. `12:30:00`, `5:30:00`,
or `mm:ss`, e.g. `4:30`, or `ss`, e.g. `45` (45 seconds).

EDF Policy applies 6 following rules until at least one rule sorts two jobs:
...
4. job that started earlier has advantage over job that started later
...
`expectedJobFinishingTime` equals to current date time + job exepcted execution time.

To use the EDF policy, edit the file `PROACTIVE_HOME/config/scheduler/settings.ini` and change the following line:

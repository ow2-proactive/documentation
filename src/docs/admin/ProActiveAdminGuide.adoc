...
. Once a job is finished, set its scheduled time for removal (the job expiration date)
. Periodically trigger the cleaning of the expired jobs off the scheduler and/or the database
...
expression `pa.scheduler.core.automaticremovejobexpression`.
If the `pa.scheduler.job.removeFromDataBase` property is
also set to `true`, a query is executed in-base to remove all jobs that qualify for the bulk removal.

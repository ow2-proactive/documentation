...
Whichever the way you use to create a node source, ultimately you just need to choose a *Node Source Infrastructure* from the list of <<_node_source_infrastructures,supported Node Source Infrastructures>>
...
Please see detailed information about policies in <<_node_source_policies>>.
...
== Add custom third-party dependencies to a workflow classpath

Some workflows may need to add some third-party dependencies to their classpath, so they can be executed properly. The admin determines the required dependencies depending on the workflow implementation and the used libraries. Third-party dependencies can be added to the workflow classpath in three ways (depending on the nodes that will run the workflow):
...
Note that the added dependencies in the addons folder (for the server as well as the node) can be accessed from:

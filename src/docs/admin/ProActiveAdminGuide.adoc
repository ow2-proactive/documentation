...
* For HSQLDB, the drive is already embedded in *PROACTIVE_HOME/addons*
...
Moreover, it also important to notice that the Scheduler, and 3 microservices (*scheduling-api*, *job-planner*, and *proactive-cloud-watch*) have to point to the same scheduler database since they use that one.

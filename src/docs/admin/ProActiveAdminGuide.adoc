...
The `inputConfig.json` file, included in the installation package, is necessary to provide the correct configuration values for installing ProActive.
...
This method provides a simple and minimal installation of the ProActive server without supporting advanced configurations (such as LDAP, Users, TLS, or database setup).
...
This method offers an interactive way to install the ProActive server, guiding the user step-by-step through the necessary passwords to be given interactively.

During this mode, you specify the installation mode using `-i` option (for interactive), and then the installer will ask you to provide the passphrase and the required passwords.
It supports providing passwords for advanced installation such as LDAP, Users, TLS, or database setup.

This mode is ideal for users who prefer a hands-on approach and want to avoid passing sensitive information directly through the command line, enhancing security.
...
NOTE: After the installation is complete, it is recommended to delete the clear-text password file to enhance security.

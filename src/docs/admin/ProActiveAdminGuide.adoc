...
|Keycloak configuration|config/authentication/keycloak.cfg|<<_ldap>>
...
ProActive uses the OpenID Connect (OIDC) protocol to communicate with Keycloak and acquire an access tokens for a given user.
...
In order to enact ProActive access control against a Keycloak server, two steps must be accomplished: (i) deploying and configuring a Keycloak server and (ii) configuring ProActive to connect to the Keycloak server and perform access control against it.
...
2. The administrator of the Keycloak should select a security realm (or create a new one if needed).

===== Configuring a new Keycloak client for ProActive
The administrator of the Keycloak have to configure a new client for ProActive in the chosen realm.
A client is an external software application to be protected using Keycloak.
In the section *General Settings* og the configuration, the parameters following must be properly set (as shown in the screenshot below):
...
* The property `Name` specifies display name of the client  (e.g., ProActive Workflows and Scheduling).
* The property `Description` specifies description of the client (e.g., ProActive workflows engine).

.General settings of Keycloak client
image::KeycloakClientGeneralConfig.png[Keycloak Client General Configuration,500,400]

===== Configuring the authentication mechanism of the client
In the section *Capability config* of the client configuration, the parameters following must be properly set (as shown in the screenshot below):
...
image::KeycloakClientAuthConfig.png[Keycloak Client Authentication Configuration,500,400]
...
image::KeycloakClientRolesConfig.png[Keycloak Client Roles Configuration,500,400]
...
image::KeycloakGroupToRolesBinding.png[Keycloak Role-to-Group Binding,500,400]
...
* Using the fall back to file authentication: ProActive can be configured to use simultaneously file-based authentication and Keycloak-based access control. In this scenario, ProActive Scheduler checks at first user credentials (login/password) and group memberships in
...
** `pa.keycloak.authentication.fallback`: if set to `true`, the user credentials will be checked first in the login file, and then in the Keycloak server. If set to `false`, only the Keycloak server will be queried.
** `pa.keycloak.group.membership.fallback`: if set to `true`, group memberships will be read from the Keycloak server if the user exists, and then from the group file. In that sense, a user will be affected to groups defined in both file and Keycloak server. If set to `false`, only the Keycloak server will be queried.
...
If the roles configured in Keycloak (for the ProActive client) do not already exist, the administrator of ProActive server have to create these roles with the appropriate privileges, as explained in the section <<_creating_new_groups,creating new groups>>.

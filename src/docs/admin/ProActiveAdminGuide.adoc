...
Scheduler, Resource Mananger, and three microservices require to have direct access to database.
Thus they have to be properly configured to have access to database.

To configure Scheduler or Resource Mananger, one have to modify config/scheduler/database.properties
and config/rm/database.properties respectively. It is necaessary to configure five following preperties:
...
Microservice database configuration is always in `/dist/war/MICROSERVICE-NAME/classes/applicaiton.properties`.
To configure database connection it is necessary to configure five following properties:

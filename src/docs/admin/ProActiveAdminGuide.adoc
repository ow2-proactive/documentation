...
=== Tunning Linux for 15K nodes

It is necessary to tune underlying Linux system, in case there will be plenty of nodes in Resource Manager.

There should be big enough number of processes allowed, and number of open files as well.

For 15.000 nodes, it is recommended to set `ulimit -u` bigger than 100.000 and `ulimit -n` at least 65.536.

As well, we recommend to have at least 16GB RAM for 15.000 nodes.

...
Prior to execute the above operations, a preliminary configuration step is necessary (this preliminary step is done once for a given ProActive installation).

After the first start of the ProActive server, the following SQL queries will alter several tables of the database (this is needed due to a PotsgreSQL driver issue):
...
After the initial configuration step is done, it is possible to execute a full `vacuum` by using the following commands:

...
In order to create ssh node source you should first configure an *ssh access* from the head node to computing nodes
that http://www.linuxproblem.org/art_9.html[does not require password]. Then create a text file where you list all
your computing nodes and number of jobs that can be launch in parallel, e.g.
...
In this command line you specify a path to JDK scheduler distribution on remote hosts. They should be the same on
all computing nodes.
...
To install proactive agent on linux


* download the debian installer from the download page of http://www.activeeon.com/community-downloads[ActiveEon website]

* install the package

    sudo dpkg -i proactive-agent-standalone.deb

By default the Linux Agent will launch locally a single worker process (a.k.a. RM-Node) but will not register it
in the ProActive Resource Manager. The user has to configure the agent explicitly by specifying the required
parameters in the “config.xml” file.


===== Configuring Linux Agent

To configure the agent bahavior
...
+
In case there is no such file directory, create the directory. Create a the *config.xml* file or create a symbolic link
...
If the “proactiveuser” group does not exist, create the group and add “proactive” user to the group:
...
In case of *problems* take a look at

    /var/log/proactive/agent/agent.log


=== Installation on a Cluster with Firewall

When the configuration of the network is unfriendly proactive has a special protocol that help to connect machines
to Proactive Scheduler without signitican change in your network firewall configuration. This protocol is
called *PAMR* (Proactive Message Routing Protocol). It has the weakest expectations on how the network is configured.
...
When PAMR is activated, each ProActive Scheduler connects to a PAMR router. This connection is kept open, and used
...
To soften this limitation PAMR can be used with other communication protocol. This way, PAMR is used only when needed.
...
To enable this protocol edit proactive configuration file inside the scheduler distribution
...
Put the same file for all computing nodes. Typically you run the scheduler server on the *same machine* where the router is
launched. This sample configuration requires to open only one port +8090+ for *incoming connections* on the server host
and all the computing nodes will be able to connect to the server.

Now in order to run the scheduler you should first run Proactive Router

    $ cd SCHEDULER_HOME/bin
...
TIP: *Node source policy* is a set of rules and conditions which describes when and how many nodes have to be selected for computations.

Each node source policy regardless it specificity has a common part where you describe users and group permission.
...
The user created the node source is the administrator of this node source. It can add and removed nodes to it, remove the node source itself,
but cannot use nodes if usage policy is set to *PROVIDER* or *PROVIDER_GROUPS* (unless it's granted *AllPermissions*).

New infrastructure manager or node source policy can be *dynamically plugged* into Proactive Resource Manager.
In order to do that, it is just required to add new implemented classes in the class path and update corresponding list in the
configuration file

    config/rm/nodesource/policies

In the resource manager, there is always a default node source consisted of *DefaultInfrastrucureManager* and *Static policy*.
It is not able to deploy nodes anywhere but makes it possible to add existing nodes to the scheduler.

Out of the box we support time slot policies, cron policies and many others. Please see deatiled information about policies in http://tbs[References section].

==== Agents schedule

*Node source policies* limit computing nodes utilization on the level of Proactive Scheduler. As the same time
 if you need fine-grained limits on machine level you need to use *Proactive Agents*.

TIP: The typical scenario is when you use desktop workstation for computations during working hours.

Both linux and windows agents has an ability to

* run node daemons according to the schedule
* limit resources utilization for these daemons (e.g CPU, memory)

Please see details on http://tbd[agent configuration page]
...
better achieved with tolls like http://www.nagios.org[Nagios].

In the *resource management* interface of Proactive Scheduler you can see how many nodes are added to the
scheduler, which node sources were created and how are using nodes right now.

image::admin_web.png[]

The same info can be seen using the command line
...
For more details about JMX concepts, please refer to official documentation about the http://java.sun.com/j2se/1.5.0/docs/guide/jmx/overview/architecture.html[JMX architecture].
...
The JMX url on which to connect can be obtained from the Authentication API of the RM or by reading the log file located in $SCHEDULER_HOME/.logs/RM.log.
...
image::jmx_jconsole_connect.png[]

Then depending on the allowed permissions browse the attributes of the MBeans.

image::jmx_jconsole.png[]

==== Accounting

The users of Proactive Scheduler request and offer nodes for computation. To keep track on how much node time was
...
+SCHEDULER_HOME/.logs+ directory.

[source]
----
# for problems related to nodes deployment or permissions please check
SCHEDULER_HOME/.logs/RM.log

# for the problems related to jobs execution check
SCHEDULER_HOME/.logs/Scheduler.log
----

Users also have access to server logs of their jobs through the *scheduler web interface*

image::server_logs.png[]

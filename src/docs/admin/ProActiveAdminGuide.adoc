...
* In case the default database is used (HSQLDB), the backup mechanism can be used to store the database content, data and log files of the server.
* In case another database is used (MySQL, Oracle, etc), the integrated backup mechanism can only
be used to store data and log files. Backup of the database contents will need
...
(configurable using the pa.server.backup.possile.delay property). In case a currently running

...
.1+^.^|Networking
|Network, firewall, protocols configuration|config/network/ProActiveConfiguration.ini|<<_network_properties>>
...
----
$ PROACTIVE_HOME/bin/node -r pnp://SCHEDULER_ADDRESS:41303
----
where +-r+ option is used to specify the URL of the *Resource Manager*. You can find this URL in the output of the <<_run_the_proactive_scheduler,ProActive Scheduler>> (the Resource Manager URL).
...
[source]
----
$ PROACTIVE_HOME/bin/client -createns SSH_node_source --infrastructure org.ow2.proactive.resourcemanager.nodesource.infrastructure.SSHInfrastructure SCHEDULER_ADDRESS HOSTS_FILE 60000 3 \"\" /home/user/jdk/bin/java PROACTIVE_HOME Linux \"\" config/authentication/rm.cred
----
...
By default, PNP and PAMR are enabled. PNP is the default protocol for better performance and nodes can also use PAMR if needed.
The PAMR Router is started by default along with the ProActive Scheduler.

For a node to connect to the ProActive Scheduler using PAMR, the following ProActive configuration file can be used
(+PROACTIVE_HOME/config/network/ProActiveConfiguration.ini+).

[source]
----
proactive.communication.protocol=pamr
proactive.net.router.address=ROUTER_HOSTNAME
proactive.net.router.port=8090
----

This sample configuration requires to open only one port +8090+ for *incoming connections* on the router host
...
[source]
----
$ PROACTIVE_HOME/bin/client --listnodes
----

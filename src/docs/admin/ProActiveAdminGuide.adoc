...
The ProActive Scheduler & Resource Manager are able to interact with *PAM (Linux Pluggable Authentication Modules)* to check users login/password.
...
The *first step* to authenticate users with PAM is to configure the PAM login methods in the scheduler and rm settings:
...
The *second step* is to copy the file `PROACTIVE_HOME/config/authentication/proactive-jpam` to `/etc/pam.d` (you must be root to perform this operation).

You can modify this default PAM configuration file if you need specific PAM policies.

The *third step* is to add the user which will start the scheduler process (*PROACTIVE_HOME/bin/proactive-server*) to the shadow group

[source]
----
sudo usermod -G shadow proactive
----
After this command, the user, called "proactive" here in this example will be added to group shadow. You may need to logoff/logon before this modification can be effective.

The *fourth step* will be to add, for each linux system users which will connect to the scheduler, a corresponding group in `PROACTIVE_HOME/config/authentication/group.cfg`

In that case, don't use the *proactive-users* tool, but rather edit the group file manually.

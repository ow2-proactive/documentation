...
* Navigate to the Resource Manager Web Interface. You can find the URL in the output of the <<_run_the_proactive_scheduler,ProActive Scheduler launcher>> (look for the string +Resource Manager webapp+).
...
----
$ cd PROACTIVE_HOME
$ ./bin/unix/rm-client -createns SSH_node_source --infrastructure org.ow2.proactive.resourcemanager.nodesource.infrastructure.SSHInfrastructure rmi://SCHEDULER_ADDRESS:1099/ HOSTS_FILE 60000 3 \"\" /home/user/jdk/bin/java PROACTIVE_HOME Linux \"\" config/authentication/rm.cred
----
...
Agents configuration is detailed in the section <<_proac222tive_agents>>.

...
The `proactive-server` script allocates 4GB heap space. One can change the allocated heap by changing the corresponding options in the `JAVA_OPTS` variable in the `proactive-server` script.

...
It is recommanded to use a PostgreSQL database in a ProActive prod environment.
In the following, we show an example of PostgreSQL installation and configuration to be used along with ProActive.

===== PostgreSQL version
PostgreSQL version 12.2 or 12.9 is required to be installed. PostgreSQL could be installed on the same machine as for ProActive or on a separate server (on-prem or on the cloud).

===== Initialize the database
To initialize the DB, some SQL queries should be executed in the PostgreSQL server machine.
To do so, please create the init script in the /tmp folder for example and put the following SQL queries:
....
-- Create users
...
Set the host name or IP address(es) to listen to:
....
...
they remain present until a purge (VACUUM) is done. Therefore it is necessary to do a purge periodically, especially on frequently-updated tables.
In our case, the preparation for the DB purge operation is done in two steps.

In the first step, we alter several tables in the databases after the first start of the server by executing the following SQL queries:
....
# rm database
\c rm
...
\q
....
In the second step, we remove large objects stored in these tables by executing the following SQL queries.
....
# rm database
vacuumlo -v rm
...
The Postgres driver have to be downloaded and added to the addons folder of ProActive (SCHEDULER_PATH/addons/)
...
By default, the ProActive Resource Manager stores users accounts, passwords, group memberships (user or admins), and tenant memberships (group agglomerate or organization), in two files.
...
A *tenant* can either be a collection of groups (if defined using a file) or an organization unit (if defined using an _LDAP_ attribute).
...
When defined via <<_ldap,LDAP>>, the name of the <<_configure_tenant_attribute,LDAP attribute>> which will contain tenant information must be configured.

Tenants are used to isolate groups of users, where users belonging to one tenant will not see the activities launched by users belonging to other tenants.

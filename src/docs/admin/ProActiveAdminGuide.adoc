...
First, https://www.activeeon.com/community-downloads[Download^] ProActive Scheduler and *unzip* the archive inside a temporary folder that will be used only during the installation (for example `/home/user/temp`).
...
In some cases, this operation can induce some conflicts that can be solved easily using *Git* https://www.themoderncoder.com/fix-git-cherry-pick-merge-conflicts/[command-line tools].
...
'''
.*[underline blue]#Click here to display the Helm input file#*
[example%collapsible]
====
...
- The `password-template.properties` file contains the clear-text passwords required for installation, which will be encrypted during the process. This file is used only for a specific installation method (more details can be found here: <<_shell_Starting_ProActive_using_Shell_scripts, Starting ProActive using Shell scripts>>).
...
- The `lib` folder, containing the libraries required for the installation (e.g., postgresql jar driver).
...
The `inputConfig.json` file, included in the installation package, is necessary to provide the specific configuration values for installing ProActive.

This section explains the configuration parameters required, as specified in the `inputConfig.json` file.

'''
.*[underline blue]#Click here to display the configuration input file#*
[example%collapsible]
====
...
**Important:** Please note that all password fields defined in the configuration sections listed below **should be left empty**. Their values will be provided at runtime. They will be then encrypted and inserted back into the `inputConfig.json` file.
We provide a more detailed explanation of how to configure the password values in the next section <<_shell_Starting_ProActive_using_Shell_scripts, Starting ProActive using Shell scripts>>.
...
*** `DB_SCHEDULER_PASSWORD`: Database user encrypted password. This value should be left empty.
...
After configuring the necessary settings as described in the previous section, you are ready to begin the installation of the ProActive server using the `proactiveInstaller.sh` script.

The `proactiveInstaller.sh` script simplifies the installation and configuration process by automating key tasks such as password encryption and offering flexibility through offline and upgrade modes.

Before exploring the installation options, it is important to note that `proactiveInstaller.sh` must be executed with root privileges, and you need to grant it execution permission:
...
This script supports three different modes for password encryption: via command-line arguments, interactive, and automatic (using a password file).
It ensures that you can tailor the installation method based on your environment and security requirements.

The help menu provides a detailed description of the available options:

- **CLI Arguments Mode** (`-p passphrase -admin_pwd admin_password -user_pwd user_password`): Allows you to pass the passphrase and passwords directly as command-line arguments for minimal installation setups.
- **Interactive Mode** (`-i`): Prompts you to enter passwords and passphrases interactively, followed by automatic installation.
- **Automatic Mode** (`-a -f file_path`): Uses a predefined file with clear-text passwords for installation, ideal for automated deployments.

Additional options include:

- **Offline Mode** (`-o`): Ensures the installation process can be completed without internet access.
- **Upgrade Mode** (`-u`): Allows upgrading an existing ProActive installation.

The options `-o` and `-u` can be used with any installation mode (`-i`, `-a`, or `-p`).

Below is the help output that showcases the available options and example commands:

----
$ sudo ./proactiveInstaller.sh -h
 This script automates the installation and configuration process for ProActive, including password encryption and optional offline or upgrade modes.
 The script supports 3 modes for passwords encryption: interactive, automatic, and CLI arguments.

 Usage: ./proactiveInstaller.sh [-i | -a -f file_path | -h | -p passphrase [-admin_pwd admin_password -user_pwd user_password] | -u] [-o]
 (Only one of the following installation methods can be used at a time: -i, -a, -p, or -h):
   -i              Proceed with the interactive password encryption and then automatic installation mode
   -a              Proceed with the automatic installation mode (requires -f)
      -f file_path Specify the clear passwords file path to be used in automatic installation mode
   -h              Display this help message
   -p passphrase   Specify the passphrase for password encryption. Without -u, the following passwords are required:
      -admin_pwd   ProActive login admin password (mandatory unless -u is used)
      -user_pwd    ProActive user password (mandatory unless -u is used)
   -o              (optional) Enable offline mode. Can be used with -i, -a, or -p
   -u              (optional) Enable upgrade mode. Can be used with -i, -a, or -p and makes -admin_pwd and -user_pwd optional

 Examples:
   sudo ./proactiveInstaller.sh -i
   sudo ./proactiveInstaller.sh -i -o
   sudo ./proactiveInstaller.sh -i -u
   sudo ./proactiveInstaller.sh -i -o -u
   ...
   sudo ./proactiveInstaller.sh -a -f ./password-template.properties -o -u
   sudo ./proactiveInstaller.sh -p 1234 -admin_pwd adminPass123 -user_pwd userPass456
   sudo ./proactiveInstaller.sh -p 1234 -admin_pwd adminPass123 -user_pwd userPass456 -o
   sudo ./proactiveInstaller.sh -p 1234 -u
   sudo ./proactiveInstaller.sh -p 1234 -u -o
----
...
During this mode, you must provide a valid passphrase for password encryption using the `-p` option, followed by the two required passwords for a basic installation: the admin login password (`-admin_pwd`) and the user password (`-user_pwd`).

The ProActive installer will use the provided passphrase to encrypt the passwords and insert them, in encrypted format, into the `inputConfig.json` file.

Example:
----
$ sudo ./proactiveInstaller.sh -p MySecurePassphrase -admin_pwd AdminPass123 -user_pwd UserPass456

 CLI installation mode is set.
 Running in CLI installation mode.
...
----

If you want to start the ProActive server installation and configuration offline (without requiring internet access), you can include the `-o` option in the installation command to ensure it runs in offline mode:
----
$ sudo ./proactiveInstaller.sh -p MySecurePassphrase -admin_pwd AdminPass123 -user_pwd UserPass456 -o
----

NOTE: It is important to ensure that the command-line interface where you provide the passwords is secure, as passwords may be visible in the process list.

NOTE: Ensure that the passphrase used is strong and secure since it will be used to encrypt sensitive passwords.
...
To use this mode, specify the installation option `-i` for interactive mode, after which the installer will prompt you to enter the passphrase and the required passwords.
...
Once provided, the installer will encrypt the passwords using the passphrase and store them in the `inputConfig.json` file in encrypted format.

Example:
----
$ sudo ./proactiveInstaller.sh -i

 Interactive installation mode is set.
 Running in interactive installation mode.
You are going to set the passphrase for password encryption [y/n] y
Enter Passphrase:
Retype Passphrase:
 Passphrase is set.
Do you want to set ProActive admin password? [y/n] y
Enter ProActive admin password:
Retype ProActive admin password:
 ProActive admin password is set.
Do you want to set ProActive user password? [y/n] y
Enter ProActive user password:
Retype ProActive user password:
 ProActive user password is set.
Do you want to set the TLS certificate keystore password? [y/n] n
Do you want to set LDAP configuration passwords? [y/n] n
Do you want to set ProActive users passwords? [y/n] n
Do you want to set ProActive external database passwords? [y/n] n
...
----
...
During this mode, you specify the installation mode using `-a` option (for automatic) and the path to a clear-text password file using the `-f` option (e.g., `-f ./password-template.properties`) that contains the clear-text passwords for various configurations, such as admin, user, LDAP, TLS, or database passwords.

A pre-filled `password-template.properties` file is provided in the installation package, and you can use it to provide the clear-text passwords.
If some advanced configurations are disabled in the `inputConfig.json` file (e.g., LDAP or database configuration are disabled), the relative passwords will be simply ignored.

The installer will then use the provided passphrase in the password file to encrypt the passwords and insert them into the `inputConfig.json` file.
...
$ sudo ./proactiveInstaller.sh -a -f ./password-template.properties
 ...
 Automatic installation mode is set.
 Using file: ./password-template.properties for automatic installation.
 Running in automatic installation mode.
...
----
...
NOTE: After the installation is complete, it is recommended to delete all clear-text passwords to improve security. In case you want to proceed with an installation upgrade (`-u`), the `PASSPHRASE` value will be required.
...
==== Postgres Database Purge Operation
The Postgres database purge process is required on any fresh ProActive installation using the Postgres database, and it explained in section: <<_compose_purge, Docker Compose Purge Operation>>. If you are not using a Postgres database in your ProActive installation, please ignore this section.
...
----
Now, while the ProActive server is running, you can use the given script `purge-operation.sh` to execute a full vacuum regularly using a cron expression for example:
----
$ sudo crontab -e
----
...
First, a link to the new ProActive release has to be provided in the `inputConfig.json` file:
...
**Note**: You can apply the upgrade in offline mode using the `-o` option together with `-u` option.
...
[NOTE]
====
It is also possible to configure the User Space without impersonation. In that case, all connections to the SFTP server will be done under a single user. In order to do this, `PROACTIVE_HOME/config/scheduler/settings.ini` will contain:
....
pa.scheduler.dataspace.defaultuser.url=sftp://username@your-server/path
pa.scheduler.dataspace.defaultuser.impersonation=false
....
See <<_sftp_global_space_configuration>> for a detailed explanation of this configuration.
====
...
`path` refers to a folder path relative to the HOME directory of `username`.
 ...
 * In `PROACTIVE_HOME/config/network/server.ini` and `PROACTIVE_HOME/config/network/node.ini`, add the following property:
+
....
proactive.dataspaces.sftp.useSystemPrivateKey=/path/to/private-key
....

 * In each <<_glossary_node_source,Node Source>> defined in the <<_glossary_resource_manager,Resource Manager>>, you must ensure that the java property `-Dproactive.dataspaces.sftp.useSystemPrivateKey=/path/to/private-key` is set for all Nodes deployed by the Node Source (this configuration is available in the Node Source parameters).

 * Finally, you must ensure that:

 ** The private key used does not contain a passphrase.
 ** The associated public key is added to the `authorized_keys` of `username` on the SFTP server.
 ** The private key is physically present on each <<_glossary_proactive_node,Node>> connected to the <<_glossary_resource_manager,Resource Manager>>. The private key location on the Node's file system must match the java property `proactive.dataspaces.sftp.useSystemPrivateKey` configuration for this Node (The private key location can differ from one Node to another).
...
[[_glossary_custom_logo]]
=== Custom logo configuration

image::generic_logo.png[align="center"]

You can change the customer logo interactively in the Automation Dashboard portal (Catalog, Workflow Execution, etc...) using the `Change Customer Logo` menu item from the Help menu located in the top left corner.

Alternatively, you can change the logo using command line by replacing the image `PROACTIVE_HOME/dist/war/getstarted/assets/image/custom-logo.png` with your company custom logo.
...
The following property must be configured to define allowed domains:

 * in `PROACTIVE_HOME/config/shared/settings.ini`
----
...
By default, the scheduler displays all submitted jobs (though job logs, results, etc are visible only by job owners or administrators).

Tenant isolation allows to restrict jobs displayed on the scheduler portal to the job owner's tenant.
...
When this configuration is enabled, standard users will only see jobs according to the user's tenant. Users that belong to an administrator group with the permission `org.ow2.proactive.scheduler.permissions.TenantAllAccessPermission` enabled will be able to see jobs belonging to any tenant (see ).
...
.Scheduler Portal for Global Administrator
image::TenantSPAdmins.png[Admins Tenant View]

===== Tenants Global Space Isolation

As described in the <<../user/ProActiveUserGuide.adoc#_data_spaces,Data Spaces>> chapter, the *User* and *Global Spaces* are server-side file storage spaces that can be accessed by either workflow tasks or the ProActive portals.

By default, the <<../user/ProActiveUserGuide.adoc#_global_and_user_spaces,Global Space>> is public and any user can download/upload files from/to it.

In a multi-tenant configuration, this default behavior can be problematic as users belonging to one tenant may not want their files to be accessed or modifed by another tenant's users.

In order to isolate global space accross tenants, simply set `pa.scheduler.core.tenant.filter` to `true` in `PROACTIVE_HOME/config/scheduler/settings.ini`.

This is the same configuration as <<_tenants_job_isolation>>, so both job and global spaces will be isolated when this setting is applied.

The default location of the Global Space on the ProActive server is at `PROACTIVE_HOME/data/defaultglobal`. See <<_data_spaces_configuration>> for more information about this configuration.

In case tenant isolation is enabled, each tenant will have a dedicated subfolder in the global space, e.g. `PROACTIVE_HOME/data/defaultglobal/tenant1`, `PROACTIVE_HOME/data/defaultglobal/tenant2`, etc.

Each connected user of tenant1 will have its root global space at `PROACTIVE_HOME/data/defaultglobal/tenant1` and will only see files inside this directory.

Users that belong to an administrator group with the permission `org.ow2.proactive.scheduler.permissions.TenantAllAccessPermission` enabled will have their root global space at `PROACTIVE_HOME/data/defaultglobal`. They will thus see several subfolders, one for each tenant, allowing these administrators full access to each tenant global spaces files.

TIP: During the installation of ProActive, the Global Space is populated by default files used by several workflows inside the ProActive Catalog library. This includes for example JDBC drivers, default input files, etc. It is therefore recommended to copy the default content of `PROACTIVE_HOME/data/defaultglobal` inside each tenant folder.

Below are examples view of the Global Space by different users (we did not follow the advice above to make the screenshot more understandable):

.Global Space View for Global Administrator
image::TenantGSAdmin.png[Global Space View for Global Administrator]

Here we can see the default global space files along with _tenant1_ and _tenant2_ subfolders.

.Global Space View for Tenant1 Users
image::TenantGSTenant1.png[Global Space View for Tenant1 Users]

Here we see only some files created by Tenant1 users.

.Global Space View for Tenant2 Users
image::TenantGSTenant2.png[Global Space View for Tenant2 Users]

Here we see only some files created by Tenant2 users.



===== Tenants Resource Manager Isolation

Apart from scheduler job isolation, it is also possible to isolate the <<_glossary_resource_manager,Resource Manager>> and <<_glossary_node_source,Node Sources>> across tenants.

By default, the Resource Manager displays all Nodes and Node Sources, regardless of tenants.
Tenant isolation allows to restrict the Nodes and Node Souces that will be visible on the Resource Manager portal.

In this scenario, the Resource Manager may contain:

 * Node Sources that belong to a single tenant and thus only visible to members of this tenant.
 * Node Sources that don't belong to any tenant and thus visible to all users, regardless of tenant membership.

Statistics - such as the number of nodes and nodes history - displayed in the Resource Manager portal will match the user's visibility access.

In order to enable Resource Manager tenants isolation, the following properties must be configured in `PROACTIVE_HOME/config/rm/settings.ini`:
----
pa.rm.tenant.filter=true
pa.rm.jmx.tenant.names=tenant1,tenant2
----

`pa.rm.jmx.tenant.names` contains the list of tenants names. This setting is mandatory to allow Resource Manager statistics isolation.

When the above properties are configured, the following behavior applies:

 * A Node Source created by an administrator who belongs to a given tenant will be visible only to members of the same tenants. This Node Source will contain Nodes that are restricted to a single tenant.
 * A Node Source created by an administrator who does not belong to any tenant will be visible to all users, regardless of tenants. This Node Source will contain Nodes that are shared across tenants.

From a given user point of view, the following applies:

 * A user who does not belong to any tenant will only see shared Node Sources.
 * A user who belongs to a tenant will see shared Node Sources and Node Sources associated with the same tenant.
 * A user or administrator who has the permission `org.ow2.proactive.scheduler.permissions.TenantAllAccessPermission` will see all Node Sources.

In order to create a Node Source restricted to a tenant, it is necessary to create a Resource Manager administrator group attached to this tenant.

The command line tool <<_copy_role_command,Copy Role>> can be used to create such group. For example, the following command duplicates the core Resource Manager administrator role to the group `tenant1admin` belonging to tenant `tenant1`:

----
tools/copy-role -S rmcoreadmins -D tenant1admin
----

Below are different views of the Resource Manager for:

 * A global administrator with `org.ow2.proactive.scheduler.permissions.TenantAllAccessPermission`

.Resource Manager Portal view of Global Administrator
image::RMAllTenant.png[RM All Tenant View]

In this view the Global Administrator sees all Node Sources, and the Resource Manager statistics contain all Nodes.

 * A `tenant1` Resource Manager administrator

.Resource Manager Portal view of `tenant1` Administrator
image::RMTenant1View.png[RM Tenant1 View]

In this view the `tenant1` administrator sees only the shared Node Source and a Node Source belonging to `tenant1`. The Resource Manager statistics contain only these Nodes.

* A `tenant2` Resource Manager administrator

.Resource Manager Portal view of `tenant2` Administrator
image::RMTenant2View.png[RM Tenant2 View]

In this view the `tenant2` administrator sees only the shared Node Source and a Node Source belonging to `tenant2`. The Resource Manager statistics contain only these Nodes.

NOTE: It is also possible to give tenant access to a Node Source by using the `nodeUsers` parameter which is common to all <<_glossary_node_source_policy,Node Source Policies>>. See <<_policy_common_parameters,Policy Common Parameters>> for more information.

===== Tenants Catalog Isolation

The last service that can benefit from tenant isolation is the <<_glossary_catalog,ProActive Catalog>>.

By default, the ProActive Catalog contains buckets which can either be *public* or *restricted to a user group*.

In a multi-tenant architecture, the default behavior can be problematic as:

 * Any *public* bucket created by users of a tenant will be also visible by the other tenants users. Modifications applied by other tenants users could also impact the bucket creator processes.
 * Similarly, default buckets that are included inside the ProActive release, may be modified by users coming from different tenants, possibly impacting the other tenants processes.

In order to address these issues, ProActive catalog features a tenant isolation configuration. The following property must be set in `PROACTIVE_HOME/dist/war/catalog/WEB-INF/classes/application.properties`:

----
pa.catalog.tenant.filtering=true
----

When Catalog tenant isolation is enabled, the following behavior applies:

 * Any *public* bucket created by users of a tenant will only be visible to users of the same tenant.
 * Any *public* bucket created by users who do not belong to any tenant will be visible by all users.
 * A bucket can be restricted to a single user group. This is also possible without tenant isolation configured.
 * A bucket restricted to a single user group can have *grant* rules as described in <<../user/ProActiveUserGuide.adoc#_catalog_rbac_management,Catalog RBAC Management>>.
It is possible for example to create a *tenant read grant* for a group-restricted bucket.

Accordingly, when installing the ProActive server with catalog tenant isolation, we should:

 * Ensure that default buckets coming with ProActive release are restricted to the global administrator group `server-admins` of the user `admin`. See <<_default_groupsroles>>.
 * Add tenant read *grants* to the default buckets that need to be visible to the tenants users. See  <<../user/ProActiveUserGuide.adoc#_catalog_rbac_management,Catalog RBAC Management>>.
The list of buckets to include should contain __a-minimum__: `basic-examples`, `calendars`, `control-notification`, `control-signal`, `control-templates`, `event-rules`, `scripts` and `service-automation`.

In order to automatically restrict the default buckets to `server-admins`, you need to edit the script `PROACTIVE_HOME/tools/LoadPackage.groovy` and change the following line:
...
----
    private String BUCKET_OWNER = "GROUP:server-admins"
----

This change must be done before starting the ProActive server for the first time. As the ProActive Catalog will be populated by this script with default buckets at first start of the server.

Below are examples of the Catalog Portal view with tenant isolation enabled as seen by different users.

* A global administrator with `org.ow2.proactive.scheduler.permissions.TenantAllAccessPermission`

.Catalog Portal view of Global Administrator
image::CatalogAllTenant.png[Catalog All Tenant View]

* A `tenant1` user

.Catalog Portal view of Tenant1 user
image::CatalogTenant1View.png[Catalog Tenant1 View]


* A `tenant2` user

.Catalog Portal view of Tenant2 user
image::CatalogTenant2View.png[Catalog Tenant2 View]
...
Default passwords for admin and user accounts are `admin/admin` and `user/pwd`. Make sure to change these passwords and remove any unnecessary account present in the default login file.

`rm`, `scheduler` and `watcher` are internal accounts which must remain.

Example:

----
admin:$2a$12$2IVFh1cV6/m8yUuYF4e/tO7jyGHK3R7L5dVb/.84ibvWh1i/16n8K
rm:$2a$12$reLZNyffRQmZ1ACZNn85x.rosnt4AXEnsn/tfEq6bxw0mCT2Qwbl6
scheduler:$2a$12$.cmyzTeCiFo8pxr6k9mCF.rNqwUMS2OY2zwlRh1lTQbjkqePd9O5q
user:$2a$12$GWJnc6LaQNNHoCtusN626efZiCEWCA5rBPA7jBeLiWgERmxOlTs8C
watcher:$2a$12$ko6Hf7SX.dqTY.zYKFUBLOrM1Kx8Ujuvb8uGAVKUTgvRmzZYCXdUG
----

WARNING: The format of encrypted passwords has evolved since ProActive 14.1.0. Passwords are now stored in hash/salt format,
while they were before ciphered using hybrid encryption. During an upgrade from a prior version, if using one of the automated
upgrade methods such as <<_how_to_upgrade_proactive_on_linux>>, the password encryption will be automatically updated.
If a different method is used, the following command can be used to convert passwords to the new format:
`PROACTIVE_HOME/tools/regenerate-passwords -ns -ltn -d`

===== Password Strength

Creating users or modify an existing user's password should be done using the `PROACTIVE_HOME/tools/proactive-users` command line. See <<_command_line>>.

It is possible to control the acceptable password strength by using the following options in `PROACTIVE_HOME/config/shared/settings.ini`:
...
The default regular expression configured ensures that passwords must be between 8 to 32 characters long and have at least a uppercase letter, a lowercase letter, a digit and a symbol.

In order to enable this feature, simply set `pa.password.strength.enable=true` and modify `pa.password.strength.regexp` and `pa.password.strength.message` according to your needs.

WARNING: this configuration ensures that a valid password is entered when creating a user or modifying an existing user password through the `tools/proactive-users` command line. It does not impact existing users current passwords.

===== Users Login Regular Expression

The following settings are enabled by default to control login attempts and prevent special characters injection in third-party authentication frameworks (LDAP, Active Directory, etc).
...
This ensures that login contains only alphanumeric characters and only the symbols *_*,*-*,*@* and *.*

In case these regular expressions do not match your existing user's login names, you should modify these settings.
...
Shadow accounts are an optional feature that can be enabled in the LDAP configuration file.

When enabled, whenever a user logs in successfully using LDAP credentials, an internal shadow account with a randomly generated password will be created.

This shadow account's credentials will be used internally by ProActive whenever the user submits a job, plans a job through the *Job Planner*, starts a service using *Cloud Automation*, etc.

Shadow accounts ensure that LDAP user credentials are never stored inside the ProActive server.

In order to enable shadow accounts, set the following property in `PROACTIVE_HOME/config/authentication/ldap.cfg`:
...
pa.ldap.authentication.shadow.users=true
----

WARNING: When shadow accounts are configured, `pa.ldap.authentication.fallback` and `pa.ldap.group.membership.fallback` must be set to `true`.

==== Specific example of LDAP configuration

Below is an example of LDAP configuration which summarizes the LDAP properties frequently configured in the file `PROACTIVE_HOME/config/authentication/ldap.cfg`.
[source,properties]
----
...
+
4. In `config\shared\settings.ini`, uncomment the property `pa.multi.ldap.config` and define it according to the ldap configuration files you created and their corresponding domain names:
+
----
...
+
----
pa.allowed.domains=,domain1,domains2
----
+
...
* `server-admins` : this group combine the rights of `rmcoreadmins` and `scheduleradmins` groups. Additionally, this group has access to all tenants, is a catalog service administrator, and can manage users, groups or defined tenants.
Accordingly, `server-admins` is a _global administrator_ role.
...
----
permission org.ow2.proactive.permissions.ChangePasswordPermission;
----
Permission that allows a user to change his own password. Only valid for internal accounts, not for accounts imported from external authentication frameworks such as LDAP.

----
permission org.ow2.proactive.permissions.ManageUsersPermission;
----
_Administrative_ permission that allows to manager internal users, groups or defined tenants.
...
----
This permission gives _administrative_ access to the <<_glossary_catalog,ProActive Catalog>>. A member of a group with such permission has administrator access to any catalog bucket or object.
...
In order to decide which pending Tasks will be scheduled for execution, the ProActive Scheduler uses a <<_glossary_scheduling_policy,*Scheduling Policy*>>.
...
The default scheduling policy, called _ExtendedSchedulerPolicy_:

 * Excludes pending tasks when the <<../user/ProActiveUserGuide.adoc#_start_at,START_AT>> generic information is set, and the configured time is still in the future.
 * Sort Pending tasks according to their corresponding <<../user/ProActiveUserGuide.adoc#_job_priority,Job Priority>>, and then by _First-In-First-Out_ order.
...
In the following sections, we show several alternate policies that can be used. Each of these policies inherits from the _ExtendedSchedulerPolicy_, which means that they apply the default policy yet may override the default behavior in some cases.

=== Earliest deadline first (EDF) policy

*Early Deadline First Policy* sorts jobs based on (by decreasing importance):

 1. job priority (from highest to lowest).
 2. presence of a deadline (those that have a deadline overtake those without a deadline).
 3. job started or not (jobs already started overtake those not already started), and jobs started first overtake those started last.
 4. For pending jobs, sort them according to the configured deadline (earliest deadlines overtake others).
...
Each workflow can configure its deadline by using the <<../user/ProActiveUserGuide.adoc#_earliest_deadline_first_policy,JOB_DDL>> and <<../user/ProActiveUserGuide.adoc#_earliest_deadline_first_policy,JOB_EXEC_TIME>> generic information.
...
To use the _LicenseSchedulingPolicy_, edit the file `PROACTIVE_HOME/config/scheduler/settings.ini` and change the following line:
...
Users declare their usage of software licenses by adding the <<../user/ProActiveUserGuide.adoc#_required_licenses,REQUIRED_LICENSES>> generic information at job level or task level.

`REQUIRED_LICENSES` expects a comma-separated list of software names defined in the above configuration file, e.g.:

----
softwareA,softwareB
----

The behavior of `REQUIRED_LICENSES` is different when declared at job-level or task-level.
...
It is also possible to combine - in the same workflow - `REQUIRED_LICENSES` at both levels.
...
To use the _NodeUsageSchedulingPolicy_, edit the file `PROACTIVE_HOME/config/scheduler/settings.ini` and change the following line:
...
In order to configure the number of Nodes a workflow is allowed to use in parallel, set the <<../user/ProActiveUserGuide.adoc#_max_nodes_usage,MAX_NODES_USAGE>> generic information at job-level.

...
ProActive could be easily deployed and started in a Kubernetes cluster. For this purpose, we provide a ready-to-use Kubernetes configuration files that enable you running ProActive Workflows and Scheduling as a set of Pods.
...
apiVersion: v1
data:
  DB_CATALOG_PASS: SjSWUF72s?C*eWhB
  DB_NOTIFICATION_PASS: udEHNmxWp*JBrq6?
  DB_PCA_PASS: rtj!BZ94!JX*nbDk
  DB_RM_PASS: Z*+ra3vDEsyrDrW?
  DB_SCHEDULER_PASS: b93r*V=c77cs42Py
  DB_TYPE: default
  DYNAMIC_NS: "true"
  ...
  JOB_CLEANUP_DAYS: "30"
  MYSQL_ROOT_PASSWORD: YhUk*5tc_aS7Rtdp
  PAMR_PORT: "33647"
  PORT: "8080"
  POSTGRES_PASSWORD: 87zF=JPZAc%UVby_
  PROACTIVE_ADMIN_PASSWORD: activeeon
  PROTOCOL: http
  ...
  template:
    ...
    spec:
      ...
      - envFrom:
        - configMapRef:
            name: env-config-g8cf4cd4d8
        image: dockerhub.activeeon.com/k8s/postgres-pa:12.2
        imagePullPolicy: IfNotPresent
        name: proactive-db
        ports:
        - containerPort: 5432
  ...
  template:
    ...
    spec:
      ...
      - envFrom:
        - configMapRef:
            name: env-config-g8cf4cd4d8
        image: dockerhub.activeeon.com/k8s/proactive-scheduler-external-db:12.0.0
        imagePullPolicy: IfNotPresent
        livenessProbe:
          failureThreshold: 24
          httpGet:
      ...
      - command:
        - sh
        - -c
        - until nc -vz -w 3 proactive-database 5432; do echo "Waiting for postgres..."; sleep 3; done;
        image: busybox:latest
        imagePullPolicy: IfNotPresent
        name: wait-for-pg
      volumes:
  ...
  template:
    ...
    spec:
      ...
      - envFrom:
        - configMapRef:
            name: env-config-g8cf4cd4d8
        image: dockerhub.activeeon.com/k8s/mysql-pa:8.0
        imagePullPolicy: IfNotPresent
        name: proactive-db
        ports:
        - containerPort: 3306
  ...
  template:
    ...
    spec:
      ...
      - command:
        - sh
        - -c
        - until nc -vz -w 3 proactive-database 3306; do echo "Waiting for mysql..."; sleep 3; done;
        image: busybox:latest
        imagePullPolicy: IfNotPresent
        name: wait-for-mysql
      volumes:

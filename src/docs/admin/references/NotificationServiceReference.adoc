...
[[third-party-binding]]
====== User scoped Third-party user parameter

When <<multiple-executions-mode,multiple execution mode>> is activated, the user which is notified can add
an additional argument to the script. This parameter will be used inside the script as a new
binding named `args[]` and is injected in script executions for *all user's subscriptions*.
...
====== Subscription scoped Third-party parameter

In addition to the User scoped parameter, each subscription allows an extra parameter to be passed to script executions, defined for *each subscription*.

[title="Subscription Third-party parameter"]
image::subscription-third-party-param.png[align="center"]

The subscription specific parameter is an extra element in the args array shown just before.

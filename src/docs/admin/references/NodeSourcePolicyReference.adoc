 ...
 * `z(temp=nodeSources.get('FirstNS')) {plus}`: read the state of the first node source and store it into a temporary variable. Notice the use of the `z()` function and the `{plus}` sign, so that this affectation does not change the value returned by the expression.

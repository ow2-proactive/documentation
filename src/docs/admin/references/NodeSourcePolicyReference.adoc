...
-   *nodeUsers* - see <<_policy_common_parameters>>

-   *nodeProviders* - see <<_policy_common_parameters>>
...
Symmetrically, if an infrastructure defines a minimum number of nodes to acquire, this policy will never go beyond this number.
...
and so on ...
The load factor accepts decimal values to allow more precise control (2/3, etc)

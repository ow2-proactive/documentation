...
1.  First, you need to have an admin account on your OpenStack server

2.  Use the login and password information to fill in the properties *Openstack_Username*,
    *Openstack_Username* in the *Create Node Source* panel located in the resource manager.
    Those two parameters should never change, except if you need for
    some reason to handle multiple OpenStack accounts. Other properties in the
    *Create Node Source* are:
    ...
    -   *publicKeyName:* Defines the name of the public key to push to the instance when it is created.
...
+
    ...
    -   *downloadCommand:* The command to download the Proactive node.jar. This command is executed in all the newly created OpenStack instances at cloud init time.
    	The full URL path of the node.jar to download, needs to be accessible from the OpenStack cloud.
        Example based on OpenStack image with windows operating system:
...
WARNING: When the Scheduler is stopped, the OpenStack instances will not be terminated.

When the Scheduler is restarted, all the OpenStack instances that were previously created, will be terminated first and then re-created and re-configured as per previous settings.
...
1.  First, you need to have an admin account on your VmWare server

2.  Use the login and password information to fill in the properties *VmWare_Username*,
    *VmWare_Username* in the *Create Node Source* panel located in the resource manager.
    Those two parameters should never change, except if you need for
...
+
    ...
    -   *downloadCommand:* The command to download the Proactive node.jar. This command is executed in all the newly created VmWare instances at cloud init time.
    	The full URL path of the node.jar to download, needs to be accessible from the OpenStack cloud.
        Example based on VmWare image with windows operating system:

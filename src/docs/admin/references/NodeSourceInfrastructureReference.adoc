...
NOTE: New Node Source Infrastructures can be dynamically plugged into the Resource Manager. In order to do that, it is required to add classes extending the link:../javadoc/org/ow2/proactive/resourcemanager/nodesource/infrastructure/InfrastructureManager.html[InfrastructureManager class] in the server class path and update the policy implementation list in the configuration file (+PROACTIVE_HOME/config/rm/nodesource/infrastructures+).
...
WARNING: When using the command line, the order of parameters is important, and each infrastructure section below describes the correct parameter order. Each section show as well the fully qualified name of the infrastructure, to pass on the command line.
...
-   *javaProperties* - Java options appended to the command used to start the node on the remote host.
...
-   **nodeTimeOut** - A duration after which one the remote nodes are
    considered to be lost.

-   **maxDeploymentFailure** - Maximum number of failed attempt to deploy on host before discarding it.

-   **waitBetweenDeploymentFailures** - The time in milliseconds that the Resource Manager wait before trying to
...
This infrastructure allows to deploy nodes over SSH.
This infrastructure needs 14 arguments, described hereafter:
...
-   **nodeTimeOut** - The length in ms after which one a node is not
  expected anymore.

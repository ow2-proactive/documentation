 ...
 ** Deallocate: When an instance is evicted, it is deallocated but not immediately deleted.
 In both cases, ProActive will be able to automatically reschedule any workloads (Tasks) on available Nodes provided by another Azure Scale Set with regular instances.
...
- *maxPrice:* Defines the maximum price in USD that you are willing to pay for Low or Spot virtual machines in the scale set. By providing the value of -1, you set no maximum price.

...
- *image:* Docker image used to deploy ProActive Nodes. Example: java:8-jre
...
if ! command -v java; then wget -nv -N https://s3.amazonaws.com/ci-materials/Latest_jre/jre-8u131-linux-x64.tar.gz; tar -xf jre-8u131-linux-x64.tar.gz; export PATH=/tmp/node/jre1.8.0_131/bin/:$PATH; fi

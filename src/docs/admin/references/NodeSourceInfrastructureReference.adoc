...
- *nodeJarDownloadCommand:* command which will be executed inside the pod to download *ProActive libraries*. Thanks to this command, ProActive libraries are downloaded when the pod starts and prior installation of the libraries inside the Docker image is not required.
This command needs to download using the http(s) protocol the `node.jar`from the ProActive Resource Manager server.
+
Examples:
....
wget -P /opt/proactive/ http://192.168.99.1:8080/rest/node.jar

wget -P /opt/proactive/ --no-check-certificate https://192.168.99.1:8443/rest/node.jar
....

- *rmCredentials:* path to a file which contains the credentials of an administrator user which will own the node source.
...
- *javaNodeJarCommand:* command used to start the ProActive Node. The default value needs to be modified according to the location of the java command.
+
Example:
....
java -jar /opt/proactive/node.jar
....
...
Example:
....
-Dproactive.communication.protocol=pamr -Dproactive.useIPaddress=true -Dproactive.pamr.router.address=192.168.99.1
....

 - *kubernetesNamespace:* (Optional) Kubernetes namespace if different from _default_ namespace.
 - *image:* Docker image used to deploy ProActive Nodes. This image must contain a Java Runtime Environment. Example: java:8-jre
 - *nodeTimeout:* (Optional) The estimated startup time of the nodes (expressed in millisecond). After this timeout expired, the node will be considered as lost. Default value is 10 minutes.

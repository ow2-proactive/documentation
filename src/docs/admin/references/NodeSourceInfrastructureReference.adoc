...
-   *image:* Defines which image will be used to create the GoogleComputeEngine instance.
        ...
        If not provided, then the default value "debian-9-stretch-v20190326" will be used.
        For more information see https://cloud.google.com/compute/docs/images[Google Compute Engine Images]
-   *region:* The geographic zone for Google Cloud Platform resources to use.
        If not provided, then the default value "us-central1-a" will be used.
...
WARNING: When the Scheduler is stopped, the GoogleComputeEngine instances will be automatically deleted. And when the Scheduler is restarted, the infrastructure will be recovered as previous settings. The required GoogleComputeEngine instances will be re-created and re-configured.

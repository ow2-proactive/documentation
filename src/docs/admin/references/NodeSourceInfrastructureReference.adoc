...
the nodes will attach the nodes to the infrastructure without modifications to
the startup command. This mode is selected by default.

*** _useStartupScript_: wtarts the ProActive node using the script in the variable
`%startupScriptStandard%`, Allowing the user to modify the startup command of the nodes.
This mode uses the ProActive node agent identified in the `%schedulingPath%`.


*** _useNodeJarStartupScript_: enables connecting to the SSHInfrastructureV2
using node.jar file. This mode uses `%nodeJarUrl%` and `%startupScriptWitNodeJarDownload%`
...
on each node added to the hostList. The URL have to be accessible from the nodes.
...
The script by default locates java and node agent directory using `%javaPath%` and `%schedulingPath%` variables respectively.
The user can modify or extend this script to make changes to the node before or after the startup.
...
then download and execute ProActive _node.jar_, if they are not already provided by the node.

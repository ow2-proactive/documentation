...
4.  Use this information to fill in the properties *aws_key* (Access
    ...
    *Create Node Source* are:
    ...
    -   *rmDomain:* The public IP address of the resource manager. This IP needs to be accessible from the AWS cloud. 
    
    -   *connectorIaasURL:* Connector-iaas is a service embedded in the scheduler used to interact with IaaS like AWS. By default it runs on the same domain and port of the scheduler.    
...
+
    ...
    -   *operatingSystem:* The operating system running in the AWS instance.
    ...
    -   *additionalProperties:* Additional java command properties to be added when connecting the node.jar to the resource manager (e.g. \"-Dpropertyname=propertyvalue\").
     
    -   *ram:* The desired RAM amount expressed in Mega Bytes for each AWS instance that needs to be created.
    
    -   *cpu:* The desired CPU amount amount expressed in units for each AWS instance that needs to be created.

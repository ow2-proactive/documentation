...
2. The creation of Openstack Infrastructure asks for an authentification to the Openstack server and a deployment of instances to host proactive nodes.
...
Other properties needed for the authentification are :
  ...
  -   *endpoint:* The hostname or the IP address of the OpenStack server. This address needs to be accessible from the Resource Manager.
   ...
   -   *scopePrefix:* The scope prefix to use
   ...
   -   *identityVersion:* The REST API version of openstack. For more information see https://docs.openstack.org/keystone/pike/contributor/http-api.html[OpenStack API Version]. 

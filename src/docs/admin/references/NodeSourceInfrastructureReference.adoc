...
However, it implements a different instance management from it to lower the delay of node acquisition and node removal process and ease inter-node collaboration in the same cluster, thanks to the following changes:
...
The configuration of the AWS Autoscaling infrastructure is subjected to several requirements.
 ...
 You can get additional information about the PAMR protocol in the secion <<_installation_on_a_cluster_with_firewall>>.
